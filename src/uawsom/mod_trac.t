...
!
!    call uawsom_get_pthermal(w,x,ixI^L,ixO^L,pth)
...
!  end subroutine get_Tmax_grid  
  ...
  subroutine get_Te_grid()
    ...
    do j=1,ngrid_trac
      ...
      call uawsom_get_pthermal(ps(igrid)%w,ps(igrid)%x,ixI^L,ixI^L,ps(igrid)%wextra(ixI^S,Tcoff_))
  ...
  subroutine get_Btracing_dir(ipel,igridl,forwardl)
    ...
    do j=1,ngrid_ground
      ...
      ^D&ixmin(^D)=floor((xbmin^D-xprobmin^D)/(uawsom_trac_finegrid*dL))+1;
      ^D&ixmax(^D)=floor((xbmax^D-xprobmin^D)/(uawsom_trac_finegrid*dL));
      ^D&numL(^D)=floor((xprobmax^D-xprobmin^D)/(uawsom_trac_finegrid*dL));
      ixmin(^ND)=1
      ixmax(^ND)=1
      numL(^ND)=1
