...
contains
  ...
  subroutine read_par_files()
    ...
    integer          :: nx_vec(2), block_nx_vec(2)
    integer          :: my_unit, iostate
    integer          :: ilev
    double precision :: dx_vec(2)
    ...
    character(len=std_len), dimension(:), allocatable :: typeboundary_min1,&
       typeboundary_min2, typeboundary_max1,typeboundary_max2
    ...
    double precision :: sizeuniformpart1,sizeuniformpart2
    ...
    namelist /savelist/ tsave,itsave,dtsave,ditsave,nslices,slicedir,&
        ...
        tsave_dat, tsave_slice, tsave_collapsed, tsave_custom, dtsave_log,&
    ...
    namelist /methodlist/ time_stepper,time_integrator, source_split_usr,&
       typesourcesplit,dimsplit,typedimsplit,flux_scheme,limiter,&
       gradient_limiter,cada3_radius,loglimit,typeboundspeed, H_correction,&
       typetvd,typeentropy,entropycoef,typeaverage, typegrad,typediv,typecurl,&
       nxdiffusehllc, flathllc, tvdlfeps,flatcd,flatsh,rk2_alfa,imex222_lambda,&
       ssprk_order,rk3_switch,imex_switch,small_temperature,small_pressure,&
       small_density, small_values_method, small_values_daverage,&
        fix_small_values, check_small_values, trace_small_values, angmomfix,&
        small_values_fix_iw, schmid_rad1,schmid_rad2

    namelist /boundlist/ nghostcells,ghost_copy,internalboundary,&
        typeboundary_min1,typeboundary_min2,typeboundary_max1,&
       typeboundary_max2, save_physical_boundary

    namelist /meshlist/ refine_max_level,nbufferx1,nbufferx2,refine_threshold,&
       derefine_ratio, refine_criterion, stretch_dim, stretch_uncentered,&
        qstretch_baselevel, nstretchedblocks_baselevel, amr_wavefilter,&
       max_blocks,block_nx1,block_nx2,domain_nx1,domain_nx2,iprob,xprobmin1,&
       xprobmin2,xprobmax1,xprobmax2, w_refine_weight, prolongprimitive,&
       coarsenprimitive, typeprolonglimit, logflag,tfixgrid,itfixgrid,&
       ditregrid
    ...
    namelist /emissionlist/ filename_euv,wavelength,resolution_euv,&
       filename_sxr,emin_sxr,emax_sxr,resolution_sxr,LOS_theta,LOS_phi,&
       image_rotate,x_origin,big_image,spectrum_wl,location_slit,&
    ...
    allocate(dg1(nlevelshi))
    allocate(dg2(nlevelshi))
    allocate(ng1(nlevelshi))
    allocate(ng2(nlevelshi))
...
108    close(unitpar)
       ...
       if (base_filename /= basename_prev) basename_full = trim(basename_full) &
          // trim(base_filename)
...
'Output type | tsavestart |  dtsave   | ditsave | itsave(1) | tsave(1)'
    ...
    do ifile = 1, nfile
       if (mype == 0) write(unitterm, fmt_string) trim(output_names(ifile)),&
           tsavestart(ifile), dtsave(ifile), ditsave(ifile), itsave(1, ifile),&
           tsave(1, ifile)
    end do
    ...
    do islice=1,nslices
       if(slicedir(islice) > ndim) write(uniterr,&
          *)'Warning in read_par_files: ', 'Slice ', islice,' direction',&
          slicedir(islice),'larger than ndim=',ndim
       if(slicedir(islice) < 1) write(uniterr,*)'Warning in read_par_files: ',&
    ...
    case ("onestep")
       ...
       case default
          write(unitterm,*) "time_integrator=",time_integrator,"time_stepper=",&
             time_stepper
    ...
    case ("twostep")
       ...
       use_imex_scheme=(t_integrator==IMEX_Midpoint.or.t_integrator==&
          IMEX_Trapezoidal.or.t_integrator==IMEX_222)
    ...
    case ("threestep")
       ...
       if(t_integrator==IMEX_CB3a) then
          ...
          imex_b3   = (6.0d0*imex_c2**2 - 3.0d0*imex_c2 + 1.0d0) / &
             (6.0d0*imex_c2**2)
          imex_a33  = (1.0d0/6.0d0 - imex_b2*imex_c2**2 - &
             imex_b3*imex_c2*imex_c3) / (imex_b3*(imex_c3-imex_c2))
       ...
       use_imex_scheme=(t_integrator==IMEX_ARS3.or.t_integrator==&
          IMEX_232.or.t_integrator==IMEX_CB3a)
    case ("fourstep")
       t_stepper=fourstep
       nstep=4
       if (time_integrator=='default') then
       ...
       if(t_integrator==ssprk5) then
         select case(ssprk_order)
           ...
           case(2) ! From Spireti-Ruuth
            ...
            rk_c4=rk_alfa33*rk_c3+rk_beta33
            rk_c5=rk_alfa44*rk_c4+rk_beta44
            rk_beta55=1.0d0-rk_beta54-rk_alfa53*rk_c3-rk_alfa54*rk_c4-&
               rk_alfa55*rk_c5
           case default
                call mpistop("Unknown ssprk_order")
         end select
    ...
    if (any(limiter(1:nlevelshi)== 'ppm').and.(flatsh.and.physics_type==&
       'rho')) then
       call mpistop&
    ...
    do iw=1,nwfluxbc
      ...
      case("special")
        typeboundary(iw,2*1-1)=bc_special
      case("cont")
        typeboundary(iw,2*1-1)=bc_cont
      case("symm")
        typeboundary(iw,2*1-1)=bc_symm
      case("asymm")
        typeboundary(iw,2*1-1)=bc_asymm
      case("periodic")
        typeboundary(iw,2*1-1)=bc_periodic
      case("aperiodic")
        typeboundary(iw,2*1-1)=bc_aperiodic
      case("noinflow")
        typeboundary(iw,2*1-1)=bc_noinflow
      case("pole")
        typeboundary(iw,2*1-1)=12
      case("bc_data")
        typeboundary(iw,2*1-1)=bc_data
      case("character")
        typeboundary(iw,2*1-1)=bc_character
      case default
         write (unitterm,*) "Undefined boundarytype found in read_par_files",&
             typeboundary_min1(iw),"for variable iw=",iw," and side iB=",2*1-1
      end select
    end do
    do iw=1,nwfluxbc
      select case(typeboundary_max1(iw))
      case("special")
        typeboundary(iw,2*1)=bc_special
      case("cont")
        typeboundary(iw,2*1)=bc_cont
      case("symm")
        typeboundary(iw,2*1)=bc_symm
      case("asymm")
        typeboundary(iw,2*1)=bc_asymm
      case("periodic")
        typeboundary(iw,2*1)=bc_periodic
      case("aperiodic")
        typeboundary(iw,2*1)=bc_aperiodic
      case("noinflow")
        typeboundary(iw,2*1)=bc_noinflow
      case("pole")
        typeboundary(iw,2*1)=12
      case("bc_data")
        typeboundary(iw,2*1)=bc_data
      case("bc_character")
        typeboundary(iw,2*1)=bc_character
      case default
         write (unitterm,*) "Undefined boundarytype found in read_par_files",&
    ...
    do iw=1,nwfluxbc
      ...
      case("special")
        typeboundary(iw,2*2-1)=bc_special
      case("cont")
        typeboundary(iw,2*2-1)=bc_cont
      case("symm")
        typeboundary(iw,2*2-1)=bc_symm
      case("asymm")
        typeboundary(iw,2*2-1)=bc_asymm
      case("periodic")
        typeboundary(iw,2*2-1)=bc_periodic
      case("aperiodic")
        typeboundary(iw,2*2-1)=bc_aperiodic
      case("noinflow")
        typeboundary(iw,2*2-1)=bc_noinflow
      case("pole")
        typeboundary(iw,2*2-1)=12
      case("bc_data")
        typeboundary(iw,2*2-1)=bc_data
      case("character")
        typeboundary(iw,2*2-1)=bc_character
      case default
         write (unitterm,*) "Undefined boundarytype found in read_par_files",&
             typeboundary_min2(iw),"for variable iw=",iw," and side iB=",2*2-1
      end select
    end do
    do iw=1,nwfluxbc
      select case(typeboundary_max2(iw))
      case("special")
        typeboundary(iw,2*2)=bc_special
      case("cont")
        typeboundary(iw,2*2)=bc_cont
      case("symm")
        typeboundary(iw,2*2)=bc_symm
      case("asymm")
        typeboundary(iw,2*2)=bc_asymm
      case("periodic")
        typeboundary(iw,2*2)=bc_periodic
      case("aperiodic")
        typeboundary(iw,2*2)=bc_aperiodic
      case("noinflow")
        typeboundary(iw,2*2)=bc_noinflow
      case("pole")
        typeboundary(iw,2*2)=12
      case("bc_data")
        typeboundary(iw,2*2)=bc_data
      case("bc_character")
        typeboundary(iw,2*2)=bc_character
      case default
         write (unitterm,*) "Undefined boundarytype found in read_par_files",&
    ...
    do idim=1,ndim
       ...
       if (periodB(idim).or.aperiodB(idim)) then
          do iw=1,nwflux
             if (typeboundary(iw,2*idim-1) .ne. typeboundary(iw,&
                2*idim)) call mpistop("Wrong counterpart in periodic boundary")
    ...
    nx_vec = [domain_nx1, domain_nx2]
    block_nx_vec = [block_nx1, block_nx2]

    if (any(nx_vec < 4) .or. any(mod(nx_vec,&
        2) == 1)) call mpistop&
       ('Grid size (domain_nx^D) has to be even and >= 4')

    if (any(block_nx_vec < 4) .or. any(mod(block_nx_vec,&
        2) == 1)) call mpistop&
       ('Block size (block_nx^D) has to be even and >= 4')

     if(mod(domain_nx1,block_nx1)/=0) call &
        mpistop('Grid (domain_nx^D) and block (block_nx^D) must be consistent')
...
'will try default value for qstretch_baselevel in dimension', 2
    ...
    dx_vec = [xprobmax1-xprobmin1, xprobmax2-xprobmin2] / nx_vec

    if (mype==0) then
       write(c_ndim, '(I1)') 2
       write(unitterm, '(A30,' // c_ndim // '(I0," "))') &
          ' Domain size (cells): ', nx_vec
       write(unitterm, '(A30,' // c_ndim // '(E9.3," "))') ' Level one dx: ',&
           dx_vec
    end if

    if (any(dx_vec < smalldouble)) call mpistop(&
    ...
    if (itfixgrid<biginteger/2) then
       if(mype==0)print*,'Warning, at iteration=',itfixgrid,&
          'the grid will be fixed'
    end if
    if (ditregrid>1) then
       if(mype==0)print*,'Note, Grid is reconstructed once every',ditregrid,&
          'iterations'
    end if
    ...
    if (mype==0) then
       write(unitterm, '(A30,A,A)') 'restart_from_file: ', ' ',&
  ...
  subroutine get_fields_string(line, delims, n_max, fields, n_found,&
      fully_read)
  ...
  subroutine snapshot_write_header1(fh, offset_tree, offset_block,&
    ...
    call MPI_FILE_WRITE(fh, [ xprobmin1,xprobmin2 ], ndim,&
        MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, [ xprobmax1,xprobmax2 ], ndim,&
        MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, [ domain_nx1,domain_nx2 ], ndim, MPI_INTEGER, st,&
        er)
    call MPI_FILE_WRITE(fh, [ block_nx1,block_nx2 ], ndim, MPI_INTEGER, st,&
        er)
  ...
  subroutine snapshot_read_header(fh, offset_tree, offset_block)
    ...
    if (maxval(abs(rbuf(1:ndim) - [ xprobmin1,xprobmin2 ])) > 0) then
    ...
    ! xprobmax^D
    call MPI_FILE_READ(fh,rbuf(1:ndim),ndim,MPI_DOUBLE_PRECISION,st,er)
    if (maxval(abs(rbuf(1:ndim) - [ xprobmax1,xprobmax2 ])) > 0) then
    ...
    if (any(ibuf(1:ndim) /= [ domain_nx1,domain_nx2 ])) then
    ...
    ! block_nx^D
    call MPI_FILE_READ(fh,ibuf(1:ndim), ndim, MPI_INTEGER,st,er)
    if (any(ibuf(1:ndim) /= [ block_nx1,block_nx2 ])) then
    ...
    if (version > 4) then
      ...
      if (periodic(1) .and. .not.periodB(1) .or. .not.periodic(1) .and. &
         ...
         .not.periodic(2) .and. periodB(2)) call &
      ...
      if (stagger_grid .and. .not. stagger_mark_dat .or. &
        ...
        write(*,*) "Error: stagger grid flag differs from restart data:",&
            stagger_mark_dat
...
!        call mpistop("Cannot restart with a different physics type")
  ...
  pure integer function count_ix(ixOmin1,ixOmin2,ixOmax1,ixOmax2)
    integer, intent(in) :: ixOmin1,ixOmin2,ixOmax1,ixOmax2

    count_ix = product([ ixOmax1,ixOmax2 ] - [ ixOmin1,ixOmin2 ] + 1)
  end function count_ix

  !> Determine the shape of a block for output (whether to include ghost cells,
  !> and on which sides)
  subroutine block_shape_io(igrid, n_ghost, ixOmin1,ixOmin2,ixOmax1,ixOmax2,&
      n_values)
    use mod_global_parameters
    ...
    n_values = count_ix(ixOmin1,ixOmin2,ixOmax1,ixOmax2) * nw
  ...
  subroutine write_snapshot
    ...
    integer                       :: ixOmin1,ixOmin2,ixOmax1,ixOmax2,&
        n_ghost(2*ndim)
    integer                       :: ixOsmin1,ixOsmin2,ixOsmax1,ixOsmax2,&
       n_values_stagger
    ...
    n_values = count_ix(ixGlo1,ixGlo2,ixGhi1,ixGhi2) * nw
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGslo2,ixGshi1,ixGshi2) * nws
    end if
    ...
    if (mype==0) then
      ...
      do Morton_no = Morton_start(0), Morton_stop(npe-1)
         ...
         block_ig(:, Morton_no) = [ pnode%ig1,pnode%ig2 ]
         block_lvl(Morton_no) = pnode%level
         block_offset(Morton_no) = 0 ! Will be determined later
      end do

      call MPI_FILE_WRITE(file_handle, block_lvl, size(block_lvl), MPI_INTEGER,&
          istatus, ierrmpi)
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      call block_shape_io(igrid, n_ghost, ixOmin1,ixOmin2,ixOmax1,ixOmax2,&
          n_values)
      if(stagger_grid) then
        w_buffer(1:n_values) = pack(ps(igrid)%w(ixOmin1:ixOmax1,&
           ixOmin2:ixOmax2, 1:nw), .true.)
        ixOsmin1 = ixOmin1 -1
        ixOsmin2 = ixOmin2 -1
        ixOsmax1 = ixOmax1 
        ixOsmax2 = ixOmax2 
        n_values_stagger= count_ix(ixOsmin1,ixOsmin2,ixOsmax1,ixOsmax2)*nws
        w_buffer(n_values+1:n_values+n_values_stagger) = &
           pack(ps(igrid)%ws(ixOsmin1:ixOsmax1,ixOsmin2:ixOsmax2, 1:nws),&
            .true.)
  ...
  subroutine read_snapshot
    ...
    integer                       :: ix_buffer(2*ndim+1), n_values,&
        n_values_stagger
    integer                       :: ixOmin1,ixOmin2,ixOmax1,ixOmax2, ixOsmin1,&
       ixOsmin2,ixOsmax1,ixOsmax2
    integer                       :: file_handle, amode, igrid, Morton_no,&
        iread
    integer                       :: istatus(MPI_STATUS_SIZE)
    integer                       :: iorecvstatus(MPI_STATUS_SIZE)
    ...
    double precision :: ws(ixGslo1:ixGshi1,ixGslo2:ixGshi2,1:ndim)
    ...
    ! Allocate send/receive buffer
    n_values = count_ix(ixGlo1,ixGlo2,ixGhi1,ixGhi2) * nw_found
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGslo2,ixGshi1,ixGshi2) * nws
    end if
    allocate(w_buffer(n_values))
    allocate(w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw_found))
    ...
    if (mype==0) then
      call MPI_FILE_SEEK(file_handle, offset_block_data, MPI_SEEK_SET,&
          ierrmpi)
      ...
      do ipe = 0, npe-1
        do Morton_no=Morton_start(ipe),Morton_stop(ipe)
          ...
          ixOmin1 = ixMlo1 - ix_buffer(1)
          ixOmin2 = ixMlo2 - ix_buffer(2)
          ixOmax1 = ixMhi1 + ix_buffer(ndim+1)
          ixOmax2 = ixMhi2 + ix_buffer(ndim+2)
          n_values = count_ix(ixOmin1,ixOmin2,ixOmax1,ixOmax2) * nw_found
          if(stagger_grid) then
            ixOsmin1 = ixOmin1 - 1
            ixOsmin2 = ixOmin2 - 1
            ixOsmax1 = ixOmax1
            ...
            n_values = n_values + count_ix(ixOsmin1,ixOsmin2,ixOsmax1,&
               ixOsmax2) * nws
          end if
          ...
          if (mype == ipe) then ! Root task
            ...
            if(stagger_grid) then
              w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
                  1:nw_found) = reshape(w_buffer(1:n_values_stagger),&
                  shape(w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, 1:nw_found)))
              ps(igrid)%ws(ixOsmin1:ixOsmax1,ixOsmin2:ixOsmax2,&
                 1:nws)=reshape(w_buffer(n_values_stagger+1:n_values),&
                  shape(ws(ixOsmin1:ixOsmax1,ixOsmin2:ixOsmax2, 1:nws)))
            else
              w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
                  1:nw_found) = reshape(w_buffer(1:n_values),&
                  shape(w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, 1:nw_found)))
            end if
            if (nw_found<nw) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
                   ixMhi1,ixMhi2,nw_found,w,ps(igrid)%x,ps(igrid)%w)
              else
                ps(igrid)%w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
                   1:nw_found)=w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nw_found)
              end if
              ...
              if (associated(usr_transform_w)) then
                ...
                ps(igrid)%w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
                   1:nw)=w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nw)
              end if
            else
              ps(igrid)%w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
                 1:nw)=w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nw)
            ...
            call MPI_SEND([ ixOmin1,ixOmin2,ixOmax1,ixOmax2, n_values ],&
                2*ndim+1, MPI_INTEGER, ipe, itag, icomm, ierrmpi)
      ...
      do Morton_no=Morton_start(mype),Morton_stop(mype)
        ...
        ixOmin1 = ix_buffer(1)
        ixOmin2 = ix_buffer(2)
        ixOmax1 = ix_buffer(ndim+1)
        ixOmax2 = ix_buffer(ndim+2)
        ...
        if(stagger_grid) then
          n_values_stagger = count_ix(ixOmin1,ixOmin2,ixOmax1,&
             ixOmax2) * nw_found
          ixOsmin1 = ixOmin1 - 1
          ixOsmin2 = ixOmin2 - 1
          ixOsmax1 = ixOmax1
        ...
        if (nw_found<nw) then
          ...
          ps(igrid)%w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nw)=w(ixOmin1:ixOmax1,&
  ...
  subroutine read_snapshot_old()
    ...
    double precision              :: wio(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)
    integer                       :: fh, igrid, Morton_no, iread
    integer                       :: levmaxini, ndimini, ndirini
    integer                       :: nwini, neqparini, nxini1,nxini2
    ...
    if (mype==0) then
      ...
      offset=offset-int(ndimini*size_int+neqparini*size_double,&
      ...
      if (ixGhi1/=nxini1+2*nghostcells.or.ixGhi2/=nxini2+2*nghostcells) then
        write(*,*) "Error: reset resolution to ",nxini1+2*nghostcells,&
           nxini2+2*nghostcells
    ...
    if (npe>1) then
      ...
      call MPI_BCAST(sendini,7+2,MPI_INTEGER,0,icomm,ierrmpi)
      nleafs=sendini(1);levmaxini=sendini(2);ndimini=sendini(3);
      ndirini=sendini(4);nwini=sendini(5);
      neqparini=sendini(6);it=sendini(7);
      nxini1=sendini(7+1);nxini2=sendini(7+2);
      nleafs_active = nleafs
      call MPI_BCAST(global_time,1,MPI_DOUBLE_PRECISION,0,icomm,ierrmpi)
    end if

    if (mype == 0) then
      offset = int(size_block_io,kind=MPI_OFFSET_KIND) * int(nleafs,&
         kind=MPI_OFFSET_KIND)
    ...
    do iigrid = 1, igridstail
       ...
       do iw = 1, nw
          wsum(iw) = wsum(iw) + sum(ps(igrid)%dvolume(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2)*ps(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
             iw)**power)
       end do
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wsum, dsum_recv, nw+1, MPI_DOUBLE_PRECISION, MPI_SUM,&
        icomm, ierrmpi)
  ...
  subroutine get_volume_coverage(vol_cov)
    ...
    do iigrid = 1, igridstail
       ...
       vol_cov(level) = vol_cov(level)+ (rnode(rpxmax1_,igrid)-rnode(rpxmin1_,&
          igrid))*(rnode(rpxmax2_,igrid)-rnode(rpxmin2_,igrid))
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(vol_cov, dsum_recv, refine_max_level,&
  ...
  subroutine get_volume_average_func(func, f_avg, volume)
    ...
    do iigrid = 1, igridstail
       ...
       ! Store total volume in last element
       wsum(2) = wsum(2) + sum(ps(igrid)%dvolume(ixMlo1:ixMhi1,ixMlo2:ixMhi2))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do i1 = ixMlo1, ixMhi1
       do i2 = ixMlo2, ixMhi2
       wsum(1) = wsum(1) + ps(igrid)%dvolume(i1,i2) * func(ps(igrid)%w(i1,i2,&
           :), nw)
       end do
       end do
    end do
    ...
    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wsum, dsum_recv, 2, MPI_DOUBLE_PRECISION, MPI_SUM,&
        icomm, ierrmpi)
  ...
  subroutine get_global_maxima(wmax)
    ...
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nw
          wmax_mype(iw)=max(wmax_mype(iw),maxval(ps(igrid)%w(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2,iw)))
       end do
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wmax_mype, wmax_recv, nw, MPI_DOUBLE_PRECISION, MPI_MAX,&
        icomm, ierrmpi)
  ...
  subroutine get_global_minima(wmin)
    ...
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nw
          wmin_mype(iw)=min(wmin_mype(iw),minval(ps(igrid)%w(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2,iw)))
       end do
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wmin_mype, wmin_recv, nw, MPI_DOUBLE_PRECISION, MPI_MIN,&
        icomm, ierrmpi)
