...
contains
  ...
  subroutine initialize_vars
    ...
    allocate(neighbor_type(-1:1,-1:1,max_blocks),neighbor_active(-1:1,-1:1,&
       max_blocks))
    allocate(neighbor_pole(-1:1,-1:1,max_blocks))
    ...
    if(stagger_grid) then
      allocate(pface(2,2,max_blocks),fine_neighbors(2,2,2,max_blocks))
      allocate(old_neighbor(2,-1:11,-1:12,max_blocks))
    end if
    ...
    if((ng1(1)/=1.or.ng2(1)/=1).and.refine_max_level>1) then
