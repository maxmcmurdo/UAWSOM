...
contains
  ...
  subroutine level1_Morton_order
    ...
    integer :: ig1,ig2, ngsq1,ngsq2, isq, total_number
    ...
    do ig1=1,ngsq1
       ...
       seq_sfc(gsq_sfc(ig1,ig2))=gsq_sfc(ig1,ig2)
       seq_ig1(gsq_sfc(ig1,ig2))=ig1 
       seq_ig2(gsq_sfc(ig1,ig2))=ig2 
    end do
    end do
    ! mark blocks that are out of the domain and change Morton number
    do isq=1,total_number
       if (seq_ig1(isq)>ng1(1).or.seq_ig2(isq)>ng2(1)) then
    ...
    do isq=1,total_number
      if(in_domain(isq)) then
        iglevel1_sfc(seq_ig1(isq),seq_ig2(isq))=seq_sfc(isq)
        sfc_iglevel1(1,seq_sfc(isq))=seq_ig1(isq) 
        sfc_iglevel1(2,seq_sfc(isq))=seq_ig2(isq) 
      end if
    end do

    deallocate(gsq_sfc,seq_sfc,seq_ig1,seq_ig2,in_domain)
  ...
  integer(kind=8) function mortonEncode(ig1,ig2,ndim)
    ...
    implicit none
    integer(kind=4), intent(in) :: ig1,ig2,ndim
    integer(kind=4) :: i
    integer(kind=8) :: answer, lg1,lg2
