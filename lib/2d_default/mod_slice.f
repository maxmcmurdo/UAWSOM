...
contains
      ...
      case('vtuCC') ! celldata
         ...
         do iw=1,nw+nwauxio
            ...
            write(slice_fh,'(200(1pe14.6))') &
               (roundoff_minmax(ps_sub(jgrid)%w(ix1,iw)*normconv(iw),minvalue,&
      ...
      case('vtu') ! pointdata
         ...
         do iw=1,nw+nwauxio
            ...
            write(slice_fh,'(a,a,a)')'<DataArray type="Float64" Name="',&
               TRIM(wnamei(iw)),'" format="ascii">'
            write(slice_fh,'(200(1pe14.6))') &
               (roundoff_minmax(ps_sub(jgrid)%wC(ix1,iw)*normconv(iw),minvalue,&
 ...
 !write cell corner coordinates in a backward dimensional loop, always 3D output
      do ix1=ixCmin1,ixCmax1 
            x_VTK(1:3)=zero;
            x_VTK(1:ndim)=ps_sub(jgrid)%xC(ix1,1:ndim)*normconv(0);
      ...
      double precision, parameter :: minvalue = 1.0d-99, maxvalue = 1.0d+99
            ...
            do ix1=ixsubMlo(1),ixsubMhi(1)
               ...
               do idir=1,ndim
                  ...
                  write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%x(ix1,&
               ...
               do iw = 1,nw+nwauxio-1
                  ...
                  write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%w(ix1,&
...
!         call MPI_FILE_WRITE(slice_fh,neqpar+nspecialpar,1,MPI_INTEGER,status,ierrmpi)
  ...
  subroutine fill_subnode(igrid,active,jgrid,dir,xslice,normconv)
    ...
    select case (dir)
    case (1)
       ixslice = minloc(dabs(xslice-ps(igrid)%x(:,ixMlo2,dir)),1,mask(:,&
          ixMlo2))
    case (2)
    ...
    select case (dir)
    case (1)
       ps_sub(jgrid)%w(ixCCmin2:ixCCmax2,1:nw+nwexpand) = wCC_TMP(ixslice,&
          ixCCmin2:ixCCmax2,1:nw+nwexpand)
       ps_sub(jgrid)%x(ixCCmin2:ixCCmax2,1:ndim) = xCC_TMP(ixslice,&
          ixCCmin2:ixCCmax2,1:ndim)
       ps_sub(jgrid)%wC(ixCmin2:ixCmax2,1:nw+nwexpand) = wC_TMP(ixslice,&
          ixCmin2:ixCmax2,1:nw+nwexpand)
       ps_sub(jgrid)%xC(ixCmin2:ixCmax2,1:ndim) = xC_TMP(ixslice,&
          ixCmin2:ixCmax2,1:ndim)
    case (2)
       ps_sub(jgrid)%w(ixCCmin1:ixCCmax1,&
          1:nw+nwexpand) = wCC_TMP(ixCCmin1:ixCCmax1,ixslice,1:nw+nwexpand)
       ps_sub(jgrid)%x(ixCCmin1:ixCCmax1,1:ndim) = xCC_TMP(ixCCmin1:ixCCmax1,&
          ixslice,1:ndim)
       ps_sub(jgrid)%wC(ixCmin1:ixCmax1,1:nw+nwexpand) = &
          wC_TMP(ixCmin1:ixCmax1,ixslice,1:nw+nwexpand)
       ps_sub(jgrid)%xC(ixCmin1:ixCmax1,1:ndim) = xC_TMP(ixCmin1:ixCmax1,&
          ixslice,1:ndim)
    case default
       call mpistop("slice direction not clear in fill_subnode")
    end select
