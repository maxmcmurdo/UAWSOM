module mod_convert
  ...
  abstract interface
     ...
     function sub_convert_vars(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
...
contains
  ...
  ! shortcut
  subroutine add_convert_method2(phys_convert_vars, nwc, aux_variable_names,&
      file_suffix)
    use mod_global_parameters
    integer, intent(in) :: nwc
    character(len=*),intent(in):: aux_variable_names
    ...
    interface
     function phys_convert_vars(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
        ixOmin2,ixOmax1,ixOmax2, w, x, nwc) result(wnew)
       use mod_global_parameters
       integer, intent(in)             :: ixImin1,ixImin2,ixImax1,ixImax2,&
           ixOmin1,ixOmin2,ixOmax1,ixOmax2, nwc
    ...
    call add_convert_method(phys_convert_vars, nwc,&
        get_names_from_string(aux_variable_names,nwc), file_suffix)

  end subroutine add_convert_method2

  subroutine add_convert_method(phys_convert_vars, nwc, dataset_names,&
      file_suffix)
    integer, intent(in) :: nwc
  ...
  subroutine convert_all()
    ...
    do while(associated(temp))
      call convert_dat_generic(temp%nwc, temp%dataset_names, temp%file_suffix,&
          temp%phys_convert_vars)
      temp=>temp%next
    end do
  ...
  subroutine convert_dat_generic(nwc, dataset_names, file_suffix,&
    ...
    use mod_forest
    use mod_global_parameters
    use mod_input_output, only: count_ix, create_output_file,&
        snapshot_write_header1, block_shape_io

    integer                       :: file_handle, igrid, Morton_no, iwrite
    integer                       :: ipe, ix_buffer(2*ndim+1), n_values
    integer                       :: ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
       ixOmin2,ixOmax1,ixOmax2, n_ghost(2*ndim)
    ...
    double precision, allocatable:: converted_vars(:,:,:)
    ...
    interface
      ...
      function convert_vars(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
         ixOmax1,ixOmax2,w,x,nwc) result(wres)
        use mod_global_parameters
        ...
        double precision, intent(in) :: w(ixImin1:ixImax1,ixImin2:ixImax2,&
           1:nw)
        double precision  :: wres(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nwc)
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      if(save_physical_boundary) then
        do idim=1,ndim
          ...
          if(ps(igrid)%is_physical_boundary(2*idim-1)) &
             n_ghost(idim)=nghostcells
      ...
      n_values = count_ix(ixOmin1,ixOmin2,ixOmax1,ixOmax2) * nwc
      ...
      w_buffer(1:n_values) = pack(convert_vars(ixImin1,ixImin2,ixImax1,ixImax2,&
          ixOmin1,ixOmin2,ixOmax1,ixOmax2,ps(igrid)%w(ixImin1:ixImax1,&
         ixImin2:ixImax2, 1:nw), ps(igrid)%x(ixImin1:ixImax1,ixImin2:ixImax2,&
          1:ndim),nwc), .true.)
