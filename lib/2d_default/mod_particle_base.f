...
module mod_particle_base
  ...
  procedure(sub_fill_gridvars), pointer              :: &
     particles_fill_gridvars => null()
  procedure(sub_define_additional_gridvars),&
      pointer :: particles_define_additional_gridvars => null()
  procedure(sub_fill_additional_gridvars),&
      pointer   :: particles_fill_additional_gridvars => null()
...
contains
  ...
  subroutine particles_finish()
    ...
    call destroy_particles(nparticles_on_mype,&
       particles_on_mype(1:nparticles_on_mype))
  ...
  subroutine particles_params_read(files)
    ...
    namelist /particles_list/ physics_type_particles, nparticleshi,&
        ...
        write_snapshot, downsample_particles, dtsave_particles, tmax_particles,&
        num_particles, ndefpayload, nusrpayload, losses, const_dt_particles,&
        particles_cfl, dtheta, relativistic, integrator_type_particles,&
...
111   close(unitpar)
  ...
  subroutine get_uniform_pos(x)
    ...
    x(1) = xprobmin1 + x(1) * (xprobmax1 - xprobmin1)
    x(2) = xprobmin2 + x(2) * (xprobmax2 - xprobmin2)
    x(ndim+1:) = 0.0d0
  ...
  subroutine init_gridvars()
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      allocate(gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ngridvars))
      gridvars(igrid)%w = 0.0d0

      if (time_advance) then
        allocate(gridvars(igrid)%wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
           1:ngridvars))
  ...
  subroutine fill_gridvars_default
    ...
    integer :: igrid, iigrid
    double precision :: E(ixGlo1:ixGhi1,ixGlo2:ixGhi2, ndir)
    double precision :: B(ixGlo1:ixGhi1,ixGlo2:ixGhi2, ndir)

    do iigrid=1,igridstail; igrid=igrids(iigrid);
      if (associated(usr_particle_fields)) then
        call usr_particle_fields(ps(igrid)%w, ps(igrid)%x, E, B)
        gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(:)) = E
        gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(:)) = B
      else
      ...
      if (time_advance) then
        if (associated(usr_particle_fields)) then
          ...
          gridvars(igrid)%wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(:)) = E
          gridvars(igrid)%wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(:)) = B
        else
  ...
  subroutine fields_from_mhd(igrid, w_mhd, w_part)
    ...
    double precision, intent(in)    :: w_mhd(ixGlo1:ixGhi1,ixGlo2:ixGhi2,nw)
    double precision, intent(inout) :: w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       ngridvars)
    ...
    w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw) = w_mhd(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       1:nw)
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(1):bp(3)) = 0.0d0
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(1):ep(3)) = 0.0d0
    ...
    if (B0field) then
      do idir = 1, ndir
        w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(idir))=w(ixGlo1:ixGhi1,&
           ixGlo2:ixGhi2,iw_mag(idir))+ps(igrid)%B0(ixGlo1:ixGhi1,&
           ixGlo2:ixGhi2,idir,0)
      end do
    else
      w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(:)) = w(ixGlo1:ixGhi1,&
         ixGlo2:ixGhi2,iw_mag(:))
    end if
!    w_part(ixG^T,bp(:)) = w(ixG^T,iw_mag(:))

    ! fill with current
    current = zero
    call particle_get_current(w,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1+1,ixGlo2+1,&
       ixGhi1-1,ixGhi2-1,idirmin,current)
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,jp(:)) = current(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,:)

    ! fill with electric field
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(1)) = w_part(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,bp(2)) * w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       iw_mom(3)) - w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       bp(3)) * w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       iw_mom(2)) + particles_eta * current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1)
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(2)) = w_part(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,bp(3)) * w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       iw_mom(1)) - w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       bp(1)) * w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       iw_mom(3)) + particles_eta * current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,2)
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(3)) = w_part(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,bp(1)) * w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       iw_mom(2)) - w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       bp(2)) * w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       iw_mom(1)) + particles_eta * current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,3)

    ! Hall term
    if (particles_etah > zero) then
      w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(1)) = w_part(ixGlo1:ixGhi1,&
         ixGlo2:ixGhi2,ep(1)) + particles_etah/w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_rho) * (current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         2) * w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         bp(3)) - current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         3) * w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(2)))
      w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(2)) = w_part(ixGlo1:ixGhi1,&
         ixGlo2:ixGhi2,ep(2)) + particles_etah/w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_rho) * (-current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         1) * w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         bp(3)) + current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         3) * w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(1)))
      w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(3)) = w_part(ixGlo1:ixGhi1,&
         ixGlo2:ixGhi2,ep(3)) + particles_etah/w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_rho) * (current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         1) * w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         bp(2)) - current(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         2) * w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(1)))
    end if
  ...
  subroutine particle_get_current(w,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2,idirmin,current)
    use mod_global_parameters
    use mod_geometry
    ...
    if (B0field) then
      do idir = 1, ndir
        bvec(ixImin1:ixImax1,ixImin2:ixImax2,idir)=w(ixImin1:ixImax1,&
           ixImin2:ixImax2,iw_mag(idir))+block%B0(ixImin1:ixImax1,&
           ixImin2:ixImax2,idir,0)
      end do
    else
      do idir = 1, ndir
        bvec(ixImin1:ixImax1,ixImin2:ixImax2,idir)=w(ixImin1:ixImax1,&
           ixImin2:ixImax2,iw_mag(idir))
      end do
    end if
  ...
  subroutine handle_particles()
    ...
    do
      if (tmax_particles >= t_next_output) then
        ...
        ! at the same time
        t_next_output = max(t_next_output,&
            min_particle_time) + dtsave_particles
      else
...
!        call write_particle_output()
      ...
      if (nparticles_left == 0 .and. convert) call &
         mpistop('No particles left')
  ...
  subroutine get_vec(ix,igrid,x,tloc,vec)
    ...
    double precision                                   :: td
    integer                                            :: ic1,ic2,idir

    if (associated(usr_particle_analytic)) then
      call usr_particle_analytic(ix, x, tloc, vec)
    else if (.not.time_advance) then
      do idir=1,ndir
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
           ix(idir)), ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,&
           vec(idir))
      end do
    else
      do idir=1,ndir
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,gridvars(igrid)%wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
           ix(idir)), ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,&
           vec1(idir))
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
  ...
  subroutine interpolate_var(igrid,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2,gf,x,xloc,gfloc)
    use mod_global_parameters

    integer, intent(in)                   :: igrid,ixImin1,ixImin2,ixImax1,&
    ...
    integer                               :: ic1,ic2, ic11,ic12, ic21,ic22,&
        idir
  ...
  subroutine time_spent_on_particles()
    ...
    call MPI_REDUCE(tpartc,tpartc_avg,1,MPI_DOUBLE_PRECISION,MPI_SUM,0,icomm,&
       ierrmpi)
    call MPI_REDUCE(tpartc_int,tpartc_int_avg,1,MPI_DOUBLE_PRECISION,MPI_SUM,0,&
       icomm,ierrmpi)
    call MPI_REDUCE(tpartc_io,tpartc_io_avg,1,MPI_DOUBLE_PRECISION,MPI_SUM,0,&
       icomm,ierrmpi)
    call MPI_REDUCE(tpartc_com,tpartc_com_avg,1,MPI_DOUBLE_PRECISION,MPI_SUM,0,&
       icomm,ierrmpi)
    call MPI_REDUCE(tpartc_grid,tpartc_grid_avg,1,MPI_DOUBLE_PRECISION,MPI_SUM,&
       0,icomm,ierrmpi)
...
('npayload in restart file does not match npayload in mod_particles')
    ...
    do while (mynparticles .lt. nparticles)
      if (mype == 0) then
        do while (nparticles_on_mype .lt. nparticles_per_cpu_hi .and. &
           mynparticles .lt. nparticles)
...
!    if (particle(index)%self%follow) print*, 'follow index:', index
  ...
  subroutine write_particles_snapshot()
    ...
    double precision, dimension(npayload,&
       nparticles_per_cpu_hi)  :: send_payload
    double precision, dimension(npayload,&
       nparticles_per_cpu_hi)  :: receive_payload
    ...
    if (mype .eq. 0) then
      ...
      if (.not. file_exists) then
        ...
        open(unit=unitparticles,file=filename,form='unformatted',&
  ...
  subroutine init_particles_output()
    ...
    do iipart=1,nparticles_on_mype;ipart=particles_on_mype(iipart);
      if (particle(ipart)%self%follow) then
        filename = make_particle_filename(particle(ipart)%self%time,&
           particle(ipart)%self%index,'individual')
        ...
        write(unitparticles,"(a,a,a)") 'time,dt,x1,x2,x3,u1,u2,u3,',trim(line),&
           'ipe,iteration,index'
        close(unit=unitparticles)
      end if
    end do
  ...
  subroutine locate_particle(index,igrid_particle,ipe_particle)
    ...
    if (npe>0) call MPI_ALLGATHER(has_particle(mype),1,MPI_LOGICAL,&
       has_particle,1,MPI_LOGICAL,icomm,ierrmpi)
  ...
  subroutine find_particle_ipe(x,igrid_particle,ipe_particle)
    ...
    branch=tree_root(ig(1,1),ig(2,1))
    do while (.not.branch%node%leaf)
      ...
      branch%node => branch%node%child(ic(1),ic(2))%node
    end do
  ...
  logical function particle_in_domain(x)
    ...
    particle_in_domain = all(x >= [ xprobmin1,&
       xprobmin2 ]) .and. all(x < [ xprobmax1,xprobmax2 ]) !Jannis: as before < instead of <= here, necessary?
  ...
  logical function particle_in_igrid(ipart, igrid)
    ...
    if (.not. allocated(ps(igrid)%w)) then
      ...
      grid_rmin         = [ rnode(rpxmin1_,igrid),rnode(rpxmin2_,igrid) ]
      grid_rmax         = [ rnode(rpxmax1_,igrid),rnode(rpxmax2_,igrid) ]
  ...
  subroutine set_neighbor_ipe()
    ...
    integer              :: my_neighbor_type, i1,i2
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      case (neighbor_coarse) ! fine-coarse
        call ipe_fc(i1,i2,igrid,ipe_is_neighbor)
      case (neighbor_sibling) ! same level
        call ipe_srl(i1,i2,igrid,ipe_is_neighbor)
      case (neighbor_fine) ! coarse-fine
        call ipe_cf(i1,i2,igrid,ipe_is_neighbor)
      end select
  ...
  subroutine ipe_fc(i1,i2,igrid,ipe_is_neighbor)
    use mod_global_parameters
    integer, intent(in) :: i1,i2, igrid
    logical, intent(inout) :: ipe_is_neighbor(0:npe-1)

    ipe_is_neighbor(neighbor(2,i1,i2,igrid)) = .true.

  end subroutine ipe_fc

  subroutine ipe_srl(i1,i2,igrid,ipe_is_neighbor)
    use mod_global_parameters
    integer, intent(in) :: i1,i2, igrid
    logical, intent(inout) :: ipe_is_neighbor(0:npe-1)
  ...
  subroutine ipe_cf(i1,i2,igrid,ipe_is_neighbor)
    use mod_global_parameters
    integer, intent(in)    :: i1,i2, igrid
    logical, intent(inout) :: ipe_is_neighbor(0:npe-1)
    integer                :: ic1,ic2, inc1,inc2
    ...
    ipe_is_neighbor( neighbor_child(2,inc1,inc2,igrid) ) = .true.
  ...
  character(len=128) function make_particle_filename(tout,index,type)
    ...
    case ('ensemble')
      ...
      write(make_particle_filename,"(a,a,i6.6,a)") trim(base_filename),&
         '_ensemble',nout,'.csv'
    case ('destroy')
      write(make_particle_filename,"(a,a)") trim(base_filename),&
         '_destroyed.csv'
    case ('individual')
      write(make_particle_filename,"(a,a,i6.6,a)") trim(base_filename),&
         '_particle',index,'.csv'
  ...
  subroutine output_ensemble(send_n_particles_for_output,send_particles,&
     send_payload,typefile)
    use mod_global_parameters

    integer, intent(in)             :: send_n_particles_for_output
    type(particle_t), dimension(send_n_particles_for_output),&
  ...
  subroutine output_particle(myparticle,payload,ipe,file_unit)
    ...
    write(file_unit, csv_format) myparticle%time, myparticle%dt, x(1:3), u,&
        payload(1:npayload), ipe, it_particles, myparticle%index
  ...
  subroutine comm_particles()
    ...
    integer                         :: index
    integer                         :: tag_send, tag_receive, tag_send_p,&
        tag_receive_p, send_buff, rcv_buff
    integer                         :: status(MPI_STATUS_SIZE)
    ...
    integer, dimension(nparticles_per_cpu_hi,&
       0:npe-1)  :: particle_index_to_be_sent_to_ipe
    integer, dimension(nparticles_per_cpu_hi,&
       0:npe-1)  :: particle_index_to_be_received_from_ipe
    integer, dimension(nparticles_per_cpu_hi) :: &
    ...
    do iipart=1,nparticles_on_mype;ipart=particles_on_mype(iipart);
      ...
      if ( .not.time_advance .and. particle(ipart)%self%time .gt. &
         tmax_particles ) then
      ...
      if (.not.particle_in_igrid(ipart,particle(ipart)%igrid)) then
        ...
        if (igrid_particle == -1 )then
          call apply_periodB(particle(ipart)%self,igrid_particle,ipe_particle,&
             BC_applied)
    ...
    allocate( sndrqst(1:npe_neighbors*nparticles_per_cpu_hi),&
        rcvrqst(1:npe_neighbors*nparticles_per_cpu_hi) )
    sndrqst = MPI_REQUEST_NULL; rcvrqst = MPI_REQUEST_NULL;
    isnd = 0; ircv = 0;
    do iipe=1,npe_neighbors;ipe=ipe_neighbor(iipe);
      if (send_n_particles_to_ipe(ipe) > 0) then
        tag_send    = mype * npe + ipe
        isnd = isnd + 1
        call MPI_ISEND(particle_index_to_be_sent_to_ipe(:,ipe),&
           send_n_particles_to_ipe(ipe),MPI_INTEGER, ipe,tag_send,icomm,&
           sndrqst(isnd),ierrmpi)
      end if
      if (receive_n_particles_from_ipe(ipe) > 0) then
        tag_receive = ipe * npe + mype
        ircv = ircv + 1
        call MPI_IRECV(particle_index_to_be_received_from_ipe(:,ipe),&
           receive_n_particles_from_ipe(ipe),MPI_INTEGER, ipe,tag_receive,&
     ...
     ! should i send some particles to ipe?
      if (send_n_particles_to_ipe(ipe) .gt. 0) then
        ...
        do ipart = 1, send_n_particles_to_ipe(ipe)
          ...
          isnd = isnd+1
          call MPI_ISEND(particle(particle_index_to_be_sent_to_ipe(ipart,&
             ipe))%self, 1,type_particle,ipe,tag_send,icomm,sndrqst(isnd),&
             ierrmpi)
        end do ! ipart
      end if ! send .gt. 0
      ...
      if (receive_n_particles_from_ipe(ipe) .gt. 0) then
        ...
        do ipart = 1, receive_n_particles_from_ipe(ipe)
          ...
          allocate(particle(index)%self)
          call MPI_IRECV(particle(index)%self,1,type_particle,ipe,tag_receive,&
             icomm,rcvrqst(ircv),ierrmpi)
        end do ! ipart
    ...
    do iipe=1,npe_neighbors;ipe=ipe_neighbor(iipe);
      ...
      if (send_n_particles_to_ipe(ipe) .gt. 0) then
        ...
        do ipart = 1, send_n_particles_to_ipe(ipe)
          ...
          isnd = isnd+1
          call MPI_ISEND(particle(particle_index_to_be_sent_to_ipe(ipart,&
             ipe))%payload(1:npayload), npayload,MPI_DOUBLE_PRECISION,ipe,&
      ...
      if (receive_n_particles_from_ipe(ipe) .gt. 0) then
        do ipart = 1, receive_n_particles_from_ipe(ipe)
          ...
          call MPI_IRECV(particle(index)%payload(1:npayload),npayload,&
              MPI_DOUBLE_PRECISION,ipe,tag_receive,icomm,rcvrqst(ircv),&
             ierrmpi)
        end do ! ipart
      end if ! receive .gt. 0
    end do ! ipe
  ...
  subroutine comm_particles_global()
    ...
    do iipart=1,nparticles_on_mype;ipart=particles_on_mype(iipart);
      ...
      if (particle(ipart)%ipe .ne. mype) then
        send_n_particles_to_ipe(ipe_particle) = &
           send_n_particles_to_ipe(ipe_particle) + 1
        particle_index_to_be_sent_to_ipe(send_n_particles_to_ipe(ipe_particle),&
           ipe_particle) = ipart
      end if ! ipe_particle
    ...
    do ipe=0,npe-1;if (ipe .eq. mype) cycle;
      tag_send    = mype * npe + ipe
      tag_receive = ipe * npe + mype
      call MPI_SEND(send_n_particles_to_ipe(ipe),1,MPI_INTEGER,ipe,tag_send,&
         icomm,ierrmpi)
      call MPI_RECV(receive_n_particles_from_ipe(ipe),1,MPI_INTEGER,ipe,&
         tag_receive,icomm,status,ierrmpi)
    end do
    ...
    do ipe=0,npe-1;if (ipe .eq. mype) cycle;
      ...
      if (send_n_particles_to_ipe(ipe) .gt. 0) then
        ...
        do ipart = 1, send_n_particles_to_ipe(ipe)
          send_particles(ipart) = particle(particle_index_to_be_sent_to_ipe(&
             ipart,ipe))%self
          send_payload(1:npayload,ipart) = &
             particle(particle_index_to_be_sent_to_ipe(ipart,&
  ...
  subroutine destroy_particles(destroy_n_particles_mype,&
    ...
    integer, dimension(1:destroy_n_particles_mype),&
        intent(in) :: particle_index_to_be_destroyed
    type(particle_t), dimension(destroy_n_particles_mype):: &
       destroy_particles_mype
    double precision, dimension(npayload,&
       destroy_n_particles_mype):: destroy_payload_mype
    integer                                               :: iipart,ipart,&
       destroy_n_particles
    ...
    ipart=particle_index_to_be_destroyed(iipart);
      ...
      destroy_payload_mype(1:npayload,iipart) = &
         particle(ipart)%payload(1:npayload)
    end do
    ...
    ipart=particle_index_to_be_destroyed(iipart);
      ...
      if(time_advance) then
        write(*,*) particle(ipart)%self%time, ': particle',ipart,&
           ' has left at it ',it_particles,' on pe', mype
        write(*,*) particle(ipart)%self%time, ': particles remaining:',&
           nparticles, '(total)', nparticles_on_mype-1, 'on pe', mype
      endif
