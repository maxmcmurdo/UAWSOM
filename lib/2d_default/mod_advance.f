...
contains
  ...
  subroutine advect(idimmin,idimmax)
    ...
    case (onestep)
       ...
       case (Forward_Euler)
          call advect1(flux_method,one,idimmin,idimmax,global_time,ps1,&
             global_time,ps)

       case (IMEX_Euler)
          call advect1(flux_method,one,idimmin,idimmax,global_time,ps,&
             global_time,ps1)
    ...
    case (twostep)
       ...
       case (Predictor_Corrector)
          ...
          call advect1(typepred1,half,idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          ! corrector step
          fix_conserve_at_step = time_advance .and. levmax>levmin
          call advect1(flux_method,one,idimmin,idimmax,global_time+half*dt,ps1,&
             global_time,ps)

       case (RK2_alf)
          ! RK2 with alfa parameter, where rk_a21=alfa
          call advect1(flux_method,rk_a21, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w = ps(igrid)%w+rk_b1*(ps1(igrid)%w-ps(igrid)%w)/rk_a21
             if(stagger_grid) ps(igrid)%ws = &
                ps(igrid)%ws+(one-rk_b2)*(ps1(igrid)%ws-ps(igrid)%ws)/rk_a21
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,rk_b2,idimmin,idimmax,global_time+rk_a21*dt,&
             ps1,global_time+rk_b1*dt,ps)
       ...
       case (ssprk2)
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%ws = &
                half*ps(igrid)%ws+half*ps1(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,half,idimmin,idimmax,global_time+dt,ps1,&
             global_time+half*dt,ps)
       ...
       case (IMEX_Midpoint)
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_method,half, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%ws = &
                ps(igrid)%ws+2.0d0*(ps2(igrid)%ws-ps1(igrid)%ws)
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,one, idimmin,idimmax,global_time+half*dt,&
             ps2,global_time,ps)
       ...
       case (IMEX_Trapezoidal)
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps2(igrid)%ws = &
                half*(ps(igrid)%ws+ps1(igrid)%ws)
          end do
          !$OMP END PARALLEL DO
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps1(igrid)%ws = &
                ps1(igrid)%ws+half*dt*ps(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%ws = &
                ps2(igrid)%ws+half*dt*ps(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%ws = &
                ps(igrid)%ws+ps2(igrid)%ws-ps1(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,half, idimmin,idimmax,global_time+dt,ps2,&
             global_time+half*dt,ps)
       ...
       case (IMEX_222)
          ...
          call global_implicit_update(imex222_lambda, dt, global_time, ps2,&
              ps)

          ! Set ps1 = y^n + dt.F_ex(xi1)
          call advect1(flux_method, one, idimmin,idimmax, global_time, ps2,&
              global_time, ps1)
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps2(igrid)%ws = (ps2(igrid)%ws - ps(igrid)%ws) / &
                imex222_lambda
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%ws = half*(ps(igrid)%ws + &
                ps1(igrid)%ws + ps2(igrid)%ws)
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps1(igrid)%w = ps1(igrid)%w + (1.0d0 - &
                2.0d0*imex222_lambda)*ps2(igrid)%w
             if(stagger_grid) ps1(igrid)%ws = ps1(igrid)%ws + (1.0d0 - &
                2.0d0*imex222_lambda)*ps2(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call getbc(global_time+dt,dt,ps1,iwstart,nwgc,phys_req_diagonal)
          ...
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%w = 2.0d0*ps2(igrid)%w - ps1(igrid)%w - &
                imex222_lambda*ps2(igrid)%w
             if(stagger_grid) ps2(igrid)%ws = 2.0d0*ps2(igrid)%ws - &
                ps1(igrid)%ws - imex222_lambda*ps2(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w = ps(igrid)%w + (ps2(igrid)%w - ps1(igrid)%w) / &
                (2.0d0 * imex222_lambda)
             if(stagger_grid) ps(igrid)%ws = ps(igrid)%ws + (ps2(igrid)%ws - &
                ps1(igrid)%ws) / (2.0d0 * imex222_lambda)
          end do
          !$OMP END PARALLEL DO
    ...
    case (threestep)
       ...
       case (ssprk3)
          ...
          call advect1(flux_method,rk_beta22, idimmin,idimmax,&
             global_time+rk_c2*dt,ps1,global_time+rk_alfa22*rk_c2*dt,ps2)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          call advect1(flux_method,rk_beta33, idimmin,idimmax,&
             global_time+rk_c3*dt,ps2,global_time+(1.0d0-rk_beta33)*dt,ps)

       case (RK3_BT)
          ! this is a general threestep RK according to its Butcher Table
          call advect1(flux_method,rk3_a21, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps3(igrid)%w=(ps1(igrid)%w-ps(igrid)%w)/rk3_a21
             if(stagger_grid) ps3(igrid)%ws=(ps1(igrid)%ws-&
                ps(igrid)%ws)/rk3_a21
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_method,rk3_a32, idimmin,idimmax,&
             global_time+rk3_c2*dt,ps1,global_time+rk3_a31*dt,ps2)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w=ps(igrid)%w+rk3_b1*ps3(igrid)%w &
                +rk3_b2*(ps2(igrid)%w-(ps(igrid)%w+&
                rk3_a31*ps3(igrid)%w))/rk3_a32
             if(stagger_grid)then
                 ps(igrid)%ws=ps(igrid)%ws+rk3_b1*ps3(igrid)%ws &
                    +rk3_b2*(ps2(igrid)%ws-(ps(igrid)%ws+&
                    rk3_a31*ps3(igrid)%ws))/rk3_a32
             endif
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,rk3_b3, idimmin,idimmax,&
             global_time+rk3_c3*dt,ps2,global_time+(1.0d0-rk3_b3)*dt,ps)

       case (IMEX_ARS3)
          ! this is IMEX scheme ARS3
          call advect1(flux_method,ars_gamma, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps4(igrid)%w=(ps1(igrid)%w-ps(igrid)%w)/ars_gamma
             if(stagger_grid) ps4(igrid)%ws=(ps1(igrid)%ws-&
                ps(igrid)%ws)/ars_gamma
          end do
          !$OMP END PARALLEL DO
          call global_implicit_update(ars_gamma,dt,global_time+ars_gamma*dt,&
             ps2,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps1(igrid)%w=(ps2(igrid)%w-ps1(igrid)%w)/ars_gamma
             if(stagger_grid) ps1(igrid)%ws=(ps2(igrid)%ws-&
                ps1(igrid)%ws)/ars_gamma
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps3(igrid)%w=ps(igrid)%w+(ars_gamma-1.0d0)*ps4(igrid)%w+&
                (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%w
             if(stagger_grid) then
                ps3(igrid)%ws=ps(igrid)%ws+(ars_gamma-1.0d0)*ps4(igrid)%ws+&
                   (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%ws
             endif
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,2.0d0*(1.0d0-ars_gamma), idimmin,idimmax,&
             global_time+ars_gamma*dt,ps2,global_time+(ars_gamma-1.0d0)*dt,&
             ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%w=ps1(igrid)%w+(ps3(igrid)%w-(ps(igrid)%w+ &
                (ars_gamma-1.0d0)*ps4(igrid)%w+&
                (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%w))/(2.0d0*(1.0d0-&
                ars_gamma))
             if(stagger_grid) then
             ps2(igrid)%ws=ps1(igrid)%ws+(ps3(igrid)%ws-(ps(igrid)%ws+ &
                (ars_gamma-1.0d0)*ps4(igrid)%ws+&
                (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%ws))/(2.0d0*(1.0d0-&
                ars_gamma))
             endif
          end do
          !$OMP END PARALLEL DO
          call global_implicit_update(ars_gamma,dt,&
             global_time+(1.0d0-ars_gamma)*dt,ps4,ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_method,half, idimmin,idimmax,&
             global_time+(1.0d0-ars_gamma)*dt,ps4,global_time+half*dt,ps)

       case (IMEX_232)
          ! this is IMEX_ARK(2,3,2) or IMEX_SSP(2,3,2)
          call advect1(flux_method,imex_a21, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps1(igrid)%ws=ps1(igrid)%ws+&
                imex_ha21*dt*ps3(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call getbc(global_time+imex_a21*dt,dt,ps1,iwstart,nwgc,&
             phys_req_diagonal)
          call global_implicit_update(imex_ha22,dt,global_time+imex_c2*dt,ps2,&
             ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w=ps(igrid)%w+imex_a31*ps4(igrid)%w &
                +imex_b1*dt*ps3(igrid)%w+&
                imex_b2*(ps2(igrid)%w-ps1(igrid)%w)/imex_ha22
             if(stagger_grid) then
             ps(igrid)%ws=ps(igrid)%ws+imex_a31*ps4(igrid)%ws &
                +imex_b1*dt*ps3(igrid)%ws+&
                imex_b2*(ps2(igrid)%ws-ps1(igrid)%ws)/imex_ha22
             endif
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_method,imex_a32, idimmin,idimmax,&
             global_time+imex_c2*dt,ps2,global_time+imex_a31*dt,ps)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%w=(ps(igrid)%w-ps3(igrid)%w-&
                imex_a31*ps4(igrid)%w)/imex_a32 &
                +(1.0d0-imex_b2/imex_a32)*(ps2(igrid)%w-&
                ps1(igrid)%w)/imex_ha22
             if(stagger_grid) then
             ps2(igrid)%ws=(ps(igrid)%ws-ps3(igrid)%ws-&
                imex_a31*ps4(igrid)%ws)/imex_a32 &
                +(1.0d0-imex_b2/imex_a32)*(ps2(igrid)%ws-&
                ps1(igrid)%ws)/imex_ha22
             endif
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps1(igrid)%w=ps3(igrid)%w+imex_b1*ps4(igrid)%w+&
                imex_b2*ps2(igrid)%w
             if(stagger_grid) then
             ps1(igrid)%ws=ps3(igrid)%ws+imex_b1*ps4(igrid)%ws+&
                imex_b2*ps2(igrid)%ws
             endif
          end do
          !$OMP END PARALLEL DO
          call global_implicit_update(imex_b3,dt,global_time+imex_c3*dt,ps2,&
             ps)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_method,imex_b3, idimmin,idimmax,&
             global_time+imex_c3*dt,ps2,global_time+(1.0d0-imex_b3)*dt,ps)
       ...
       case (IMEX_CB3a)
          ...
          call advect1(flux_method, imex_ha21, idimmin,idimmax, global_time,&
              ps, global_time, ps1)
          call global_implicit_update(imex_a22, dt, global_time+imex_c2*dt,&
              ps2, ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps3(igrid)%w = ps(igrid)%w + imex_a32/imex_a22 * (ps2(igrid)%w - &
                ps1(igrid)%w)
             ps(igrid)%w  = ps(igrid)%w + imex_b2 /imex_a22 * (ps2(igrid)%w - &
                ps1(igrid)%w)
             ps1(igrid)%w = ps3(igrid)%w
             if(stagger_grid) ps3(igrid)%ws = ps(igrid)%ws + imex_a32/imex_a22 &
                * (ps2(igrid)%ws - ps1(igrid)%ws)
             if(stagger_grid) ps(igrid)%ws  = ps(igrid)%ws + imex_b2 /imex_a22 &
                * (ps2(igrid)%ws - ps1(igrid)%ws)
             if(stagger_grid) ps1(igrid)%ws = ps3(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method, imex_ha32, idimmin,idimmax,&
              global_time+imex_c2*dt, ps2, global_time, ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w = ps(igrid)%w + imex_b2 /imex_ha32 * (ps3(igrid)%w - &
                ps1(igrid)%w)
             if(stagger_grid) ps(igrid)%ws = ps(igrid)%ws + imex_b2 /imex_ha32 &
                * (ps3(igrid)%ws - ps1(igrid)%ws)
          end do
          call global_implicit_update(imex_a33, dt, global_time+imex_c3*dt,&
              ps1, ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w = ps(igrid)%w + imex_b3 /imex_a33 * (ps1(igrid)%w - &
                ps3(igrid)%w)
             if(stagger_grid) ps(igrid)%ws = ps(igrid)%ws + imex_b3 /imex_a33 &
                * (ps1(igrid)%ws - ps3(igrid)%ws)
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method, imex_b3, idimmin,idimmax,&
              global_time+imex_c3*dt, ps1, global_time+imex_b2*dt, ps)
    ...
    case (fourstep)
       ...
       case (ssprk4)
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps1(igrid)%ws=rk_alfa31*ps(igrid)%ws+&
                rk_alfa33*ps2(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,rk_beta33, idimmin,idimmax,&
             global_time+rk_c3*dt,ps2,global_time+rk_alfa33*rk_c3*dt,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w=rk_alfa41*ps(igrid)%w+rk_alfa44*ps1(igrid)%w
             if(stagger_grid) ps(igrid)%ws=rk_alfa41*ps(igrid)%ws+&
                rk_alfa44*ps1(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,rk_beta44, idimmin,idimmax,&
             global_time+rk_c4*dt,ps1,global_time+(1.0d0-rk_beta44)*dt,ps)
       ...
       case (rk4)
          ...
          call advect1(flux_method,half, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          call advect1(flux_method,half, idimmin,idimmax,global_time+half*dt,&
             ps1,global_time,ps2)
          call advect1(flux_method,1.0d0, idimmin,idimmax,global_time+half*dt,&
             ps2,global_time,ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w=(1.0d0/3.0d0)*(-ps(igrid)%w+ps1(igrid)%w+&
                2.0d0*ps2(igrid)%w+ps3(igrid)%w)
             if(stagger_grid) ps(igrid)%ws=(1.0d0/3.0d0) &
                *(-ps(igrid)%ws+ps1(igrid)%ws+2.0d0*ps2(igrid)%ws+&
                ps3(igrid)%ws)
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,1.0d0/6.0d0, idimmin,idimmax,global_time+dt,&
             ps3,global_time+dt*5.0d0/6.0d0,ps)
    ...
    case (fivestep)
       ...
       case (ssprk5)
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps3(igrid)%ws=rk_alfa53*ps2(igrid)%ws+&
                rk_alfa54*ps1(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%w=rk_alfa41*ps(igrid)%w+rk_alfa44*ps1(igrid)%w
             if(stagger_grid) ps2(igrid)%ws=rk_alfa41*ps(igrid)%ws+&
                rk_alfa44*ps1(igrid)%ws
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_method,rk_beta44, idimmin,idimmax,&
             global_time+rk_c4*dt,ps1,global_time+rk_alfa44*rk_c4*dt,ps2)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%w=ps3(igrid)%w+rk_alfa55*ps2(igrid)%w &
                +(rk_beta54/rk_beta44)*(ps2(igrid)%w-(rk_alfa41*ps(igrid)%w+&
                rk_alfa44*ps1(igrid)%w))
             if(stagger_grid) then
             ps(igrid)%ws=ps3(igrid)%ws+rk_alfa55*ps2(igrid)%ws &
                +(rk_beta54/rk_beta44)*(ps2(igrid)%ws-(rk_alfa41*ps(igrid)%ws+&
                rk_alfa44*ps1(igrid)%ws))
             endif
          end do
          !$OMP END PARALLEL DO
          !bcexch = .true.
          call advect1(flux_method,rk_beta55, idimmin,idimmax,&
             global_time+rk_c5*dt,ps2,global_time+(1.0d0-rk_beta55)*dt,ps)
  ...
  subroutine advect1(method,dtfactor,idimmin,idimmax,qtC,psa,qt,psb)
    use mod_global_parameters
    use mod_ghostcells_update
    use mod_fix_conserve
    use mod_physics
    ...
    do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
       ...
       call process1_grid(method(block%level),igrid,qdt,ixGlo1,ixGlo2,ixGhi1,&
          ixGhi2,idimmin,idimmax,qtC,psa(igrid),qt,psb(igrid),pso(igrid))
    end do
    !$OMP END PARALLEL DO
    ...
    if (fix_conserve_global .and. fix_conserve_at_step) then
      ...
      if(stagger_grid) then
        ...
        !$OMP PARALLEL DO PRIVATE(igrid)
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          call phys_face_to_center(ixMlo1,ixMlo2,ixMhi1,ixMhi2,psb(igrid))
        end do
        !$OMP END PARALLEL DO
      end if
      if(phys_solve_eaux) then
        ...
        !$OMP PARALLEL DO PRIVATE(igrid)
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          call phys_energy_synchro(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
             ixMhi1,ixMhi2,psb(igrid)%w,psb(igrid)%x)
        end do
        !$OMP END PARALLEL DO
      end if
    end if
  ...
  subroutine process1_grid(method,igrid,qdt,ixImin1,ixImin2,ixImax1,ixImax2,&
     idimmin,idimmax,qtC,sCT,qt,s,sold)
    use mod_global_parameters
    use mod_fix_conserve

    integer, intent(in) :: method
    integer, intent(in) :: igrid, ixImin1,ixImin2,ixImax1,ixImax2, idimmin,&
       idimmax
    ...
    call advect1_grid(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,idimmin,&
       idimmax,qtC,sCT,qt,s,sold,fC,fE,rnode(rpdx1_:rnodehi,igrid),&
       ps(igrid)%x)
    ...
    if (fix_conserve_global .and. fix_conserve_at_step) then
      call store_flux(igrid,fC,idimmin,idimmax,nwflux)
      if(stagger_grid) call store_edge(igrid,ixImin1,ixImin2,ixImax1,ixImax2,&
         fE,idimmin,idimmax)
    end if
  ...
  subroutine advect1_grid(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,idimmin,&
     idimmax,qtC,sCT,qt,s,sold,fC,fE,dxs,x)
    ...
    case (fs_hll,fs_hllc,fs_hllcd,fs_hlld,fs_tvdlf,fs_tvdmu)
       call finite_volume(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
          ixOmin2,ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,sold,fC,fE,dxs,&
          x)
    case (fs_cd,fs_cd4)
       call centdiff(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
          ixOmin2,ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,fC,fE,dxs,x)
    case (fs_hancock)
       call hancock(qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
          ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,dxs,x)
    case (fs_fd)
       call fd(qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,ixOmax1,&
          ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,fC,fE,dxs,x)
    case (fs_tvd)
       call centdiff(fs_cd,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
          ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,fC,fE,dxs,x)
       call tvdlimit(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
          ixOmin2,ixOmax1,ixOmax2,idimmin,idimmax,sCT,qt+qdt,s,fC,dxs,x)
    case (fs_source)
       call addsource2(qdt*dble(idimmax-idimmin+1)/dble(ndim),ixImin1,ixImin2,&
          ixImax1,ixImax2,ixOmin1,ixOmin2,ixOmax1,ixOmax2,1,nw,qtC,sCT%w,qt,&
          s%w,x,.false.)
    case (fs_nul)
  ...
  subroutine process(iit,qt)
    ...
    if (associated(usr_process_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid);
         block=>ps(igrid)
         call usr_process_grid(igrid,node(plevel_,igrid),ixGlo1,ixGlo2,ixGhi1,&
            ixGhi2,ixMlo1,ixMlo2,ixMhi1,ixMhi2, qt,ps(igrid)%w,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
    ...
    if (associated(usr_process_adv_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call usr_process_adv_grid(igrid,node(plevel_,igrid),ixGlo1,ixGlo2,&
            ixGhi1,ixGhi2,ixMlo1,ixMlo2,ixMhi1,ixMhi2, qt,ps(igrid)%w,&
            ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
      call getbc(qt,dt,ps,iwstart,nwgc,phys_req_diagonal)
