...
contains
    ...
    do n = 1, num_particles
      ...
      if (ipe_particle == mype) then
        ...
        call Lorentz_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,&
           x(:,n),v(:,n)*lfac,q(n),m(n),defpayload,ndefpayload,0.d0)
        ...
        if (associated(usr_update_payload)) then
          call usr_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,&
             x(:,n),v(:,n)*lfac,q(n),m(n),usrpayload,nusrpayload,0.d0)
  ...
  subroutine Lorentz_integrate_particles(end_time)
    ...
    double precision                  :: lfac, q, m, dt_p, cosphi, sinphi,&
        phi1, phi2, r, re
    double precision, dimension(ndir) :: b, e, emom, uminus, t_geom, s, udash,&
        tmp, uplus, xcart1, xcart2, ucart2, radmom, vfluid, current
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      particle(ipart)%self%x(1:ndir) = particle(ipart)%self%x(1:ndir) + 0.5d0 &
         * dt_p * particle(ipart)%self%u(1:ndir)/lfac

      ! Get E, B at new position
      call get_vec(bp, particle(ipart)%igrid, particle(ipart)%self%x,&
         particle(ipart)%self%time,b)
      call get_vec(vp, particle(ipart)%igrid, particle(ipart)%self%x,&
         particle(ipart)%self%time,vfluid)
      call get_vec(jp, particle(ipart)%igrid, particle(ipart)%self%x,&
         particle(ipart)%self%time,current)
      ...
      call Lorentz_kick(particle(ipart)%self%x,particle(ipart)%self%u,e,b,q,m,&
         dt_p)
      ...
      ! Update payload
      call Lorentz_update_payload(particle(ipart)%igrid,&
  ...
  subroutine Lorentz_kick(xpart,upart,e,b,q,m,dtp)
    ...
    double precision                  :: lfac, cosphi, sinphi, phi1, phi2, r,&
        re, sigma
    double precision, dimension(ndir) :: emom, uprime, tau, s, tmp, uplus,&
        xcart1, xcart2, ucart2, radmom
    double precision, dimension(ndir) :: upartk, vbar, Fk, C1, C2, dupartk
    double precision                  :: abserr, tol, lfack, J11, J12, J13,&
...
!        end if
        ...
        ! update position
        xcart2(1:ndir) = xcart1(1:ndir) + dtp * ucart2(1:ndir)/lfac
    ...
    case(Vay)
      ...
      case(Cartesian)!,Cartesian_stretched)
        ...
        lfac = sqrt((sigma + sqrt(sigma**2 + 4.d0 * (sum(tau(:)*tau(:)) + &
           sum(uprime(:)*tau(:)/c_norm)**2))) / 2.d0)
        
        call cross(uprime,tau,tmp)
        upart = (uprime + sum(uprime(:)*tau(:))*tau/lfac**2 + tmp/lfac) / &
           (1.d0+sum(tau(:)*tau(:))/lfac**2)
    ...
    case(HC)
      ...
      case(Cartesian)!,Cartesian_stretched)
        ...
        call cross(uprime,tau,tmp)
        upart = (uprime + sum(uprime(:)*tau(:))*tau/lfac**2 + tmp/lfac) / &
           (1.d0+sum(tau(:)*tau(:))/lfac**2) + emom + tmp/lfac
    ...
    case(LM)
      ...
      case(Cartesian)!,Cartesian_stretched)
        ...
        do while(abserr > tol .and. nk < nkmax)
          ...
          C1 = (lfack + lfac - upartk(1:ndim) / lfack / c_norm**2 * (upartk + &
             upart)) / (lfack + lfac)**2
          C2 = - upartk / lfack / c_norm**2 / (lfack + lfac)**2

          ! Compute Jacobian
          J11 = 1. - q*dtp/m * (C2(1) * (upartk(2) + upart(2)) * b(3) - C2(1) &
             * (upartk(3) + upart(3)) * b(2))
          J12 = - q*dtp/m * (C1(2) * b(3) - C2(2) * (upartk(3) + upart(3)) * &
             b(2))
          J13 = - q*dtp/m * (C2(3) * (upartk(2) + upart(2)) * b(3) - C1(3) * &
             b(2))
          J21 = - q*dtp/m * (- C1(1) * b(3) + C2(1) * (upartk(3) + upart(3)) * &
             b(1))
          J22 = 1. - q*dtp/m * (- C2(2) * (upartk(1) + upart(1)) * b(3) + &
             C2(2) * (upartk(3) + upart(3)) * b(1))
          J23 = - q*dtp/m * (- C2(3) * (upartk(1) + upart(1)) * b(3) + C1(3) * &
             b(1))
          J31 = - q*dtp/m * (C1(1) * b(2) - C2(1) * (upartk(2) + upart(2)) * &
             b(1))
          J32 = - q*dtp/m * (C2(2) * (upartk(1) + upart(1)) * b(2) - C1(2) * &
             b(1))
          J33 = 1. - q*dtp/m * (C2(3) * (upartk(1) + upart(1)) * b(2) - C2(3) &
             * (upartk(2) + upart(2)) * b(1))
...
("Lapenta-Markidis particle pusher incompatible with the chosen geometry")
  ...
  subroutine Lorentz_update_payload(igrid,w,wold,xgrid,xpart,upart,qpart,mpart,&
    ...
    if (mynpayload > 1) then
      ...
      mypayload(2) = mpart/abs(qpart)*sqrt(sum(tmp(:)**2)) / &
         sqrt(sum(b(:)**2))
    end if
  ...
  function Lorentz_get_particle_dt(partp, end_time) result(dt_p)
    ...
    dt_cfl = min(bigdouble, rnode(rpdx1_,partp%igrid)/max(v(1), smalldouble),&
       rnode(rpdx2_,partp%igrid)/max(v(2), smalldouble))

    if(coordinate ==cylindrical.and.phi_>0) then
      ! phi-momentum leads to radial velocity:
      if(phi_ .gt. ndim) dt_cfl = min(dt_cfl, sqrt(rnode(rpdx1_,&
         partp%igrid)/partp%self%x(r_)) / v(phi_))
      ...
      dt_cfl = min(dt_cfl,(partp%self%x(r_)+smalldouble)/max(v(r_),&
          smalldouble))
    end if
    ...
    dt_p = abs( dtheta * partp%self%m * lfac / (partp%self%q * absb) )
