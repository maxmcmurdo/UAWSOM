...
contains
  ...
  subroutine small_values_error(wprim, x, ixImin1,ixImin2,ixImax1,ixImax2,&
      ixOmin1,ixOmin2,ixOmax1,ixOmax2, w_flag, subname)
    ...
    integer                      :: iw,iiw,ix1,ix2

    if (.not.crash) then
      do iw=1,nw
       do ix2= ixOmin2,ixOmax2
       do ix1= ixOmin1,ixOmax1
          if(w_flag(ix1,ix2,iw)) then
            write(*,*) "Error: small value of ", trim(prim_wnames(iw)),&
               wprim(ix1,ix2,iw)," encountered when call ", subname
            write(*,*) "Iteration: ", it, " Time: ", global_time,&
                "Processor: ",mype
            write(*,*) "Location: ", x(ix1,ix2,:)
            write(*,*) "Cell number: ", ix1,ix2
            do iiw=1,nw
              write(*,*) trim(prim_wnames(iiw)),": ",wprim(ix1,ix2,iiw)
            end do
            ! use erroneous arithmetic operation to crash the run
            if(trace_small_values) write(*,*) sqrt(wprim(ix1,ix2,&
               iw)-bigdouble)
  ...
  subroutine small_values_average(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2, w, x, w_flag, windex)
    use mod_global_parameters
      ...
      ! point with local failure identified by w_flag
        if (w_flag(ix1,ix2,iw)) then
          ...
          do i = 1, max(small_values_daverage, 1)
            ...
            if(any(w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2,&
               iw) .eqv. .false.)) exit
          end do
          ...
          if(any(w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2,&
            ...
            if(small_values_fix_iw(iw)) then
              w(ix1,ix2, iw) = sum(w(kxOmin1:kxOmax1,kxOmin2:kxOmax2, iw),&
                  w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2,&
                 iw) .eqv. .false.)/ count(w_flag(kxOmin1:kxOmax1,&
                 kxOmin2:kxOmax2,iw) .eqv. .false.)
            end if
          else
            ...
            if(iw==iw_e) w(ix1,ix2, iw)=small_pressure
            if(iw==iw_rho) w(ix1,ix2, iw)=small_density
          end if
        end if
     enddo
     enddo
