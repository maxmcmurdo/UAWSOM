...
contains
  ...
  subroutine get_interp_factor_linear(xc,xp,factor)
    ...
    double precision :: xc(0:1,0:1,1:ndim),xp(1:ndim),factor(0:1,0:1)
    integer :: ix1,ix2
    double precision :: dxc1,dxc2,xd1,xd2

    dxc1=xc(1,1,1)-xc(0,0,1)
    dxc2=xc(1,1,2)-xc(0,0,2)
    xd1=(xp(1)-xc(0,0,1))/dxc1
    xd2=(xp(2)-xc(0,0,2))/dxc2
  ...
  subroutine interp_linear_multiD(xc,xp,wc,wp,nwc)
    ...
    integer :: nwc
    double precision :: xc(0:1,0:1,1:ndim),xp(1:ndim)
    double precision :: wc(0:1,0:1,1:nwc),wp(1:nwc)
    ...
    do iw=1,nwc
      do ix2=0,1
      do ix1=0,1
        wp(iw)=wp(iw)+wc(ix1,ix2,iw)*factor(ix1,ix2)
      enddo
      enddo
    enddo
  ...
  subroutine interp_from_grid_linear(igrid,xp,wp)
    ...
    integer :: inblock,ixOmin1,ixOmin2,ixOmax1,ixOmax2,j
    integer :: ixb1,ixb2,ixi1,ixi2
    double precision :: xc(0:1,0:1,1:ndim),wc(0:1,0:1,nw)
    ...
    ixOmin1=ixmlo1
    ixOmin2=ixmlo2
    ixb1=floor((xp(1)-ps(igrid)%x(ixOmin1,ixOmin2,1))/dxb1)+ixOmin1
    ixb2=floor((xp(2)-ps(igrid)%x(ixOmin1,ixOmin2,2))/dxb2)+ixOmin2

    ! nearby cells for interpolation
    do ixi1=0,1
    do ixi2=0,1
      xc(ixi1,ixi2,:)=ps(igrid)%x(ixb1+ixi1,ixb2+ixi2,:)
      wc(ixi1,ixi2,:)=ps(igrid)%w(ixb1+ixi1,ixb2+ixi2,:)
    enddo
    enddo
