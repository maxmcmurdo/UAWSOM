...
contains
  ...
  subroutine mg_setup_multigrid()
    ...
    if (ndim /= mg_ndim) error stop "Multigrid module was compiled for different ndim"
    ...
    if (any([ block_nx1,block_nx2 ] /= block_nx1)) error stop "Multigrid requires all block_nx to be equal"
  ...
  subroutine mg_copy_to_tree(iw_from, iw_to, restrict, restrict_gc, factor,&
    ...
    integer                        :: nc, lvl, ilo1,ilo2, ihi1,ihi2
    type(tree_node), pointer       :: pnode
    real(dp)                       :: fac
    logical                        :: do_restrict, do_gc

    if (.not. mg%is_allocated) error stop "mg_copy_to_tree: tree not allocated yet"
    ...
    ilo1=ixMlo1-1;ilo2=ixMlo2-1;
    ihi1=ixMhi1+1;ihi2=ixMhi2+1;
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_from)) then
          mg%boxes(id)%cc(0:nc+1,0:nc+1, iw_to) = fac * &
             state_from(igrid)%w(ilo1:ihi1,ilo2:ihi2, iw_from)
       else
          mg%boxes(id)%cc(0:nc+1,0:nc+1, iw_to) = fac * ps(igrid)%w(ilo1:ihi1,&
             ilo2:ihi2, iw_from)
       end if
    end do
  ...
  subroutine mg_copy_from_tree(iw_from, iw_to, state_to)
    ...
    if (.not. mg%is_allocated) error stop "mg_copy_from_tree: tree not allocated yet"
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
              iw_to) = mg%boxes(id)%cc(1:nc,1:nc, iw_from)
       else
  ...
  subroutine mg_copy_from_tree_gc(iw_from, iw_to, state_to)
    ...
    if (.not. mg%is_allocated) error stop "mg_copy_from_tree_gc: tree not allocated yet"
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%w(ilo1:ihi1,ilo2:ihi2,&
              iw_to) = mg%boxes(id)%cc(0:nc+1,0:nc+1, iw_from)
       else
          ps(igrid)%w(ilo1:ihi1,ilo2:ihi2, iw_to) = mg%boxes(id)%cc(0:nc+1,&
             0:nc+1, iw_from)
       end if
    end do
  ...
  subroutine mg_tree_from_amrvac(mg)
    ...
    call mg_build_rectangle(mg, [ domain_nx1,domain_nx2 ], block_nx1, dx(:,1),&
        [ xprobmin1,xprobmin2 ], periodB, n_finer)
    ...
    do i = 1, size(mg%lvls(1)%ids)
       ...
       pnode               => tree_root(ix(1),ix(2))%node
       pnode%id            =  id
       ...
       do i = 1, size(mg%lvls(lvl)%ids)
          ...
          if (.not. pnode%leaf) then
             ...
             do i_c = 1, mg_num_children
                ...
                pnode_ch => pnode%child(c_ix(1),c_ix(2))%node
