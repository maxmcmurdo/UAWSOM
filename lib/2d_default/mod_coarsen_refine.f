...
contains
  ...
  subroutine amr_coarsen_refine
    ...
    if(stagger_grid) then
      allocate(recvstatus_stg(MPI_STATUS_SIZE,max_blocks*2),&
         recvrequest_stg(max_blocks*2), sendstatus_stg(MPI_STATUS_SIZE,&
         max_blocks*2),sendrequest_stg(max_blocks*2))
  ...
  subroutine proper_nesting
    ...
    integer :: iigrid, igrid, level, ic1,ic2, inp1,inp2, i1,i2,&
    ...
    do ic1=1,2
       ...
       if (ipeFi==mype) then
          ...
          if (ipe==mype) then
             ...
             call coarsen_grid(ps(igridFi),ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
                ixMlo2,ixMhi1,ixMhi2,ps(igrid),ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
             ...
             call coarsen_grid(ps(igridFi),ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
                ixMlo2,ixMhi1,ixMhi2,psc(igridFi), ixCoGmin1,ixCoGmin2,&
                ixCoGmax1,ixCoGmax2,ixCoMmin1,ixCoMmin2,ixCoMmax1,ixCoMmax2)
             ...
             if(stagger_grid) then
               do idir=1,ndim
                 ...
                 call MPI_ISEND(psc(igridFi)%ws,1,type_coarse_block_stg(idir,&
                    ic1,ic2),ipe,itag_stg, icomm,sendrequest_stg(isend),&
                    ierrmpi)
               end do
             end if
          end if
          ...
          if (ipe==mype) then
             ...
             itag=ipeFi+igridFi
             irecv=irecv+1
             call MPI_IRECV(ps(igrid)%w,1,type_sub_block(ic1,ic2),ipeFi,itag,&
             ...
             if(stagger_grid) then
               do idir=1,ndim
                 ...
                 call MPI_IRECV(ps(igrid)%ws,1,type_sub_block_stg(idir,ic1,&
                    ic2),ipeFi,itag_stg, icomm,recvrequest_stg(irecv),ierrmpi)
               end do
             end if
          end if
       end if
