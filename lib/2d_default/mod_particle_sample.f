...
contains
  ...
  subroutine sample_create_particles()
    ...
    do n=1,num_particles
      ...
      if(ipe_particle == mype) then
        ...
        call sample_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,&
  ...
  subroutine sample_fill_gridvars
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         1:ngridvars) = w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ngridvars)
  ...
  subroutine sample_integrate_particles(end_time)
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      dt_p                    = sample_get_particle_dt(particle(ipart),&
          end_time)
      ...
      if (associated(usr_particle_position)) call usr_particle_position(x,&
         ipart,tloc,tlocnew)
      ...
      ! Update payload
      call sample_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,x,&
  ...
  !> Payload update
  subroutine sample_update_payload(igrid,w,wold,xgrid,xpart,upart,qpart,mpart,&
    ...
    do ii=1,mynpayload
      if (.not.time_advance) then
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ii),xgrid,xpart,wp)
      else
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ii),xgrid,xpart,wp1)
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ii),xgrid,xpart,wp2)
