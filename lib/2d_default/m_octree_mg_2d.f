...
module m_octree_mg_2d
  ...
  integer, parameter, public :: mg_child_dix(2, 4) = reshape([0,0,1,0,0,1,1,1],&
      [2,4])
  ! Reverse child index in each direction
  integer, parameter, public :: mg_child_rev(4, 2) = reshape([2,1,4,3,3,4,1,2],&
      [4,2])
  ! Children adjacent to a neighbor
  integer, parameter, public :: mg_child_adj_nb(2, 4) = reshape([1,3,2,4,1,2,3,&
     4], [2,4])
  ...
  integer, parameter, public :: mg_neighb_dix(2, 4) = reshape([-1,0,1,0,0,-1,0,&
     1], [2,4])
  ! Which neighbors have a lower index
  logical, parameter, public :: mg_neighb_low(4) = [.true., .false., .true.,&
      .false.]
  ...
  type, public :: mg_box_t
     ...
     integer  :: ix(2)          !< Spatial index
     integer  :: parent            !< Id of parent
     integer  :: children(2**2) !< Ids of children
     integer  :: neighbors(2*2) !< Ids of neighbors
     real(dp) :: r_min(2)       !< Minimum coordinate
     real(dp) :: dr(2)          !< Grid spacing
     !> Cell-centered data
  ...
  type, public :: mg_t
     ...
     integer                  :: domain_size_lvl(2, mg_lvl_lo:mg_lvl_hi)
     !> Grid spacing per level
     real(dp)                 :: dr(2, mg_lvl_lo:mg_lvl_hi)
     !> Minimum coordinates
     real(dp)                 :: r_min(2)
     !> List of all levels
  ...
  interface
     ...
     subroutine mg_box_prolong(mg, p_id, dix, nc, iv, fine)
       ...
       integer, intent(in)       :: dix(2) !< Offset of child in parent grid
...
contains
  ...
  pure function mg_highest_uniform_lvl(mg) result(lvl)
    ...
    do lvl = mg%first_normal_lvl, mg%highest_lvl-1
       ...
       if (size(mg%lvls(lvl)%leaves) /= 0 .and. size(mg%lvls(lvl)%parents) /= &
          0) exit
    end do
  ...
  subroutine mg_timers_show(mg)
    ...
    call mpi_reduce(mg%timers(1:mg%n_timers)%t, tmin, mg%n_timers, mpi_double,&
        mpi_min, 0, mg%comm, ierr)
    call mpi_reduce(mg%timers(1:mg%n_timers)%t, tmax, mg%n_timers, mpi_double,&
        mpi_max, 0, mg%comm, ierr)

    if (mg%my_rank == 0) then
       write(*, "(A20,2A16)") "name                ", "min(s)", "max(s)"
       do n = 1, mg%n_timers
          write(*, "(A20,2F16.6)") mg%timers(n)%name, tmin(n), tmax(n)
       end do
    end if
  ...
  subroutine mg_deallocate_storage(mg)
    ...
    if (.not. mg%is_allocated) error stop "deallocate_storage: tree is not allocated"
  ...
  subroutine mg_allocate_storage(mg)
    ...
    if (.not. mg%tree_created) error stop "allocate_storage: tree is not yet created"

    if (mg%is_allocated) error stop "allocate_storage: tree is already allocated"

    do lvl = mg%lowest_lvl, mg%highest_lvl
       nc = mg%box_size_lvl(lvl)
       do i = 1, size(mg%lvls(lvl)%my_ids)
          id = mg%lvls(lvl)%my_ids(i)
          allocate(mg%boxes(id)%cc(0:nc+1, 0:nc+1,&
    ...
    call mg_ghost_cell_buffer_size(mg, n_send(:, 1), n_recv(:, 1), dsize(1))
    call mg_restrict_buffer_size(mg, n_send(:, 2), n_recv(:, 2), dsize(2))
    call mg_prolong_buffer_size(mg, n_send(:, 3), n_recv(:, 3), dsize(3))
  ...
  subroutine set_rhs(mg, f1, f2)
    ...
    do lvl = 1, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_leaves)
          ...
          mg%boxes(id)%cc(1:nc, 1:nc, mg_irhs) = f1 * mg%boxes(id)%cc(1:nc,&
              1:nc, mg_iphi) + f2 * mg%boxes(id)%cc(1:nc, 1:nc, mg_irhs)
       end do
    end do
  ...
  subroutine box_gs_lpl(mg, id, nc, redblack_cntr)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      do j = 1, nc
         if (redblack) i0 = 2 - iand(ieor(redblack_cntr, j), 1)

         do i = i0, nc, di
            cc(i, j, n) = fac * ( idr2(1) * (cc(i+1, j, n) + cc(i-1, j,&
                n)) + idr2(2) * (cc(i, j+1, n) + cc(i, j-1, n)) - cc(i, j,&
                mg_irhs))
         end do
      end do
    end associate
...
!   end subroutine box_jacobi_lpl
  ...
  subroutine box_lpl(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      do j = 1, nc
         do i = 1, nc
            cc(i, j, i_out) = idr2(1) * (cc(i-1, j, n) + cc(i+1, j,&
                n) - 2 * cc(i, j, n)) + idr2(2) * (cc(i, j-1, n) + cc(i, j+1,&
                n) - 2 * cc(i, j, n))
         end do
      end do
    end associate
  ...
  subroutine box_clpl(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      do j = 1, nc
         do i = 1, nc
            cc(i, j, i_out) = idr2(1) * (r_face(i) * r_inv(i) * cc(i-1, j,&
                n) + r_face(i+1) * r_inv(i) * cc(i+1, j, n) - 2 * cc(i, j,&
                n)) + idr2(2) * (cc(i, j-1, n) +  cc(i, j+1, n) - 2 * cc(i, j,&
                n))
         end do
      end do
    end associate
  ...
  subroutine box_gs_clpl(mg, id, nc, redblack_cntr)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      do j = 1, nc
         ...
         do i = i0, nc, di
            cc(i, j, n) = fac * (idr2(1) * (r_face(i+1) * r_inv(i) * cc(i+1, j,&
                n) + r_face(i) * r_inv(i) * cc(i-1, j, n)) + idr2(2) * (cc(i,&
                j+1, n) + cc(i, j-1, n)) - cc(i, j, mg_irhs))
         end do
      end do
    end associate
  ...
  subroutine vhelmholtz_set_lambda(lambda)
    ...
    if (lambda < 0) error stop "vhelmholtz_set_lambda: lambda < 0 not allowed"
  ...
  subroutine box_gs_vhelmh(mg, id, nc, redblack_cntr)
    ...
    integer                   :: i, j, i0, di
    logical                   :: redblack
    real(dp)                  :: idr2(2*2), u(2*2)
    real(dp)                  :: a0, a(2*2), c(2*2)

    ! Duplicate 1/dr^2 array to multiply neighbor values
    idr2(1:2*2:2) = 1/mg%dr(:, mg%boxes(id)%lvl)**2
    idr2(2:2*2:2) = idr2(1:2*2:2)
    i0  = 1
    ...
    ! The parity of redblack_cntr determines which cells we use. If
    ! redblack_cntr is even, we use the even cells and vice versa.
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, i_eps => mg_iveps)
      do j = 1, nc
         if (redblack) i0 = 2 - iand(ieor(redblack_cntr, j), 1)
         ...
         do i = i0, nc, di
            ...
            cc(i, j, n) = (sum(c(:) * u(:)) - cc(i, j,&
                mg_irhs)) / (sum(c(:)) + vhelmholtz_lambda)
         end do
      end do
    end associate
  ...
  subroutine box_vhelmh(mg, id, nc, i_out)
    ...
    integer                   :: i, j
    real(dp)                  :: idr2(2*2), a0, u0, u(2*2), a(2*2)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, i_eps => mg_iveps)
      do j = 1, nc
         do i = 1, nc
            ...
            cc(i, j, i_out) = sum(2 * idr2 * a0*a(:)/(a0 + a(:)) * (u(:) - &
               u0)) - vhelmholtz_lambda * u0
         end do
      end do
    end associate
  ...
  subroutine mg_build_rectangle(mg, domain_size, box_size, dx, r_min, periodic,&
      n_finer)
    type(mg_t), intent(inout) :: mg
    integer, intent(in)       :: domain_size(2)
    ...
    integer                   :: i, j, lvl, n, id, nx(2)
    integer                   :: boxes_per_dim(2, mg_lvl_lo:1)
    integer                   :: periodic_offset(2)

    if (modulo(box_size, 2) /= 0) error stop "box_size should be even"
    if (any(modulo(domain_size, box_size) /= 0)) error stop "box_size does not divide domain_size"
    ...
    do lvl = 1, mg_lvl_lo+1, -1
       ...
       if (any(modulo(nx, 2) == 1 .or. nx == mg%coarsest_grid .or. &
          (mg%box_size_lvl(lvl) == mg%coarsest_grid .and. mg%smoother_type == &
          mg_smoother_gs))) exit
    ...
    do j=1,nx(2); do i=1,nx(1)
       ...
       where ([i, j] == 1 .and. .not. periodic)
          mg%boxes(n)%neighbors(1:mg_num_neighbors:2) = mg_physical_boundary
       end where
       where ([i, j] == 1 .and. periodic)
          mg%boxes(n)%neighbors(1:mg_num_neighbors:2) = n + periodic_offset
       end where

       where ([i, j] == nx .and. .not. periodic)
          mg%boxes(n)%neighbors(2:mg_num_neighbors:2) = mg_physical_boundary
       end where
       where ([i, j] == nx .and. periodic)
          mg%boxes(n)%neighbors(2:mg_num_neighbors:2) = n - periodic_offset
       end where
    end do; end do
    ...
    do lvl = mg%lowest_lvl, 1
       if (allocated(mg%lvls(lvl)%ref_bnds)) deallocate(mg%lvls(lvl)%ref_bnds)
  ...
  subroutine mg_set_next_level_ids(mg, lvl)
    ...
    if (allocated(mg%lvls(lvl+1)%ids)) deallocate(mg%lvls(lvl+1)%ids)
  ...
  subroutine mg_add_children(mg, id)
    ...
    do i = 1, mg_num_children
       ...
       mg%boxes(c_id)%r_min     = mg%boxes(id)%r_min + mg%dr(:,&
           lvl) * mg_child_dix(:, i) * mg%box_size
  ...
  subroutine mg_load_balance(mg)
    ...
    do lvl = mg%highest_lvl, single_cpu_lvl+1, -1
       ...
       do i = 1, size(mg%lvls(lvl)%leaves)
          ...
          if ((mg%n_cpu - i_cpu - 1) * my_work(i_cpu) >= work_left + &
             sum(my_work(i_cpu+1:))) then
             i_cpu = i_cpu + 1
          end if
    ...
    if (single_cpu_lvl < mg%highest_lvl) then
       coarse_rank = most_popular(mg%boxes(mg%lvls(single_cpu_lvl+1)%ids)%rank,&
           my_work, mg%n_cpu)
    else
       coarse_rank = 0
    end if
  ...
  pure integer function most_popular(list, work, n_cpu)
    ...
    do i = 1, size(list)
       ...
       if (current_count > best_count .or. current_count == best_count .and. &
          my_work < best_work) then
  ...
  subroutine update_lvl_info(mg, lvl)
    ...
    lvl%my_ids = pack(lvl%ids, mg%boxes(lvl%ids)%rank == mg%my_rank)
    lvl%my_leaves = pack(lvl%leaves, mg%boxes(lvl%leaves)%rank == mg%my_rank)
    lvl%my_parents = pack(lvl%parents, mg%boxes(lvl%parents)%rank == &
       mg%my_rank)
    lvl%my_ref_bnds = pack(lvl%ref_bnds, mg%boxes(lvl%ref_bnds)%rank == &
       mg%my_rank)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, i_eps => mg_iveps)
      do j = 1, nc
         ...
         do i = i0, nc, di
            ...
            cc(i, j, n) = (sum(c(:) * u(:)) - cc(i, j, mg_irhs)) / sum(c(:))
         end do
      end do
    end associate
...
!   end subroutine vlpl_prolong
  ...
  subroutine mg_fill_ghost_cells_lvl(mg, lvl, iv)
    ...
    if (lvl < mg%lowest_lvl) error stop "fill_ghost_cells_lvl: lvl < lowest_lvl"
    ...
    do nb = 1, mg_num_neighbors
       ...
       if (nb_id > mg_no_box) then
          ...
          if (nb_rank /= mg%my_rank) then
             call buffer_for_nb(mg, mg%boxes(id), nc, iv, nb_id, nb_rank, nb,&
                 dry_run)
          end if
       end if
    end do
    ...
    do nb = 1, mg_num_neighbors
       ...
       if (nb_id > mg_no_box) then
          if (mg_has_children(mg%boxes(nb_id))) then
             c_ids = mg%boxes(nb_id)%children(mg_child_adj_nb(:,&
                 mg_neighb_rev(nb)))
  ...
  subroutine set_ghost_cells(mg, id, nc, iv, dry_run)
    ...
    do nb = 1, mg_num_neighbors
       ...
       if (nb_id > mg_no_box) then
          ...
          if (nb_rank /= mg%my_rank) then
             call fill_buffered_nb(mg, mg%boxes(id), nb_rank, nb, nc, iv,&
                 dry_run)
          else if (.not. dry_run) then
             call copy_from_nb(mg%boxes(id), mg%boxes(nb_id), nb, nc, iv)
          end if
          ...
          if (mg%phi_bc_data_stored .and. iv == mg_iphi) then
             ...
             if (associated(mg%bc(nb, iv)%boundary_cond)) then
                call mg%bc(nb, iv)%boundary_cond(mg%boxes(id), nc, iv, nb,&
                    bc_type, bc)
             else
  ...
  subroutine buffer_for_fine_nb(mg, box, nc, iv, fine_id, fine_rank, nb,&
      dry_run)
    type(mg_t), intent(inout)  :: mg
    type(mg_box_t), intent(inout) :: box
    integer, intent(in)        :: nc
  ...
  subroutine bc_to_gc(mg, id, nc, iv, nb, bc_type)
    ...
    select case (nb)
    case (mg_neighb_lowx)
       mg%boxes(id)%cc(0, 1:nc, iv) = c0 * mg%boxes(id)%cc(0, 1:nc,&
           iv) + c1 * mg%boxes(id)%cc(1, 1:nc, iv) + c2 * mg%boxes(id)%cc(2,&
           1:nc, iv)
    case (mg_neighb_highx)
       mg%boxes(id)%cc(nc+1, 1:nc, iv) = c0 * mg%boxes(id)%cc(nc+1, 1:nc,&
           iv) + c1 * mg%boxes(id)%cc(nc, 1:nc,&
           iv) + c2 * mg%boxes(id)%cc(nc-1, 1:nc, iv)
    case (mg_neighb_lowy)
       mg%boxes(id)%cc(1:nc, 0, iv) = c0 * mg%boxes(id)%cc(1:nc, 0,&
           iv) + c1 * mg%boxes(id)%cc(1:nc, 1, iv) + c2 * mg%boxes(id)%cc(1:nc,&
           2, iv)
    case (mg_neighb_highy)
       mg%boxes(id)%cc(1:nc, nc+1, iv) = c0 * mg%boxes(id)%cc(1:nc, nc+1,&
           iv) + c1 * mg%boxes(id)%cc(1:nc, nc,&
           iv) + c2 * mg%boxes(id)%cc(1:nc, nc-1, iv)
    end select
  ...
  subroutine sides_rb(box, nc, iv, nb, gc)
    ...
    case (1)
       ...
       do j = 1, nc
          dj = -1 + 2 * iand(j, 1)
          box%cc(i-di, j, iv) = 0.5_dp * gc(j) + 0.75_dp * box%cc(i, j,&
              iv) - 0.25_dp * box%cc(i+di, j, iv)
       end do
    case (2)
       j = ix
       dj = dix
       do i = 1, nc
          di = -1 + 2 * iand(i, 1)
          box%cc(i, j-dj, iv) = 0.5_dp * gc(i) + 0.75_dp * box%cc(i, j,&
              iv) - 0.25_dp * box%cc(i, j+dj, iv)
       end do
    end select
  ...
  subroutine mg_prolong_buffer_size(mg, n_send, n_recv, dsize)
    ...
    allocate(mg%comm_prolong%n_send(0:mg%n_cpu-1, min_lvl:mg%highest_lvl))
    allocate(mg%comm_prolong%n_recv(0:mg%n_cpu-1, min_lvl:mg%highest_lvl))

    mg%comm_prolong%n_recv(:, mg%highest_lvl) = 0
    mg%comm_prolong%n_send(:, mg%highest_lvl) = 0

    do lvl = min_lvl, mg%highest_lvl-1
       mg%comm_prolong%n_recv(:, lvl) = mg%comm_restrict%n_send(:, lvl+1)
       mg%comm_prolong%n_send(:, lvl) = mg%comm_restrict%n_recv(:, lvl+1)
    end do
  ...
  subroutine prolong_onto(mg, id, nc, iv, iv_to, add, method)
    ...
    logical, intent(in)       :: add   !< If true, add to current values
    procedure(mg_box_prolong) :: method
    integer                   :: hnc, p_id, p_rank, i, dix(2), dsize
    real(dp)                  :: tmp(nc, nc)
    ...
    if (p_rank == mg%my_rank) then
       ...
       tmp = reshape(mg%buf(p_rank)%recv(i+1:i+dsize), [nc, nc])
       mg%buf(p_rank)%i_recv = mg%buf(p_rank)%i_recv + dsize
    end if

    if (add) then
       mg%boxes(id)%cc(1:nc, 1:nc, iv_to) = mg%boxes(id)%cc(1:nc, 1:nc,&
           iv_to) + tmp
    else
       mg%boxes(id)%cc(1:nc, 1:nc, iv_to) = tmp
    end if
  ...
  subroutine helmholtz_set_lambda(lambda)
    ...
    if (lambda < 0) error stop "helmholtz_set_lambda: lambda < 0 not allowed"
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      do j = 1, nc
         do i = 1, nc
            cc(i, j, i_out) = idr2(1) * (cc(i-1, j, n) + cc(i+1, j,&
                n) - 2 * cc(i, j, n)) + idr2(2) * (cc(i, j-1, n) + cc(i, j+1,&
                n) - 2 * cc(i, j, n)) - helmholtz_lambda * cc(i, j, n)
         end do
      end do
    end associate
  ...
  subroutine check_methods(mg)
    ...
    if (.not. associated(mg%box_op) .or. .not. associated(mg%box_smoother)) &
       then
       call mg_set_methods(mg)
    end if
  ...
  subroutine mg_fas_fmg(mg, have_guess, max_res)
    ...
    do lvl = mg%lowest_lvl, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_ids)
          id = mg%lvls(lvl)%my_ids(i)
          mg%boxes(id)%cc(:, :, mg_iold) = mg%boxes(id)%cc(:, :, mg_iphi)
       end do
  ...
  subroutine mg_fas_vcycle(mg, highest_lvl, max_res, standalone)
    ...
    if (is_standalone) call mg_timer_start(mg%timers(timer_total_vcycle))
    ...
    do i = 1, mg%max_coarse_cycles
       ...
       if (res < mg%residual_coarse_rel * init_res .or. res < &
          mg%residual_coarse_abs) exit
    end do
    ...
    if (present(max_res)) then
       ...
       call mpi_allreduce(init_res, max_res, 1, mpi_double, mpi_max, mg%comm,&
           ierr)
    end if
    ...
    if (is_standalone) call mg_timer_end(mg%timers(timer_total_vcycle))
  ...
  subroutine subtract_mean(mg, iv, include_ghostcells)
    ...
    call mpi_allreduce(sum_iv, mean_iv, 1, mpi_double, mpi_sum, mg%comm, ierr)

    ! Divide by total grid volume to get mean
    volume = nc**2 * product(mg%dr(:, 1)) * size(mg%lvls(1)%ids)
    ...
    do lvl = mg%lowest_lvl, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_ids)
          ...
          if (include_ghostcells) then
             mg%boxes(id)%cc(:, :, iv) = mg%boxes(id)%cc(:, :, iv) - mean_iv
          else
             mg%boxes(id)%cc(1:nc, 1:nc, iv) = mg%boxes(id)%cc(1:nc, 1:nc,&
                 iv) - mean_iv
          end if
       end do
    end do
  ...
  real(dp) function get_sum(mg, iv)
    ...
    do lvl = 1, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_leaves)
          id = mg%lvls(lvl)%my_leaves(i)
          get_sum = get_sum + w * sum(mg%boxes(id)%cc(1:nc, 1:nc, iv))
       end do
    end do
  ...
  real(dp) function max_residual_lvl(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl)%my_ids)
       ...
       res = maxval(abs(mg%boxes(id)%cc(1:nc, 1:nc, mg_ires)))
  ...
  !     integer  :: nc, nx(2), my_boxes, total_boxes
  ...
  !        rmin  = [0.0_dp, 0.0_dp]
  !        rmax  = mg%dr(1) * [nc, nc]
  !        rhs   = mg%boxes(1)%cc(1:nc, 1:nc, mg_irhs)
  ...
  !        mg%boxes(1)%cc(1:nc, 1:nc, mg_iphi) = rhs
  ...
  subroutine update_coarse(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl-1)%my_parents)
       ...
       mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_irhs) = mg%boxes(id)%cc(1:nc_c,&
           1:nc_c, mg_irhs) + mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_ires)
  ...
  subroutine correct_children(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl)%my_parents)
       ...
       mg%boxes(id)%cc(:, :, mg_ires) = mg%boxes(id)%cc(:, :,&
           mg_iphi) - mg%boxes(id)%cc(:, :, mg_iold)
    end do
  ...
  subroutine residual_box(mg, id, nc)
    ...
    mg%boxes(id)%cc(1:nc, 1:nc, mg_ires) = mg%boxes(id)%cc(1:nc, 1:nc,&
        mg_irhs) - mg%boxes(id)%cc(1:nc, 1:nc, mg_ires)
  ...
  subroutine restrict_set_buffer(mg, id, iv)
    ...
    if (p_rank /= mg%my_rank) then
       do j = 1, hnc
          do i = 1, hnc
             tmp(i, j) = 0.25_dp * sum(mg%boxes(id)%cc(2*i-1:2*i, 2*j-1:2*j,&
                 iv))
          end do
       end do
    ...
    do i_c = 1, mg_num_children
       ...
       if (c_rank == mg%my_rank) then
          do j=1, hnc; do i=1, hnc
             mg%boxes(id)%cc(dix(1)+i, dix(2)+j,&
                 iv) = 0.25_dp * sum(mg%boxes(c_id)%cc(2*i-1:2*i, 2*j-1:2*j,&
                 iv))
          end do; end do
       else
          i = mg%buf(c_rank)%i_recv
          mg%boxes(id)%cc(dix(1)+1:dix(1)+hnc, dix(2)+1:dix(2)+hnc,&
              iv) = reshape(mg%buf(c_rank)%recv(i+1:i+dsize), [hnc, hnc])
  ...
  subroutine ahelmholtz_set_methods(mg)
    ...
    if (mg%n_extra_vars == 0 .and. mg%is_allocated) then
      ...
      mg%n_extra_vars = max(2, mg%n_extra_vars)
    end if
  ...
  subroutine ahelmholtz_set_lambda(lambda)
    ...
    if (lambda < 0) error stop "ahelmholtz_set_lambda: lambda < 0 not allowed"
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, i_eps1 => mg_iveps1,&
      ...
      do j = 1, nc
         ...
         do i = i0, nc, di
            ...
            cc(i, j, n) = (sum(c(:) * u(:)) - cc(i, j,&
                mg_irhs)) / (sum(c(:)) + ahelmholtz_lambda)
         end do
      end do
    end associate
  ...
  subroutine box_ahelmh(mg, id, nc, i_out)
    ...
    integer, intent(in)       :: i_out !< Index of variable to store Helmholtz in
    integer                   :: i, j
    real(dp)                  :: idr2(2*2), a0(2*2), u0, u(2*2), a(2*2)
