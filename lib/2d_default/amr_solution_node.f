...
subroutine alloc_node(igrid)
  ...
  if(.not. allocated(ps(igrid)%w)) then
    ...
    call alloc_state_coarse(igrid, psc(igrid), ixCoGmin1,ixCoGmin2,ixCoGmax1,&
  ...
  select case (coordinate)
    ...
    case (spherical)
      ps(igrid)%dvolume(ixGextmin1:ixGextmax1,&
         ...
         2))) *dsin(half*ps(igrid)%dx(ixGextmin1:ixGextmax1,&
         ixGextmin2:ixGextmax2,2))
      psc(igrid)%dvolume(ixCoGmin1:ixCoGmax1,&
         ...
         ixCoGmin2:ixCoGmax2,1) *two*dabs(dsin(psc(igrid)%x(&
...
subroutine alloc_state(igrid, s, ixGmin1,ixGmin2,ixGmax1,ixGmax2, ixGextmin1,&
   ixGextmin2,ixGextmax1,ixGextmax2, alloc_once_for_ps)
  use mod_global_parameters
  type(state) :: s
  ...
  allocate(s%w(ixGmin1:ixGmax1,ixGmin2:ixGmax2,1:nw))
  s%igrid=igrid
  s%w=0.d0
  ...
  if(stagger_grid) then
    allocate(s%ws(ixGsmin1:ixGsmax1,ixGsmin2:ixGsmax2,1:nws))
    s%ws=0.d0
    if(record_electric_field) then
      allocate(s%we(ixGsmin1:ixGsmax1,ixGsmin2:ixGsmax2,1:nws))
      s%we=0.d0
    end if
  ...
  if(alloc_once_for_ps) then
    ...
    if(B0field) then
      allocate(s%B0(ixGmin1:ixGmax1,ixGmin2:ixGmax2,1:ndir,0:ndim))
      allocate(s%J0(ixGmin1:ixGmax1,ixGmin2:ixGmax2,7-2*ndir:3))
    end if
    if(number_equi_vars > 0) then
      allocate(s%equi_vars(ixGmin1:ixGmax1,ixGmin2:ixGmax2,1:number_equi_vars,&
         0:ndim))
    endif
...
subroutine alloc_state_coarse(igrid, s, ixGmin1,ixGmin2,ixGmax1,ixGmax2,&
