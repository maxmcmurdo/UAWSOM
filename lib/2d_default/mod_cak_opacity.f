...
subroutine init_cak(tablename)
  ...
  call read_table(Log_D_list, Log_T_list, alpha_vals,&
      TRIM(tablename)//"/al_TD")
  call read_table(Log_D_list, Log_T_list, Qbar_vals,&
      TRIM(tablename)//"/Qb_TD")
  call read_table(Log_D_list, Log_T_list, Q0_vals, TRIM(tablename)//"/Q0_TD")
  call read_table(Log_D_list, Log_T_list, kappa_e_vals,&
      TRIM(tablename)//"/Ke_TD")
...
subroutine set_cak_opacity(rho,temp, gradv,alpha_output, Qbar_output,&
    Q0_output, kappa_e_output)
  double precision, intent(in) :: rho, temp, gradv
  double precision, intent(out) :: alpha_output, Qbar_output, Q0_output,&
      kappa_e_output
  ...
  call get_val_comb(alpha_vals,Qbar_vals,Q0_vals,kappa_e_vals, Log_D_list,&
      Log_T_list, D_input, T_input, alpha_output, Qbar_output, Q0_output,&
      kappa_e_output)
...
subroutine read_table(D, T, K, filename)
    ...
    double precision, intent(out) :: K(Dmin:Dmax,Tmin:Tmax), D(Dmin:Dmax),&
        T(Tmin:Tmax)
...
subroutine get_val_comb(K1_vals,K2_vals,K3_vals,K4_vals, Log_D_list,&
    Log_T_list, D, T, K1, K2, K3, K4)
    ...
    elseif (D .lt. minval(Log_D_list)) then
        ...
        call get_low_up_index(D, Log_D_list, Dmin, Dmax, low_r_index,&
            up_r_index)
    endif
    ...
    elseif ( T .lt. minval(Log_T_list)) then
        ...
        call get_low_up_index(T, Log_T_list, Tmin, Tmax, low_t_index,&
            up_t_index)
    endif
...
subroutine interpolate_KRT(low_r, up_r, low_t, up_t, Log_D_list, Log_T_list,&
    Kappa_vals, D, T, k_interp)
