...
integer             :: filenr,ncells1,ncells2,ncellx1,ncellx2,jg1,jg2,jig1,&
   jig2
  ...
  if (nwauxio > 0) then
    if (.not. associated(usr_aux_output)) then
      ...
      call usr_aux_output(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1-1,ixMlo2-1,&
         ixMhi1+1,ixMhi2+1, ps(igrid)%w,ps(igrid)%x,normconv)
    end if
  end if
end do
...
if (saveprim) then
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    ...
    call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1+1,ixGlo2+1,&
       ixGhi1-1,ixGhi2-1,ps(igrid)%w,ps(igrid)%x)
    if(B0field) then
      ! add background magnetic field B0 to B
      ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_mag(:))=ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
  ...
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    ...
    if (B0field) then
      ...
      if(phys_energy) ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_e)=ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       ...
       do ig1=1,ng1(level_io)
         ...
         do ix1=ixMlo1,ixMhi1
           Master_write : if(mype==0) then
             select case(convert_type)
               case("oneblock")
                 write(qunit,fmt="(100(e14.6))") ps(igrid)%x(ix1,ix2,&
                    1:ndim)*normconv(0),(ps(igrid)%w(ix1,ix2,&
                    iwrite(iw))*normconv(iwrite(iw)),iw=1,writenw)
               case("oneblockB")
                 write(qunit) real(ps(igrid)%x(ix1,ix2,1:ndim)*normconv(0)),&
                    (real(ps(igrid)%w(ix1,ix2,&
                    iwrite(iw))*normconv(iwrite(iw))),iw=1,writenw)
             end select
           end if Master_write
         end do
...
integer             :: itag,Morton_no,igrid,ix1,ix2,iw
logical             :: fileopen
character(len=80)   :: filename
integer             :: filenr
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  ...
  if(saveprim) call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
     ixMlo2,ixMhi1,ixMhi2,ps(igrid)%w,ps(igrid)%x)
  if (mype/=0)then
      itag=Morton_no
   ...
   do ix1=ixMlo1,ixMhi1
      do iw=1,nw
        if( dabs(ps(igrid)%w(ix1,ix2,iw)) < 1.0d-32 ) ps(igrid)%w(ix1,ix2,&
           iw) = zero
      enddo
       write(qunit,fmt="(100(e14.6))") ps(igrid)%x(ix1,ix2,1:ndim),&
          ps(igrid)%w(ix1,ix2,1:nw)
   end do
   end do
  end if
end do   
 ...
 if (mype==0) then
  loop_cpu : do ipe =1, npe-1
   loop_Morton : do Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixMlo1,ixMhi1
            do iw=1,nw
              if( dabs(ps(igrid)%w(ix1,ix2,&
                 iw)) < smalldouble ) ps(igrid)%w(ix1,ix2,iw) = zero
            enddo
...
integer, allocatable :: intstatus(:,:)
integer :: itag,ipe,igrid,level,icel,ixCmin1,ixCmin2,ixCmax1,ixCmax2,ixCCmin1,&
...
'<DataArray type="Int32" Name="offsets" format="appended" offset="',offset,&
  ...
  do Morton_no=Morton_start(0),Morton_stop(0)
    ...
    do iw=1,nw+nwauxio
      ...
      if(cell_corner) then
        write(qunit) length
        write(qunit) ((wC_TMP(ix1,ix2,iw)*normconv(iw),ix1=ixCmin1,ixCmax1),&
           ix2=ixCmin2,ixCmax2)
      else
        write(qunit) lengthcc
        write(qunit) ((wCC_TMP(ix1,ix2,iw)*normconv(iw),ix1=ixCCmin1,ixCCmax1),&
           ix2=ixCCmin2,ixCCmax2)
      end if
    end do
    ...
    do ipe=1, npe-1
      do Morton_no=Morton_start(ipe),Morton_stop(ipe)
        ...
        do iw=1,nw+nwauxio
          ...
          if(cell_corner) then
            write(qunit) length
            write(qunit) ((wC_TMP(ix1,ix2,iw)*normconv(iw),ix1=ixCmin1,&
               ixCmax1),ix2=ixCmin2,ixCmax2)
          else
            write(qunit) lengthcc
            write(qunit) ((wCC_TMP(ix1,ix2,iw)*normconv(iw),ix1=ixCCmin1,&
               ixCCmax1),ix2=ixCCmin2,ixCCmax2)
          end if
        end do
...
double precision, dimension(ixMlo1-1:ixMhi1,ixMlo2-1:ixMhi2,ixMlo1-1:ixMhi1,&
   3) :: xC_TMP
...
double precision, dimension(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ixMlo1:ixMhi1,&
   nw+nwauxio)     :: wCC_TMP
double precision, dimension(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo1:ixGhi1,&
   1:nw+nwauxio)   :: w
integer::               igrid,iigrid,level,igonlevel,icel,ixCmin1,ixCmin2,&
   ixCmin3,ixCmax1,ixCmax2,ixCmax3,ixCCmin1,ixCCmin2,ixCCmin3,ixCCmax1,&
   ixCCmax2,ixCCmax3
integer::               NumGridsOnLevel(1:nlevelshi)
integer :: nx1,nx2,nx3,nxC1,nxC2,nxC3,nodesonlevel,elemsonlevel,nc,np,VTK_type,&
   ix1,ix2,ix3
double precision :: normconv(0:nw+nwauxio)
character(len=80)::  filename
...
close(qunit)
open(qunit,file=filename,form='unformatted',access='stream',status='old',&
   position='append')
buffer='_'
 ...
 !extra layer around mesh only needed when storing corner values and averaging
       if(saveprim) then
         call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,&
            ixGhi1,ixGhi2,ps(igrid)%w,ps(igrid)%x)
       endif
 ...
 !extend 2D data to 3D insuring variables are independent on the third coordinate
       ...
       do i3grid=1,n3grid !subcycles
        call calc_grid23(qunit,igrid,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
           ...
           ixCCmin3,ixCCmax1,ixCCmax2,ixCCmax3,.true.,i3grid,d3grid,w,zlength,&
           zgridsc)
        do iw=1,nw
          if(.not.w_write(iw))cycle
          select case(convert_type)
        ...
        do iw=1,nw
          ...
          if(iw==2 .or. iw==4 .or. iw==7) then
           wC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,ixCmin3:ixCmax3,&
              iw)=-wC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,ixCmin3:ixCmax3,iw)
           wCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,&
              iw)=-wCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,&
              ixCCmin3:ixCCmax3,iw)
          end if
          select case(convert_type)
            case('vtuBsym23')
              write(qunit) length
              write(qunit) (((real(wC_TMP(ix1,ix2,ix3,iw)*normconv(iw)),&
                 ix1=ixCmax1,ixCmin1,-1),ix2=ixCmin2,ixCmax2),ix3=ixCmin3,&
                 ixCmax3)
            case('vtuBCCsym23')
              write(qunit) lengthcc
              write(qunit) (((real(wCC_TMP(ix1,ix2,ix3,iw)*normconv(iw)),&
...
integer :: ixCmin1,ixCmin2,ixCmin3,ixCmax1,ixCmax2,ixCmax3,ixCCmin1,ixCCmin2,&
   ixCCmin3,ixCCmax1,ixCCmax2,ixCCmax3,nxCC1,nxCC2,nxCC3
...
do ix=ixCCmin3,ixCCmax3
    xCC(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ix,&
       3)=-zlength/two+dble(i3grid-1)*d3grid+(dble(ix-ixCCmin3)+half)*dx3
end do
...
do ix=ixCmin3,ixCmax3
    xC(ixCmin1:ixCmax1,ixCmin2:ixCmax2,ix,&
       3)=-zlength/two+dble(i3grid-1)*d3grid+dble(ix-ixCmin3)*dx3
end do
  ...
  select case(idims)
   case(1)
     ...
     do ix1=jxCmin1,jxCmax1
         w(ix1,jxCmin2:jxCmax2,jxCmin3:jxCmax3,nw-nwextra+1:nw) = w(jxCmin1-1,&
            jxCmin2:jxCmax2,jxCmin3:jxCmax3,nw-nwextra+1:nw)
     end do
     jxCmin1=ixGlo1;jxCmin2=ixGlo2;jxCmin3=ixGlo1;
     jxCmax1=ixGlo1-1+nghostcells;jxCmax2=ixGhi2;jxCmax3=ixGhi1;
     do ix1=jxCmin1,jxCmax1
         w(ix1,jxCmin2:jxCmax2,jxCmin3:jxCmax3,nw-nwextra+1:nw) = w(jxCmax1+1,&
            jxCmin2:jxCmax2,jxCmin3:jxCmax3,nw-nwextra+1:nw)
     end do
   case(2)
...
if(nwauxio>0)then
  ...
  ! maybe need for restriction to ixGlo1+1,ixGlo2+1,ixGhi1-1,ixGhi2-1 
  call specialvar_output23(ixGlo1,ixGlo2,ixGlo1,ixGhi1,ixGhi2,ixGhi1,ixGlo1+1,&
     ixGlo2+1,ixGlo1+1,ixGhi1-1,ixGhi2-1,ixGhi1-1,w,xCC,normconv)
endif
  ...
  do ix3=ixCCmin3,ixCCmax3
    do ix2=ixCCmin2,ixCCmax2
      do ix1=ixCCmin1,ixCCmax1
        wCC(ix1,ix2,ix3,iw_mag(:))=wCC(ix1,ix2,ix3,iw_mag(:))+ps(igrid)%B0(ix1,&
           ix2,:,0)
      end do
    end do
  end do
...
if(.not.saveprim .and. B0field .and. iw_e>0) then
   ...
   do ix1=ixCCmin1,ixCCmax1
       wCC(ix1,ix2,ix3,iw_e)=w(ix1,ix2,ix3,iw_e) +half*sum(ps(igrid)%B0(ix1,&
          ix2,:,0)**2 ) + sum(w(ix1,ix2,ix3,iw_mag(:))*ps(igrid)%B0(ix1,ix2,:,&
          0))
   end do
   end do
   end do
...
if(slab_uniform)then
   ...
   do iw=1,nw+nwauxio
      if (B0field.and.iw>iw_mag(1)-1.and.iw<=iw_mag(ndir)) then
         ...
         do ix2=ixCmin2,ixCmax2
         do ix1=ixCmin1,ixCmax1
           wC(ix1,ix2,ix3,iw)=sum(w(ix1:ix1+1,ix2:ix2+1,ix3,&
              iw) +ps(igrid)%B0(ix1:ix1+1,ix2:ix2+1,idir,&
              0))/dble(2**3)+sum(w(ix1:ix1+1,ix2:ix2+1,ix3+1,&
              iw) +ps(igrid)%B0(ix1:ix1+1,ix2:ix2+1,idir,0))/dble(2**3)
         end do
         end do
         end do
      else
   ...
   if(.not.saveprim .and. B0field .and. iw_e>0) then
      ...
      do ix2=ixCmin2,ixCmax2
      do ix1=ixCmin1,ixCmax1
         wC(ix1,ix2,ix3,iw_e)=sum( w(ix1:ix1+1,ix2:ix2+1,ix3,&
            iw_e) +half*sum(ps(igrid)%B0(ix1:ix1+1,ix2:ix2+1,:,0)**2,&
            dim=ndim+1) + sum( w(ix1:ix1+1,ix2:ix2+1,ix3,&
            iw_mag(:))*ps(igrid)%B0(ix1:ix1+1,ix2:ix2+1,:,0),&
            dim=ndim+1) ) /dble(2**3)+sum( w(ix1:ix1+1,ix2:ix2+1,ix3+1,&
            iw_e) +half*sum(ps(igrid)%B0(ix1:ix1+1,ix2:ix2+1,:,0)**2,&
            dim=ndim+1) + sum( w(ix1:ix1+1,ix2:ix2+1,ix3+1,&
            iw_mag(:))*ps(igrid)%B0(ix1:ix1+1,ix2:ix2+1,:,0),&
            dim=ndim+1) ) /dble(2**3)
      end do
      end do
...
xC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,ixCmin3:ixCmax3,&
   1:3)          = xC(ixCmin1:ixCmax1,ixCmin2:ixCmax2,ixCmin3:ixCmax3,1:3)
wC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,ixCmin3:ixCmax3,&
...
xCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,&
   1:3)        = xCC(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,&
   1:3)
wCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,&
   1:nw+nwauxio)  = wCC(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,&
   1:nw+nwauxio)
...
do ix1=1,nx1
        ...
        write(qunit,'(8(i7,1x))')(ix3-1)*nxC2*nxC1+(ix2-1)*nxC1+ix1-1,&
            (ix3-1)*nxC2*nxC1+(ix2-1)*nxC1+ix1,&
           (ix3-1)*nxC2*nxC1+    ix2*nxC1+ix1-1,&
           (ix3-1)*nxC2*nxC1+    ix2*nxC1+ix1,ix3*nxC2*nxC1+(ix2-1)*nxC1+ix1-1,&
           ix3*nxC2*nxC1+(ix2-1)*nxC1+ix1,ix3*nxC2*nxC1+    ix2*nxC1+ix1-1,&
...
!=============================================================================
subroutine specialvar_output23(ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,&
...
! output Te
!if(saveprim)then
 !w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,nw+1)=w(ixOmin1:ixOmax1,!   ixOmin2:ixOmax2,ixOmin3:ixOmax3,iw_e)/w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,!   ixOmin3:ixOmax3,iw_rho)
!endif
!!! store current
 !qvec(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3,1)=w(ixImin1:ixImax1,!    ixImin2:ixImax2,ixImin3:ixImax3,mag(1))
 !qvec(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3,2)=w(ixImin1:ixImax1,!    ixImin2:ixImax2,ixImin3:ixImax3,mag(2))
 !qvec(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3,3)=w(ixImin1:ixImax1,!    ixImin2:ixImax2,ixImin3:ixImax3,mag(3));
 !call curlvector3D(qvec,ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,ixOmin1,!   ixOmin2,ixOmin3,ixOmax1,ixOmax2,ixOmax3,curlvec,idirmin,1,ndir)
 !w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,nw+2)=curlvec!   (ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,1)
 !w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,nw+3)=curlvec!   (ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,2)
 !w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,nw+4)=curlvec!   (ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,3);
