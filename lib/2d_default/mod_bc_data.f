       ...
       do iw = 1, nwfluxbc
          if (typeboundary(iw, ib)==bc_data) then
             ...
             if (bc_data_time_varying) then
                ...
                lt_1d(n_bc) = LT_create_from_data(bc%origin(1), xmax(1),&
                    bc%values(:, 1, 1:1, 1))
             end if
...
111    close(unitpar)
  ...
  subroutine bc_data_set(qt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
    ...
    case (1, 2)
       do iw = 1, nwfluxbc
          ...
          tmp(ixOmin1, ixOmin2:ixOmax2) = bc_data_get_2d(n_bc, x(ixOmin1,&
              ixOmin2:ixOmax2, 2), qt)
          ...
          do i = 0, ixOmax1-ixOmin1
             w(ixOmin1+i, ixOmin2:ixOmax2, iw) = 2 * tmp(ixOmin1,&
                 ixOmin2:ixOmax2) - w(ix, ixOmin2:ixOmax2, iw)
          end do
       end do
    case (3, 4)
       do iw = 1, nwfluxbc
          ...
          do i = 0, ixOmax2-ixOmin2
             w(ixOmin1:ixOmax1, ixOmin2+i, iw) = 2 * tmp(ixOmin1:ixOmax1,&
                 ixOmin2) - w(ixOmin1:ixOmax1, ix, iw)
          end do
       end do
    case default
