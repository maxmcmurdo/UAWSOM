...
contains
      ...
      if(ipe_particle == mype) then
        ...
        do idir=1,ndir
          call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
             ixMhi1,ixMhi2,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_mom(idir)),&
             ps(igrid)%x,x(:,n),v(idir,n))
        end do
        ...
        call advect_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,&
           x(:,n),v(:,n),q(n),m(n),defpayload,ndefpayload,0.d0)
        ...
        if (associated(usr_update_payload)) then
          call usr_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,&
             x(:,n),v(:,n),q(n),m(n),usrpayload,nusrpayload,0.d0)
  ...
  subroutine advect_fill_gridvars
    ...
    double precision, dimension(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)   :: w

    do iigrid=1,igridstail; igrid=igrids(iigrid);

      dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid);

      gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ngridvars) = 0.0d0
      ...
      if(time_advance) then
        gridvars(igrid)%wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ngridvars) = 0.0d0
  ...
  subroutine advect_integrate_particles(end_time)
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      dt_p                    = advect_get_particle_dt(particle(ipart),&
          end_time)
      ...
      h1 = dt_p/2.0d0
      call odeint(x,ndir,tloc,tlocnew,eps,h1,hmin,nok,nbad,derivs_advect,rkqs,&
         ierror)
      ...
      ! Update payload
      call advect_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,x,&
         v,0.d0,0.d0,defpayload,ndefpayload,tlocnew)
      particle(ipart)%payload(1:ndefpayload) = defpayload
      if (associated(usr_update_payload)) then
        call usr_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,x,v,&
           0.d0,0.d0,usrpayload,nusrpayload,tlocnew)
      end if
  ...
  !> Payload update
  subroutine advect_update_payload(igrid,w,wold,xgrid,xpart,upart,qpart,mpart,&
    ...
    if (mynpayload > 0 ) then
      if (.not.time_advance) then
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),xgrid,xpart,&
           rho)
      else
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),xgrid,xpart,&
           rho1)
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),xgrid,xpart,&
           rho2)
  ...
  pure function advect_get_particle_dt(partp, end_time) result(dt_p)
    ...
    if(coordinate ==cylindrical.and.phi_>0) v(phi_) = &
       abs(v(phi_)/partp%self%x(r_))

    dt_cfl = min(rnode(rpdx1_,partp%igrid)/v(1),rnode(rpdx2_,partp%igrid)/v(2),&
       bigdouble)
  ...
  subroutine get_vec_advect(igrid,x,tloc,var,ibeg,iend)
    ...
    if(.not.time_advance) then
      do ivar=ibeg,iend
        iloc = ivar-ibeg+1
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ivar),&
           ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,var(iloc))
      end do
    else
      ...
      do ivar=ibeg,iend
        ...
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ...
           ivar),ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,e1(iloc))
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ivar),&
           ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,e2(iloc))
