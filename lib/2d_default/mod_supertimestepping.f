...
module mod_supertimestepping
  ...
  abstract interface
    ...
    subroutine subr1(ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,ixOmax1,&
       ixOmax2,w,x,wres,fix_conserve_at_step,my_dt,igrid,nflux)
    ...
    function subr2(w,ixGmin1,ixGmin2,ixGmax1,ixGmax2,ixmin1,ixmin2,ixmax1,&
       ixmax2,dx1,dx2,x) result(dtnew)
      use mod_global_parameters
    ...
    subroutine subr_e(w, x, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
       ixOmax1,ixOmax2, step)
      use mod_global_parameters
    ...
    subroutine subr5(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,ixOmax1,&
       ixOmax2, w, x)
      use mod_global_parameters
      integer, intent(in) :: ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
         ixOmax1,ixOmax2
  ...
  type sts_term
    ...
    integer, dimension(-1:2,-1:2,-1:1,-1:1) :: type_send_srl_sts_1,&
        type_recv_srl_sts_1
    integer, dimension(-1:1,-1:1,-1:1,-1:1) :: type_send_r_sts_1
    integer, dimension(-1:1,-1:1, 0:3,0:3) :: type_recv_r_sts_1
    integer, dimension(-1:1,-1:1, 0:3,0:3) :: type_recv_p_sts_1,&
        type_send_p_sts_1

    integer, dimension(-1:2,-1:2,-1:1,-1:1) :: type_send_srl_sts_2,&
        type_recv_srl_sts_2
    integer, dimension(-1:1,-1:1,-1:1,-1:1) :: type_send_r_sts_2
    integer, dimension(-1:1,-1:1, 0:3,0:3) :: type_recv_r_sts_2
    integer, dimension(-1:1,-1:1, 0:3,0:3) :: type_recv_p_sts_2,&
        type_send_p_sts_2
...
111    close(unitpar)
  ...
  subroutine add_sts_method(sts_getdt, sts_set_sources, startVar, nflux,&
    ...
    interface
      ...
      subroutine sts_set_sources(ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
         ixOmin2,ixOmax1,ixOmax2,w,x,wres,fix_conserve_at_step,my_dt,igrid,&
         nflux)
        use mod_global_parameters
        use mod_fix_conserve
        integer, intent(in) :: ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
           ixOmin2,ixOmax1,ixOmax2, igrid, nflux
        double precision, intent(in) ::  x(ixImin1:ixImax1,ixImin2:ixImax2,&
           1:ndim)
      ...
      function sts_getdt(w,ixGmin1,ixGmin2,ixGmax1,ixGmax2,ixmin1,ixmin2,&
         ixmax1,ixmax2,dx1,dx2,x) result(dtnew)
        use mod_global_parameters
        integer, intent(in) :: ixGmin1,ixGmin2,ixGmax1,ixGmax2, ixmin1,ixmin2,&
           ixmax1,ixmax2
        double precision, intent(in) :: dx1,dx2, x(ixGmin1:ixGmax1,&
  ...
  subroutine set_conversion_methods_to_head(sts_before_first_cycle,&
      sts_after_last_cycle)
    interface
      subroutine sts_before_first_cycle(ixImin1,ixImin2,ixImax1,ixImax2,&
      ...
      subroutine sts_after_last_cycle(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
  ...
  subroutine set_error_handling_to_head(sts_error_handling)
    interface
      subroutine sts_error_handling(w, x, ixImin1,ixImin2,ixImax1,ixImax2,&
  ...
  function set_dt_sts_ncycles(my_dt) result(dt_modified)
    ...
    do while(associated(temp))
      ...
      !$OMP PARALLEL DO PRIVATE(igrid,dx1,dx2) REDUCTION(min:dtmin_mype)
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         dtmin_mype=min(dtmin_mype, sts_dtpar * temp%sts_getdt(ps(igrid)%w,&
            ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,ixMhi1,ixMhi2,dx1,dx2,&
            ps(igrid)%x))
      end do
      !$OMP END PARALLEL DO
...
!  END FUNCTION total_chev
  ...
  ! Turlough Downes 2006,2007
    ...
    integer:: iigrid, igrid, j, ixCmin1,ixCmin2,ixCmax1,ixCmax2
    ...
    do while(associated(temp))
      ...
      if(.not.temp%evolve_magnetic_field) then
        ...
        ixCmin1=ixMlo1-1;ixCmin2=ixMlo2-1;
      end if
      ...
      if(associated(temp%sts_before_first_cycle)) then
        ...
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid);
          block=>ps(igrid)
          call temp%sts_before_first_cycle(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,&
             ixGlo2,ixGhi1,ixGhi2,ps(igrid)%w,ps(igrid)%x)  
        end do 
      end if
      ...
      do j=1,temp%s
        ...
        if(stagger_grid) then
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid);
            block=>ps(igrid)
            call temp%sts_set_sources(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
               ixMlo2,ixMhi1,ixMhi2,ps(igrid)%w,ps(igrid)%x,ps1(igrid)%w,&
               fix_conserve_at_step,dtj,igrid,temp%nflux)
            if(temp%nflux>ndir) then
              ps(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
                 temp%startVar)=ps(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
                 temp%startVar)+dtj*ps1(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
                 temp%startVar)
            end if
            ps(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               1:nws)=ps(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               1:nws)+dtj*ps1(igrid)%w(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               iw_mag(1:nws))
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            ...
            ps(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
               ...
               ixMlo2:ixMhi2,temp%startVar:temp%endVar)+&
               dtj*ps1(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
        ...
        if(fix_conserve_at_step) then
          ...
          if(stagger_grid) then
            ...
            !$OMP PARALLEL DO PRIVATE(igrid)
            do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
              call phys_face_to_center(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ps(igrid))
            end do
            !$OMP END PARALLEL DO
          end if
        end if
        if(associated(temp%sts_handle_errors)) then
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            ...
            call temp%sts_handle_errors(ps(igrid)%w,ps(igrid)%x,ixGlo1,ixGlo2,&
               ixGhi1,ixGhi2,ixMlo1,ixMlo2,ixMhi1,ixMhi2,j)
          end do
          !$OMP END PARALLEL DO
        end if
      ...
      if(associated(temp%sts_after_last_cycle)) then 
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid);
          block=>ps(igrid)
          call temp%sts_after_last_cycle(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,&
             ixGlo2,ixGhi1,ixGhi2,ps(igrid)%w,ps(igrid)%x)
        end do 
  ...
  ! Meyer 2012 MNRAS 422,2102
    ...
    double precision, allocatable :: bj(:)
    integer:: iigrid, igrid, j, ixCmin1,ixCmin2,ixCmax1,ixCmax2, ixGextmin1,&
    ...
    do while(associated(temp))
      ...
      if(associated(temp%sts_before_first_cycle)) then
        ...
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          ...
          if(.not. allocated(ps2(igrid)%w)) &
             allocate(ps2(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw))
          if(.not. allocated(ps3(igrid)%w)) &
             allocate(ps3(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw))
          if(.not. allocated(ps4(igrid)%w)) &
             allocate(ps4(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw))
          ps1(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
             1:nw)=ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)
          ps2(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
             1:nw)=ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)
        end do
        !$OMP END PARALLEL DO
      else
      ...
      if(stagger_grid) then
        ...
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          block=>ps(igrid)
          ps4(igrid)%w=zero
          call temp%sts_set_sources(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
             ixMhi1,ixMhi2,ps(igrid)%w,ps(igrid)%x,ps4(igrid)%w,&
             fix_conserve_at_step,dtj,igrid,temp%nflux)
          !!!eq solved: dU/dt = S, ps3 is stored S^n
            ps3(igrid)%w(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               temp%startVar:temp%endVar) = my_dt * &
               ps4(igrid)%w(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               temp%startVar:temp%endVar)
          if(temp%nflux>ndir) then
            ps1(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
               temp%startVar) = ps1(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
               temp%startVar) + cmut * ps3(igrid)%w(ixMlo1:ixMhi1,&
               ixMlo2:ixMhi2,temp%startVar)
          end if
          ps1(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
             1:nws) = ps1(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
             1:nws) + cmut * ps3(igrid)%w(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
             iw_mag(1:nws))
          call phys_face_to_center(ixMlo1,ixMlo2,ixMhi1,ixMhi2,ps1(igrid))
        end do
        !$OMP END PARALLEL DO
      else
        !$OMP PARALLEL DO PRIVATE(igrid)
      ...
      if(fix_conserve_at_step) then
        ...
        if(stagger_grid) then
          ...
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            call phys_face_to_center(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ps1(igrid))
          end do
          !$OMP END PARALLEL DO
        end if
      end if
      ...
      if(associated(temp%sts_handle_errors)) then
        ...
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          call temp%sts_handle_errors(ps1(igrid)%w,ps1(igrid)%x,ixGlo1,ixGlo2,&
             ixGhi1,ixGhi2,ixMlo1,ixMlo2,ixMhi1,ixMhi2,1)
        end do
        !$OMP END PARALLEL DO
      end if
      ...
      do j=2,temp%s
        ...
        if(stagger_grid) then
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            ...
            call temp%sts_set_sources(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
               ixMlo2,ixMhi1,ixMhi2,tmpPs1(igrid)%w,ps(igrid)%x,ps4(igrid)%w,&
               fix_conserve_at_step,dtj,igrid,temp%nflux)
            if(temp%nflux>ndir) then
              tmpPs2(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
                 temp%startVar)=cmu*tmpPs1(igrid)%w(ixMlo1:ixMhi1,&
                 ixMlo2:ixMhi2,temp%startVar)+&
                 cnu*tmpPs2(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
                 temp%startVar)+one_mu_nu*ps(igrid)%w(ixMlo1:ixMhi1,&
                 ixMlo2:ixMhi2,temp%startVar)+dtj*ps4(igrid)%w(ixMlo1:ixMhi1,&
                 ixMlo2:ixMhi2,temp%startVar)+cnut*ps3(igrid)%w(ixMlo1:ixMhi1,&
                 ixMlo2:ixMhi2,temp%startVar)
            end if
            tmpPs2(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               1:nws)=cmu*tmpPs1(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               1:nws)+cnu*tmpPs2(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               1:nws)+one_mu_nu*ps(igrid)%ws(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               1:nws)+dtj*ps4(igrid)%w(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
               iw_mag(1:nws))+cnut*ps3(igrid)%w(ixCmin1:ixCmax1,&
               ixCmin2:ixCmax2,iw_mag(1:nws))
            call phys_face_to_center(ixMlo1,ixMlo2,ixMhi1,ixMhi2,&
               tmpPs2(igrid))
          end do
          !$OMP END PARALLEL DO
        else
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            ...
            tmpPs2(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
               ...
               ixMlo2:ixMhi2,temp%startVar:temp%endVar)+&
               dtj*ps4(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
        ...
        if(fix_conserve_at_step) then
          ...
          if(stagger_grid) then
            ...
            !$OMP PARALLEL DO PRIVATE(igrid)
            do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
              call phys_face_to_center(ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
                 tmpPs2(igrid))
            end do
            !$OMP END PARALLEL DO
          end if
        ...
        !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            ...
            call temp%sts_handle_errors(tmpPs2(igrid)%w,ps(igrid)%x,ixGlo1,&
               ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,ixMhi1,ixMhi2,j)
          end do
        !$OMP END PARALLEL DO
        end if
        if(temp%nflux>temp%nwbc.and.temp%s==j) then
      ...
      if(associated(temp%sts_after_last_cycle)) then
        ...
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid);
          block=>ps(igrid)
          ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
             temp%startVar:temp%endVar)=tmpPs2(igrid)%w(ixGlo1:ixGhi1,&
             ixGlo2:ixGhi2,temp%startVar:temp%endVar)
          call temp%sts_after_last_cycle(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,&
             ixGlo2,ixGhi1,ixGhi2,ps(igrid)%w,ps(igrid)%x)
        end do
        !$OMP END PARALLEL DO
