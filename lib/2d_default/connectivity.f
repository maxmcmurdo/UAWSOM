...
subroutine build_connectivity
  ...
  integer :: idir,pi1,pi2, mi1,mi2, ph1,ph2, mh1,mh2, ipe_neighbor
          ...
          if (i2/=0) then
           ...
           if (neighbor_type(i1,i2,igrid)==2) then
             do idir=idim+1,ndim
               ...
               if (neighbor_type(pi1,pi2,igrid)==2.and.neighbor_type(ph1,ph2,&
                  igrid)==2.and.mype/=neighbor(2,pi1,pi2,&
                  igrid).and.neighbor_pole(pi1,pi2,igrid)==0) then
                  nsend_cc(idim) = nsend_cc(idim) + 1
               end if

               if (neighbor_type(mi1,mi2,igrid)==2.and.neighbor_type(mh1,mh2,&
                  igrid)==2.and.mype/=neighbor(2,mi1,mi2,&
           ...
           if (neighbor_type(i1,i2,igrid)==3) then
             do idir=idim+1,ndim
               ...
               if (neighbor_type(mi1,mi2,igrid)==4.and.neighbor_type(mh1,mh2,&
                  igrid)==3.and.neighbor_pole(mi1,mi2,igrid)==0) then
                ! Loop on children (several in 3D)
