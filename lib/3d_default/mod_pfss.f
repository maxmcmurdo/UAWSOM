...
contains
  ...
  subroutine harm_coef(mapname)
    ...
    if(aexist) then
      ...
      call MPI_BARRIER(icomm,ierrmpi)
      if(npe>0) call MPI_BCAST(flm,(lmax+1)*(lmax+1),MPI_DOUBLE_COMPLEX,0,&
         icomm,ierrmpi)
    else
      if(mype==0) then
        ...
        allocate(theta(ym))
        allocate(phi(xm))
        call MPI_FILE_READ(file_handle,theta,ym,MPI_DOUBLE_PRECISION,statuss,&
           ierrmpi)
        call MPI_FILE_READ(file_handle,phi,xm,MPI_DOUBLE_PRECISION,statuss,&
           ierrmpi)
        call MPI_FILE_READ(file_handle,b_r0,xm*ym,MPI_DOUBLE_PRECISION,statuss,&
           ierrmpi)
        call MPI_FILE_CLOSE(file_handle,ierrmpi)
        ...
        call MPI_FILE_OPEN(MPI_COMM_SELF,fharmcoef,amode, MPI_INFO_NULL,&
           file_handle,ierrmpi)
  ...
  subroutine pfss(ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,ixOmin1,&
     ixOmin2,ixOmin3,ixOmax1,ixOmax2,ixOmax3,Bpf,x)
    use mod_global_parameters
    ...
    double precision, intent(out) :: Bpf(ixImin1:ixImax1,ixImin2:ixImax2,&
       ixImin3:ixImax3,1:ndir)

    double complex :: Bt(0:lmax,0:lmax,ixOmin1:ixOmax1)
    double precision :: phase(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3,&
       1:ndir),Bpfiv(ixOmin3:ixOmax3,ixOmin2:ixOmax2)
    double precision :: miu(ixOmin2:ixOmax2),mius(ixOmin2:ixOmax2),xr
    integer :: l,m,ix1,ix2,ix3,j,l1,l2,ntheta,nphi,ir,qlmax
    ...
    !Calculate Br
      ...
      enddo
      call inv_sph_transform(Bt(:,:,ix1),x(ixOmin1,ixOmin2,ixOmin3:ixOmax3,3),&
         miu,mius,nphi,ntheta,Bpfiv,qlmax)
    ...
    !Calculate Btheta
      do l=0,lmax
        do m=0,l
          if (l==0) then
            Bt(l,m,ix1)=-Rlm(l+1,m)*dble(l+2)*(Alm(l+1,m)*xr**l+Blm(l+1,&
               m)*xr**(-l-3))
          else if (l>=1 .and. l<=lmax-1) then
            Bt(l,m,ix1)=Rlm(l,m)*dble(l-1)*(Alm(l-1,m)*xr**(l-2)+Blm(l-1,&
               m)*xr**(-l-1))-Rlm(l+1,m)*dble(l+2)*(Alm(l+1,m)*xr**l+Blm(l+1,&
               m)*xr**(-l-3))
          else
            Bt(l,m,ix1)=Rlm(l,m)*dble(l-1)*(Alm(l-1,m)*xr**(l-2)+Blm(l-1,&
               m)*xr**(-l-1))
      ...
      do ix3=ixOmin3,ixOmax3
        do ix2=ixOmin2,ixOmax2
          Bpf(ix1,ix2,ix3,2)=Bpfiv(ix3,ixOmax2-ix2+ixOmin2)/mius(ixOmax2-ix2+&
             ixOmin2)
        enddo
      enddo
    ...
    !Calculate Bphi
      ...
      do ix3=ixOmin3,ixOmax3
        do ix2=ixOmin2,ixOmax2
          Bpf(ix1,ix2,ix3,3)=Bpfiv(ix3,ixOmax2-ix2+ixOmin2)/mius(ixOmax2-ix2+&
             ixOmin2)
        enddo
      enddo
    enddo
  !Scalar Potential
 !Potlc(ix1,ix2,ix3)=Alm(l,m)*x(ix1,ix2,ix3,1)**l+Blm(l,m)*x(ix1,ix2,ix3,1)**(-l-1)
  ...
  subroutine inv_sph_transform(Bt,phi,miu,mius,nphi,ntheta,Bpf,qlmax)
    ...
    double precision,dimension(ntheta) :: cp_1_0,cp_l_0,cp_lm1_0,cp_lm2_0,&
       cp_m_m
    double precision,dimension(ntheta) :: cp_1_m,cp_l_m,cp_lm1_m,cp_lm2_m,&
       cp_mp1_m
    ...
    do m=1,qlmax
      ...
      if(qlmax>=m+2) then
        ...
        do l=m+2,qlmax
          ...
          c2=dsqrt((2.d0*ld+1.d0)*((ld-1.d0)**2-md**2)/(2.d0*ld-3.d0)/(ld**2-&
             md**2))
