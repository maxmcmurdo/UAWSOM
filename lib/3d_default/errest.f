...
subroutine lohner_grid(igrid)
  ...
  w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,1:nw)=ps(igrid)%w(ixGlo1:ixGhi1,&
  ...
  if(B0field) then
    if(phys_energy) w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
       ...
       iw_e)+0.5d0*sum(ps(igrid)%B0(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
       :,0)**2,dim=ndim+1) + sum(w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
       iw_mag(:))*ps(igrid)%B0(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,:,0),&
       dim=ndim+1)
        ...
        if (iflag<=nw) then
          ...
          if (logflag(iflag)) then
            tmp(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
               ...
               ixMlo3:ixMhi3))) +dabs(dlog10(tmp1(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
  ...
  do iflag=1,nw+1
     ...
     do idims=1,ndim
        ...
        if (iflag<=nw) then
          if (logflag(iflag)) then
            ...
            dref(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
               ...
               iflag))) + dabs(dlog10(ps(igrid)%w(hxmin1:hxmax1,hxmin2:hxmax2,&
               hxmin3:hxmax3,iflag)))
          else
            dp(ixmin1:ixmax1,ixmin2:ixmax2,&
          ...
          if (logflag(iflag)) then
            ...
            dref(ixmin1:ixmax1,ixmin2:ixmax2,&
               ...
               ixmin2:ixmax2,ixmin3:ixmax3))) + dabs(dlog10(tmp1(hxmin1:hxmax1,&
        ...
        numerator(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
           ...
           ixMlo3:ixMhi3)-dm(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
           ixMlo3:ixMhi3))**2.0d0
...
subroutine compare1_grid(igrid,wold,w)
  ...
  integer :: ix1,ix2,ix3, iflag, level
  double precision :: epsilon, threshold, wtol(1:nw), xtol(1:ndim)
     ...
     do iflag=1,nw+1
        ...
        if (abs(wold(ix1,ix2,ix3,iflag))<smalldouble)then
           error=error+w_refine_weight(iflag)* &
              abs(averages(iflag))/(abs(wold(ix1,ix2,ix3,iflag))+epsilon)
        else
           error=error+w_refine_weight(iflag)* &
              abs(averages(iflag))/(abs(wold(ix1,ix2,ix3,iflag)))
        end if
     end do
