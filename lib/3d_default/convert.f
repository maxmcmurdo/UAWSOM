...
integer             :: filenr,ncells1,ncells2,ncells3,ncellx1,ncellx2,ncellx3,&
   jg1,jg2,jg3,jig1,jig2,jig3
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  ...
  if((rnode(rpxmin1_,igrid)>=xprobmin1+(xprobmax1-xprobmin1)*writespshift(1,&
     ...
     igrid)<=xprobmax2-(xprobmax2-xprobmin2)*writespshift(2,&
     2).and.rnode(rpxmax3_,igrid)<=xprobmax3-(xprobmax3-&
     xprobmin3)*writespshift(3,2))) then
    writeblk(igrid)=.true.
  end if
end do
...
do iigrid=1,igridstail; igrid=igrids(iigrid)
  ...
  if (nwauxio > 0) then
    if (.not. associated(usr_aux_output)) then
      ...
      call usr_aux_output(ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,ixMlo1-1,&
         ixMlo2-1,ixMlo3-1,ixMhi1+1,ixMhi2+1,ixMhi3+1, ps(igrid)%w,ps(igrid)%x,&
         normconv)
    end if
  end if
end do
...
if (saveprim) then
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    if (.not.writeblk(igrid)) cycle
    block=>ps(igrid)
    call phys_to_primitive(ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,ixGlo1+1,&
       ixGlo2+1,ixGlo3+1,ixGhi1-1,ixGhi2-1,ixGhi3-1,ps(igrid)%w,ps(igrid)%x)
    if(B0field) then
  ...
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    ...
    if (B0field) then
      ! add background magnetic field B0 to B
      if(phys_energy) ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
         iw_e)=ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
         iw_e)+0.5d0*sum(ps(igrid)%B0(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         ixGlo3:ixGhi3,:,0)**2,dim=ndim+1) + sum(ps(igrid)%w(ixGlo1:ixGhi1,&
         ixGlo2:ixGhi2,ixGlo3:ixGhi3,iw_mag(:))*ps(igrid)%B0(ixGlo1:ixGhi1,&
         ixGlo2:ixGhi2,ixGlo3:ixGhi3,:,0),dim=ndim+1)
      ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
         iw_mag(:))=ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
         iw_mag(:))+ps(igrid)%B0(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,:,&
         0)
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  ...
  if(saveprim) call phys_to_primitive(ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,&
     ixGhi3,ixMlo1,ixMlo2,ixMlo3,ixMhi1,ixMhi2,ixMhi3,ps(igrid)%w,ps(igrid)%x)
  if (mype/=0)then
      itag=Morton_no
   ...
   do ix1=ixMlo1,ixMhi1
      do iw=1,nw
        if( dabs(ps(igrid)%w(ix1,ix2,ix3,iw)) < 1.0d-32 ) ps(igrid)%w(ix1,ix2,&
           ix3,iw) = zero
      enddo
...
integer :: nx1,nx2,nx3,nxC1,nxC2,nxC3,nodesonlevel,elemsonlevel,ixCmin1,&
   ixCmin2,ixCmin3,ixCmax1,ixCmax2,ixCmax3,ixCCmin1,ixCCmin2,ixCCmin3,ixCCmax1,&
   ixCCmax2,ixCCmax3
...
do ix1=1,nx1
   ...
   write(qunit,'(8(i7,1x))') nodenumbertec3D(ix1,  ix2-1,ix3-1,nxC1,nxC2,nxC3,&
      igonlevel,igrid),nodenumbertec3D(ix1+1,ix2-1,ix3-1,nxC1,nxC2,nxC3,&
      igonlevel,igrid),nodenumbertec3D(ix1+1,ix2  ,ix3-1,nxC1,nxC2,nxC3,&
      igonlevel,igrid),nodenumbertec3D(ix1  ,ix2  ,ix3-1,nxC1,nxC2,nxC3,&
      igonlevel,igrid),nodenumbertec3D(ix1  ,ix2-1,ix3  ,nxC1,nxC2,nxC3,&
      igonlevel,igrid),nodenumbertec3D(ix1+1,ix2-1,ix3  ,nxC1,nxC2,nxC3,&
      igonlevel,igrid),nodenumbertec3D(ix1+1,ix2  ,ix3  ,nxC1,nxC2,nxC3,&
      igonlevel,igrid),nodenumbertec3D(ix1  ,ix2  ,ix3  ,nxC1,nxC2,nxC3,&
      igonlevel,igrid)
...
integer, allocatable :: intstatus(:,:)
integer :: itag,ipe,igrid,level,icel,ixCmin1,ixCmin2,ixCmin3,ixCmax1,ixCmax2,&
...
if (mype /= 0) then
  do Morton_no=Morton_start(mype),Morton_stop(mype)
    ...
    call calc_grid(qunit,igrid,xC,xCC,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
       ...
       ixCCmin3,ixCCmax1,ixCCmax2,ixCCmax3,.true.)
    itag=Morton_no
...
'<DataArray type="Int32" Name="offsets" format="appended" offset="',offset,&
  ...
  do Morton_no=Morton_start(0),Morton_stop(0)
    ...
    do iw=1,nw+nwauxio
      ...
      if(cell_corner) then
        write(qunit) length
        write(qunit) (((real(wC_TMP(ix1,ix2,ix3,iw)*normconv(iw)),ix1=ixCmin1,&
...
integer :: ixrvCmin1,ixrvCmin2,ixrvCmin3,ixrvCmax1,ixrvCmax2,ixrvCmax3,&
    ixrvCCmin1,ixrvCCmin2,ixrvCCmin3,ixrvCCmax1,ixrvCCmax2,ixrvCCmax3, siz_ind,&
    ind_send(5*3), ind_recv(5*3)
...
character(len=1024) :: outfilehead
integer             :: nx1,nx2,nx3,nxC1,nxC2,nxC3,nc,np, igrid,ixCmin1,ixCmin2,&
   ixCmin3,ixCmax1,ixCmax2,ixCmax3,ixCCmin1,ixCCmin2,ixCCmin3,ixCCmax1,&
   ixCCmax2,ixCCmax3,level,Morton_no
   ...
   do level=levmin_recv,levmax_recv
    ...
    do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
     ...
     call write_vtk(qunit,ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,ixrvCmin1,&
        ...
        np,nx1,nx2,nx3,nxC1,nxC2,nxC3,normconv,wnamei,xC_TMP_recv,xCC_TMP_recv,&
        wC_TMP_recv,wCC_TMP_recv)
...
integer, intent(in) :: qunit
integer, intent(in) :: ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,ixCmin1,&
   ixCmin2,ixCmin3,ixCmax1,ixCmax2,ixCmax3,ixCCmin1,ixCCmin2,ixCCmin3,ixCCmax1,&
   ixCCmax2,ixCCmax3
