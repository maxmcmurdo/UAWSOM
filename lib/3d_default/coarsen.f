...
subroutine coarsen_grid(sFi,ixFiGmin1,ixFiGmin2,ixFiGmin3,ixFiGmax1,ixFiGmax2,&
   ixFiGmax3,ixFimin1,ixFimin2,ixFimin3,ixFimax1,ixFimax2,ixFimax3,sCo,&
  ...
  if(stagger_grid) then
    do iw=1,nws
      ...
      do ixCo1 = ixComin1-kr(1,iw),ixComax1
         ...
         if (sCo%surfaceC(ixCo1,ixCo2,ixCo3,iw)>1.0d-9*sCo%dvolume(ixCo1,ixCo2,&
            ixCo3)) then !Normal case
           wCos(ixCo1,ixCo2,ixCo3,iw)=sum(sFi%surfaceC(ixFi1:ixFi1+1-kr(iw,1),&
              ixFi2:ixFi2+1-kr(iw,2),ixFi3:ixFi3+1-kr(iw,3),&
              iw)*wFis(ixFi1:ixFi1+1-kr(iw,1),ixFi2:ixFi2+1-kr(iw,2),&
              ixFi3:ixFi3+1-kr(iw,3),iw)) /sCo%surfaceC(ixCo1,ixCo2,ixCo3,iw)
         else ! On axis
    ...
    if(phys_total_energy.and. .not.coarsenprimitive) then
      B_energy_change(ixComin1:ixComax1,ixComin2:ixComax2,&
         ixComin3:ixComax3)=0.5d0*sum(wCo(ixComin1:ixComax1,ixComin2:ixComax2,&
         ixComin3:ixComax3,iw_mag(:))**2,dim=ndim+1)
    end if
