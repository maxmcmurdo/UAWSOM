...
contains
  ...
  subroutine mhd_diffuse_hllcd(ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,&
  ...
  ! We also compute here the HLL flux and w value, for fallback strategy
    ...
    logical         , dimension(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       ixOmin3:ixOmax3)     :: Cond_patchf
    ...
    where(Cond_patchf(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3))
      lambdaCD(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
         ...
         mom(idim))/whll(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,rho_)
    end where
    ...
    do ix1=ixOmin1,ixOmax1
      if(Cond_patchf(ix1,ix2,ix3)) then
        ...
        if(cmin(ix1,ix2,ix3)<zero.and.lambdaCD(ix1,ix2,&
          ...
          patchf(ix1,ix2,ix3) =  1
        else if(lambdaCD(ix1,ix2,ix3)>=cmax(ix1,ix2,ix3).or.lambdaCD(ix1,ix2,&
           ix3) <= cmin(ix1,ix2,ix3)) then
      ...
      if(abs(patchf(ix1,ix2,ix3))==1) then
        ...
        do n=1,mhd_n_tracer
          ...
          wCD(ix1,ix2,ix3,iw) = wSub(ix1,ix2,ix3,iw)*(cspeed(ix1,ix2,&
             ix3)-vSub(ix1,ix2,ix3))/(cspeed(ix1,ix2,ix3)-lambdaCD(ix1,ix2,&
             ix3))
        end do
        !==== Magnetic field ====!
        do idir=1,ndir
        ...
        do iw=1, ndir
          if(iw /= idim)then
            ...
            wCD(ix1,ix2,ix3,mom(iw))=(cspeed(ix1,ix2,ix3)*wSub(ix1,ix2,ix3,&
               mom(iw))-fSub(ix1,ix2,ix3,mom(iw))-wCD(ix1,ix2,ix3,&
        ...
        enddo
        if(mhd_energy) then
          VdotBCD(ix1,ix2,ix3) = sum(whll(ix1,ix2,ix3,mom(:))*whll(ix1,ix2,ix3,&
             mag(:)))/whll(ix1,ix2,ix3,rho_)
          ! Eq 17
          pCD(ix1,ix2,ix3)  = wsub(ix1,ix2,ix3,rho_)*(cspeed(ix1,ix2,&
             ...
             mom(idim))*vSub(ix1,ix2,ix3)+ wCD(ix1,ix2,ix3,mag(idim))**2
          ! Eq 31
