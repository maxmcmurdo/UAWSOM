...
contains
  ...
  subroutine add_split_source(prior)
    ...
    case (sourcesplit_sfs)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         call addsource2(0.5d0*dt,ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,&
            ixMlo1,ixMlo2,ixMlo3,ixMhi1,ixMhi2,ixMhi3,1,nw,qt,ps(igrid)%w,qt,&
            ps(igrid)%w,ps(igrid)%x,.true.,src_active)
      end do
      !$OMP END PARALLEL DO
    case (sourcesplit_sf)
      !$OMP PARALLEL DO PRIVATE(igrid)
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid)
         dxlevel(3)=rnode(rpdx3_,igrid);
         call addsource2(dt  ,ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,ixMlo1,&
            ixMlo2,ixMlo3,ixMhi1,ixMhi2,ixMhi3,1,nw,qt,ps(igrid)%w,qt,&
            ps(igrid)%w,ps(igrid)%x,.true.,src_active)
      end do
      !$OMP END PARALLEL DO
    case (sourcesplit_ssf)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         dxlevel(3)=rnode(rpdx3_,igrid);
         call addsource2(0.5d0*dt,ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,&
            ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,1,nw,qt,ps(igrid)%w,qt,&
            ps(igrid)%w,ps(igrid)%x,.true.,src_active)
         call addsource2(dt  ,ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,ixMlo1,&
            ixMlo2,ixMlo3,ixMhi1,ixMhi2,ixMhi3,1,nw,qt,ps(igrid)%w,qt,&
            ps(igrid)%w,ps(igrid)%x,.true.,src_active)
    ...
    case (sourcesplit_ssfss)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         dxlevel(1)=rnode(rpdx1_,igrid);dxlevel(2)=rnode(rpdx2_,igrid)
         dxlevel(3)=rnode(rpdx3_,igrid);
         call addsource2(0.25d0*dt,ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,&
            ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,1,nw,qt,ps(igrid)%w,qt,&
            ps(igrid)%w,ps(igrid)%x,.true.,src_active)
         call addsource2(0.5d0*dt,ixGlo1,ixGlo2,ixGlo3,ixGhi1,ixGhi2,ixGhi3,&
            ixMlo1,ixMlo2,ixMlo3,ixMhi1,ixMhi2,ixMhi3,1,nw,qt,ps(igrid)%w,qt,&
  ...
  subroutine addsource2(qdt,ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,&
     ...
     x,qsourcesplit,src_active,wCTprim)
    use mod_global_parameters
    ...
    call phys_add_source(qdt,ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,&
