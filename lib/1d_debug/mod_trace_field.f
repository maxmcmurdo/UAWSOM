...
contains
  ...
  subroutine find_next_grid(igrid,igrid_next,ipe_next,xf1,newpe,stopT)
    ...
    double precision :: dxb1,xbmin1,xbmax1,xbmid1
    integer :: idn1,my_neighbor_type,inblock
    integer :: ic1,inc1,ipe_neighbor,igrid_neighbor
    double precision :: xbnmin1,xbnmax1
    ...
    my_neighbor_type=neighbor_type(idn1,igrid)
    igrid_neighbor=neighbor(1,idn1,igrid)
    ipe_neighbor=neighbor(2,idn1,igrid)
    ...
    case(neighbor_fine)
      ...
      ipe_next=neighbor_child(2,inc1,igrid)
      igrid_next=neighbor_child(1,inc1,igrid)
      if (mype==ipe_next) then
        newpe=.FALSE.
      else
        newpe=.TRUE.
  ...
  subroutine find_points_interp(igrid,ip_in,ip_out,xf,wP,wL,numP,nwP,nwL,dL,&
    ...
    integer          :: ip,inblock,ixImin1,ixImax1,ixOmin1,ixOmax1,j
    double precision :: field(ixglo1:ixghi1,ndir)
    ...
    MAINLOOP: do ip=ip_in,numP-1
      ...
      xs1(:)=xf(ip,:)
      call get_K(xs1,igrid,K1,ixImin1,ixImax1,dxb1,ftype)
      if (forward) then
        xs2(:)=xf(ip,:)+ds*K1(:)
      else
        xs2(:)=xf(ip,:)-ds*K1(:)
      endif
      call get_K(xs2,igrid,K2,ixImin1,ixImax1,dxb1,ftype)
      if (forward) then
      ...
      if (tcondi/='TRAC') then
        ...
        if (ip==ip_in) then
          ...
          xfnow(:)=xf(ip,:)
          xfnext(:)=xf(ip+1,:)
          call get_T_loc_TRAC(igrid,xfpre,Tpre,ixImin1,ixImax1,dxb1)
          call get_T_loc_TRAC(igrid,xfnow,Tnow,ixImin1,ixImax1,dxb1)
          call get_T_loc_TRAC(igrid,xfnext,Tnext,ixImin1,ixImax1,dxb1)
        else
          xfpre=xf(ip-1,:)
          xfnow(:)=xf(ip,:)
          xfnext(:)=xf(ip+1,:)
  ...
  subroutine get_K(xfn,igrid,K,ixImin1,ixImax1,dxb1,ftype)
    use mod_usr_methods

    integer :: ixImin1,ixImax1,igrid
    double precision :: dxb1
    double precision :: xfn(ndim),K(ndim)
    character(len=std_len) :: ftype

    integer          :: ixb1,ix1,ixbl1,ixOmin1,ixOmax1,j
    double precision :: xd1
    double precision :: field(0:1,ndir),Fx(ndim),factor(0:1)
    double precision :: vector(ixImin1:ixImax1,1:ndir)
    double precision :: Ftotal

    ixbl1=floor((xfn(1)-ps(igrid)%x(ixImin1,1))/dxb1)+ixImin1;
    xd1=(xfn(1)-ps(igrid)%x(ixbl1,1))/dxb1;
    ixOmin1=ixbl1;
    ixOmax1=ixbl1+1;
    vector=zero
    ...
    if (ftype=='Bfield') then
      if (B0field) then
        vector(ixOmin1:ixOmax1,1:ndir)=ps(igrid)%w(ixOmin1:ixOmax1,&
           iw_mag(1:ndir))+ps(igrid)%B0(ixOmin1:ixOmax1,1:ndir,0)
      else
        vector(ixOmin1:ixOmax1,1:ndir)=ps(igrid)%w(ixOmin1:ixOmax1,&
           iw_mag(1:ndir))
      endif
    else if (ftype=='Vfield') then
      call phys_get_v(ps(igrid)%w,ps(igrid)%x,ixImin1,ixImax1,ixOmin1,ixOmax1,&
         vector)
    endif
    do ix1=0,1
      factor(ix1)=abs(1-ix1-xd1)
      field(ix1,1:ndir)=vector(ixbl1+ix1,1:ndir)
    enddo    

    Fx=0.d0
    do ix1=0,1
      do j=1,ndim
        Fx(j)=Fx(j)+field(ix1,j)*factor(ix1)
      enddo
    enddo
  ...
  subroutine get_T_loc_TRAC(igrid,xloc,Tloc,ixImin1,ixImax1,dxb1)
    ! grid T has been calculated and stored in wextra(ixI^S,Tcoff_)
    ! see mod_trac
    integer, intent(in) :: igrid,ixImin1,ixImax1
    double precision, intent(inout) :: xloc(ndim)
    double precision, intent(inout) :: Tloc
    double precision, intent(in) :: dxb1

    integer          :: ixb1,ix1,ixbl1,j,ixOmin1,ixOmax1
    double precision :: xd1
    double precision :: factor(0:1),Tnear(0:1)

    ixbl1=floor((xloc(1)-ps(igrid)%x(ixImin1,1))/dxb1)+ixImin1;
    xd1=(xloc(1)-ps(igrid)%x(ixbl1,1))/dxb1;
    ixOmin1=ixbl1;
    ixOmax1=ixOmin1+1;

    do ix1=0,1
      factor(ix1)=abs(1-ix1-xd1)
      Tnear(ix1)=ps(igrid)%wextra(ixbl1+ix1,iw_tcoff)
    enddo

    Tloc=0.d0
    ! interpolation
    do ix1=0,1
      Tloc=Tloc+Tnear(ix1)*factor(ix1)
    enddo
