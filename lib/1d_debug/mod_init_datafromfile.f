...
contains
  ...
  subroutine read_data_init()
    ...
    if(usr_filename/='')then
       ...
       do ivar = 1,bc%n_variables
          ...
          lt_1d(ivar) = LT_create_from_data(bc%origin(1), xmax(1), bc%values(:,&
              1, 1:1, ivar))
  ...
  subroutine read_data_set(ixImin1,ixImax1,ixOmin1,ixOmax1,x,val,nval)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1, nval
    double precision, intent(in)    :: x(ixImin1:ixImax1,1:ndim)
    double precision, intent(out)   :: val(ixImin1:ixImax1,1:nval)
    integer                         :: ival
