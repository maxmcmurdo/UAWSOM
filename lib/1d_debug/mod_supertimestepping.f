...
module mod_supertimestepping
  ...
  abstract interface
    ...
    subroutine subr1(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x,wres,&
    ...
    function subr2(w,ixGmin1,ixGmax1,ixmin1,ixmax1,dx1,x) result(dtnew)
      use mod_global_parameters
    ...
    subroutine subr_e(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, step)
      use mod_global_parameters
    ...
    subroutine subr5(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
      use mod_global_parameters
      integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
      double precision, intent(in) :: x(ixImin1:ixImax1,1:ndim)
      double precision, intent(inout) :: w(ixImin1:ixImax1,1:nw) 
  ...
  type sts_term
    ...
    integer, dimension(-1:2,-1:1) :: type_send_srl_sts_1, type_recv_srl_sts_1
    integer, dimension(-1:1,-1:1) :: type_send_r_sts_1
    integer, dimension(-1:1, 0:3) :: type_recv_r_sts_1
    integer, dimension(-1:1, 0:3) :: type_recv_p_sts_1, type_send_p_sts_1

    integer, dimension(-1:2,-1:1) :: type_send_srl_sts_2, type_recv_srl_sts_2
    integer, dimension(-1:1,-1:1) :: type_send_r_sts_2
    integer, dimension(-1:1, 0:3) :: type_recv_r_sts_2
    integer, dimension(-1:1, 0:3) :: type_recv_p_sts_2, type_send_p_sts_2
...
111    close(unitpar)
  ...
  subroutine add_sts_method(sts_getdt, sts_set_sources, startVar, nflux,&
    ...
    interface
      ...
      subroutine sts_set_sources(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x,wres,&
         fix_conserve_at_step,my_dt,igrid,nflux)
        use mod_global_parameters
      ...
      function sts_getdt(w,ixGmin1,ixGmax1,ixmin1,ixmax1,dx1,x) result(dtnew)
        use mod_global_parameters
        integer, intent(in) :: ixGmin1,ixGmax1, ixmin1,ixmax1
        double precision, intent(in) :: dx1, x(ixGmin1:ixGmax1,1:ndim)
        double precision, intent(in) :: w(ixGmin1:ixGmax1,1:nw) 
  ...
  subroutine set_conversion_methods_to_head(sts_before_first_cycle,&
      sts_after_last_cycle)
    interface
      subroutine sts_before_first_cycle(ixImin1,ixImax1, ixOmin1,ixOmax1, w,&
          x)
        use mod_global_parameters
        integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
        double precision, intent(in) :: x(ixImin1:ixImax1,1:ndim)
      ...
      subroutine sts_after_last_cycle(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
        use mod_global_parameters
        integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
        double precision, intent(in) :: x(ixImin1:ixImax1,1:ndim)
        double precision, intent(inout) :: w(ixImin1:ixImax1,1:nw) 
  ...
  subroutine set_error_handling_to_head(sts_error_handling)
    interface
      subroutine sts_error_handling(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
          step)
        use mod_global_parameters
        use mod_small_values
        integer, intent(in)             :: ixImin1,ixImax1,ixOmin1,ixOmax1
  ...
  function set_dt_sts_ncycles(my_dt) result(dt_modified)
    ...
    do while(associated(temp))
      ...
      !$OMP PARALLEL DO PRIVATE(igrid,dx1) REDUCTION(min:dtmin_mype)
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         dtmin_mype=min(dtmin_mype, sts_dtpar * temp%sts_getdt(ps(igrid)%w,&
            ixGlo1,ixGhi1,ixMlo1,ixMhi1,dx1,ps(igrid)%x))
      end do
      !$OMP END PARALLEL DO
...
!  END FUNCTION total_chev
  ...
  ! Turlough Downes 2006,2007
    ...
    integer:: iigrid, igrid, j, ixCmin1,ixCmax1
    ...
    do while(associated(temp))
      ...
      if(associated(temp%sts_before_first_cycle)) then
        ...
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          dxlevel(1)=rnode(rpdx1_,igrid);
          block=>ps(igrid)
          call temp%sts_before_first_cycle(ixGlo1,ixGhi1,ixGlo1,ixGhi1,&
      ...
      do j=1,temp%s
        ...
        if(stagger_grid) then
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            dxlevel(1)=rnode(rpdx1_,igrid);
            block=>ps(igrid)
            call temp%sts_set_sources(ixGlo1,ixGhi1,ixMlo1,ixMhi1,ps(igrid)%w,&
               ps(igrid)%x,ps1(igrid)%w,fix_conserve_at_step,dtj,igrid,&
               temp%nflux)
            if(temp%nflux>ndir) then
              ps(igrid)%w(ixMlo1:ixMhi1,temp%startVar)=ps(igrid)%w(&
                 ixMlo1:ixMhi1,temp%startVar)+dtj*ps1(igrid)%w(ixMlo1:ixMhi1,&
                 temp%startVar)
            end if
            ps(igrid)%ws(ixCmin1:ixCmax1,1:nws)=ps(igrid)%ws(ixCmin1:ixCmax1,&
               1:nws)+dtj*ps1(igrid)%w(ixCmin1:ixCmax1,iw_mag(1:nws))
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            ...
            call temp%sts_set_sources(ixGlo1,ixGhi1,ixMlo1,ixMhi1,ps(igrid)%w,&
               ps(igrid)%x,ps1(igrid)%w,fix_conserve_at_step,dtj,igrid,&
               temp%nflux)
            ps(igrid)%w(ixMlo1:ixMhi1,temp%startVar:temp%endVar)=ps(igrid)%w(&
               ixMlo1:ixMhi1,temp%startVar:temp%endVar)+&
               dtj*ps1(igrid)%w(ixMlo1:ixMhi1,temp%startVar:temp%endVar)
          end do
          !$OMP END PARALLEL DO
        end if
        ...
        if(fix_conserve_at_step) then
          ...
          if(stagger_grid) then
            ...
            !$OMP PARALLEL DO PRIVATE(igrid)
            do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
              call phys_face_to_center(ixGlo1,ixGhi1,ps(igrid))
            end do
            !$OMP END PARALLEL DO
          end if
        end if
        if(associated(temp%sts_handle_errors)) then
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            dxlevel(1)=rnode(rpdx1_,igrid);
            block=>ps(igrid)
            call temp%sts_handle_errors(ps(igrid)%w,ps(igrid)%x,ixGlo1,ixGhi1,&
               ixMlo1,ixMhi1,j)
          end do
          !$OMP END PARALLEL DO
        end if
      ...
      if(associated(temp%sts_after_last_cycle)) then 
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          dxlevel(1)=rnode(rpdx1_,igrid);
          block=>ps(igrid)
          call temp%sts_after_last_cycle(ixGlo1,ixGhi1,ixGlo1,ixGhi1,&
             ps(igrid)%w,ps(igrid)%x)
        end do 
  ...
  ! Meyer 2012 MNRAS 422,2102
    ...
    double precision, allocatable :: bj(:)
    integer:: iigrid, igrid, j, ixCmin1,ixCmax1, ixGextmin1,ixGextmax1
    ...
    do while(associated(temp))
      ...
      if(associated(temp%sts_before_first_cycle)) then
        ...
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          ...
          if(.not. allocated(ps2(igrid)%w)) &
             allocate(ps2(igrid)%w(ixGlo1:ixGhi1,1:nw))
          if(.not. allocated(ps3(igrid)%w)) &
             allocate(ps3(igrid)%w(ixGlo1:ixGhi1,1:nw))
          if(.not. allocated(ps4(igrid)%w)) &
             allocate(ps4(igrid)%w(ixGlo1:ixGhi1,1:nw))
          ps1(igrid)%w(ixGlo1:ixGhi1,1:nw)=ps(igrid)%w(ixGlo1:ixGhi1,1:nw)
          ps2(igrid)%w(ixGlo1:ixGhi1,1:nw)=ps(igrid)%w(ixGlo1:ixGhi1,1:nw)
        end do
        !$OMP END PARALLEL DO
      else
        if(stagger_grid) then
          ...
          do iigrid=1,igridstail; igrid=igrids(iigrid);
            if(.not. allocated(ps2(igrid)%w)) then
              call alloc_state(igrid, ps2(igrid), ixGlo1,ixGhi1, ixGextmin1,&
                 ixGextmax1, .false.)
            end if
            if(.not. allocated(ps3(igrid)%w)) &
               allocate(ps3(igrid)%w(ixGlo1:ixGhi1,1:nw))
      ...
      if(stagger_grid) then
        ...
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          ps4(igrid)%w=zero
          call temp%sts_set_sources(ixGlo1,ixGhi1,ixMlo1,ixMhi1,ps(igrid)%w,&
             ps(igrid)%x,ps4(igrid)%w,fix_conserve_at_step,dtj,igrid,&
             temp%nflux)
          !!!eq solved: dU/dt = S, ps3 is stored S^n
            ps3(igrid)%w(ixCmin1:ixCmax1,&
               temp%startVar:temp%endVar) = my_dt * &
               ps4(igrid)%w(ixCmin1:ixCmax1,temp%startVar:temp%endVar)
          if(temp%nflux>ndir) then
            ps1(igrid)%w(ixMlo1:ixMhi1,temp%startVar) = &
               ps1(igrid)%w(ixMlo1:ixMhi1,&
               temp%startVar) + cmut * ps3(igrid)%w(ixMlo1:ixMhi1,&
               temp%startVar)
          end if
          ps1(igrid)%ws(ixCmin1:ixCmax1,1:nws) = ps1(igrid)%ws(ixCmin1:ixCmax1,&
             1:nws) + cmut * ps3(igrid)%w(ixCmin1:ixCmax1,iw_mag(1:nws))
          call phys_face_to_center(ixMlo1,ixMhi1,ps1(igrid))
        end do
        !$OMP END PARALLEL DO
      else
        !$OMP PARALLEL DO PRIVATE(igrid)
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          call temp%sts_set_sources(ixGlo1,ixGhi1,ixMlo1,ixMhi1,ps(igrid)%w,&
             ...
             temp%nflux)
          !!!eq solved: dU/dt = S, ps3 is stored S^n
          ps3(igrid)%w(ixMlo1:ixMhi1,temp%startVar:temp%endVar) = my_dt * &
             ps4(igrid)%w(ixMlo1:ixMhi1,temp%startVar:temp%endVar)
          ps1(igrid)%w(ixMlo1:ixMhi1,temp%startVar:temp%endVar) = &
             ps1(igrid)%w(ixMlo1:ixMhi1,temp%startVar:temp%endVar) + cmut * &
             ps3(igrid)%w(ixMlo1:ixMhi1,temp%startVar:temp%endVar)
        end do
        !$OMP END PARALLEL DO
      end if
      if(fix_conserve_at_step) then
        ...
        if(stagger_grid) then
          ...
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            call phys_face_to_center(ixGlo1,ixGhi1,ps1(igrid))
          end do
          !$OMP END PARALLEL DO
        end if
      end if
      ...
      if(associated(temp%sts_handle_errors)) then
        ...
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          dxlevel(1)=rnode(rpdx1_,igrid);
          block=>ps(igrid)
          call temp%sts_handle_errors(ps1(igrid)%w,ps1(igrid)%x,ixGlo1,ixGhi1,&
             ixMlo1,ixMhi1,1)
        end do
        !$OMP END PARALLEL DO
      end if
      ...
      do j=2,temp%s
        ...
        if(stagger_grid) then
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
            ...
            call temp%sts_set_sources(ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
               tmpPs1(igrid)%w,ps(igrid)%x,ps4(igrid)%w,fix_conserve_at_step,&
               dtj,igrid,temp%nflux)
            if(temp%nflux>ndir) then
              tmpPs2(igrid)%w(ixMlo1:ixMhi1,&
                 ...
                 temp%startVar)+dtj*ps4(igrid)%w(ixMlo1:ixMhi1,&
                 temp%startVar)+cnut*ps3(igrid)%w(ixMlo1:ixMhi1,temp%startVar)
            end if
            tmpPs2(igrid)%ws(ixCmin1:ixCmax1,&
               1:nws)=cmu*tmpPs1(igrid)%ws(ixCmin1:ixCmax1,&
               1:nws)+cnu*tmpPs2(igrid)%ws(ixCmin1:ixCmax1,&
               1:nws)+one_mu_nu*ps(igrid)%ws(ixCmin1:ixCmax1,&
               1:nws)+dtj*ps4(igrid)%w(ixCmin1:ixCmax1,&
               iw_mag(1:nws))+cnut*ps3(igrid)%w(ixCmin1:ixCmax1,iw_mag(1:nws))
            call phys_face_to_center(ixMlo1,ixMhi1,tmpPs2(igrid))
          end do
          !$OMP END PARALLEL DO
        else
          !$OMP PARALLEL DO PRIVATE(igrid)
        ...
        if(fix_conserve_at_step) then
          ...
          if(stagger_grid) then
            ...
            !$OMP PARALLEL DO PRIVATE(igrid)
            do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
              call phys_face_to_center(ixGlo1,ixGhi1,tmpPs2(igrid))
            end do
            !$OMP END PARALLEL DO
          end if
        end if
      ...
      if(associated(temp%sts_after_last_cycle)) then
        ...
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          dxlevel(1)=rnode(rpdx1_,igrid);
          block=>ps(igrid)
          ps(igrid)%w(ixGlo1:ixGhi1,temp%startVar:temp%endVar)=tmpPs2(igrid)%w(&
             ixGlo1:ixGhi1,temp%startVar:temp%endVar)
          call temp%sts_after_last_cycle(ixGlo1,ixGhi1,ixGlo1,ixGhi1,&
             ps(igrid)%w,ps(igrid)%x)
        end do
        !$OMP END PARALLEL DO
        ...
        if(stagger_grid) then
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail; igrid=igrids(iigrid);
            ps(igrid)%w(ixGlo1:ixGhi1,temp%startVar:temp%endVar)=tmpPs2(&
               igrid)%w(ixGlo1:ixGhi1,temp%startVar:temp%endVar)
