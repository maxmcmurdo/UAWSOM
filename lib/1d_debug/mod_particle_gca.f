...
contains
  ...
  subroutine gca_fill_gridvars
    ...
    double precision                          :: current(ixGlo1:ixGhi1,&
       7-2*ndir:3)
    integer                                   :: idirmin
    double precision, dimension(ixGlo1:ixGhi1,1:ndir) :: vE, bhat
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      ! fill with velocity:
      gridvars(igrid)%w(ixGlo1:ixGhi1,vp(:)) = w(ixGlo1:ixGhi1,iw_mom(:))

      ! grad(kappa B)
      absB(ixGlo1:ixGhi1) = sqrt(sum(gridvars(igrid)%w(ixGlo1:ixGhi1,bp(:))**2,&
         dim=ndim+1))
      vE(ixGlo1:ixGhi1,1) = gridvars(igrid)%w(ixGlo1:ixGhi1,&
         ep(2)) * gridvars(igrid)%w(ixGlo1:ixGhi1,&
         bp(3)) - gridvars(igrid)%w(ixGlo1:ixGhi1,&
         ep(3)) * gridvars(igrid)%w(ixGlo1:ixGhi1,bp(2))
      vE(ixGlo1:ixGhi1,2) = gridvars(igrid)%w(ixGlo1:ixGhi1,&
         ep(3)) * gridvars(igrid)%w(ixGlo1:ixGhi1,&
         bp(1)) - gridvars(igrid)%w(ixGlo1:ixGhi1,&
         ep(1)) * gridvars(igrid)%w(ixGlo1:ixGhi1,bp(3))
      vE(ixGlo1:ixGhi1,3) = gridvars(igrid)%w(ixGlo1:ixGhi1,&
         ep(1)) * gridvars(igrid)%w(ixGlo1:ixGhi1,&
         bp(2)) - gridvars(igrid)%w(ixGlo1:ixGhi1,&
         ep(2)) * gridvars(igrid)%w(ixGlo1:ixGhi1,bp(1))
      do idir=1,ndir
        where (absB(ixGlo1:ixGhi1) .gt. 0.d0)
          vE(ixGlo1:ixGhi1,idir) = vE(ixGlo1:ixGhi1,&
             idir) / absB(ixGlo1:ixGhi1)**2
        elsewhere
          vE(ixGlo1:ixGhi1,idir) = 0.d0
        end where
      end do
      if (any(sum(vE(ixGlo1:ixGhi1,:)**2,&
      ...
      if (relativistic) then
        kappa(ixGlo1:ixGhi1) = 1.d0/sqrt(1.0d0 - sum(vE(ixGlo1:ixGhi1,:)**2,&
           dim=ndim+1)/c_norm**2)
      else
        kappa(ixGlo1:ixGhi1) = 1.d0
      end if
      kappa_B(ixGlo1:ixGhi1) = absB(ixGlo1:ixGhi1) / kappa(ixGlo1:ixGhi1)
      ...
      do idim=1,ndim
        ...
        gridvars(igrid)%w(ixGlo1:ixGhi1,grad_kappa_B(idim)) = &
           tmp(ixGlo1:ixGhi1)
      end do
      ...
      do idir=1,ndir
        ...
        do idim=1,ndim
          ...
          gridvars(igrid)%w(ixGlo1:ixGhi1,&
             ...
             idim) * tmp(ixGlo1:ixGhi1)
          gridvars(igrid)%w(ixGlo1:ixGhi1,&
             vE_dot_grad_b(idir)) = gridvars(igrid)%w(ixGlo1:ixGhi1,&
             vE_dot_grad_b(idir)) + vE(ixGlo1:ixGhi1,&
             idim) * tmp(ixGlo1:ixGhi1)
          call gradient(vE(ixGlo1:ixGhi1,idir),ixGlo1,ixGhi1,ixGlo1+1,ixGhi1-1,&
             idim,tmp)
          gridvars(igrid)%w(ixGlo1:ixGhi1,&
             b_dot_grad_vE(idir)) = gridvars(igrid)%w(ixGlo1:ixGhi1,&
             b_dot_grad_vE(idir)) + bhat(ixGlo1:ixGhi1,&
             idim) * tmp(ixGlo1:ixGhi1)
          gridvars(igrid)%w(ixGlo1:ixGhi1,&
             vE_dot_grad_vE(idir)) = gridvars(igrid)%w(ixGlo1:ixGhi1,&
             vE_dot_grad_vE(idir)) + vE(ixGlo1:ixGhi1,&
             idim) * tmp(ixGlo1:ixGhi1)
        end do
      end do
      ...
      if(time_advance) then
        ...
        do idim=1,ndim
          ...
          gridvars(igrid)%wold(ixGlo1:ixGhi1,&
             grad_kappa_B(idim)) = tmp(ixGlo1:ixGhi1)
        end do
        ...
        do idir=1,ndir
          ...
          do idim=1,ndim
            ...
            gridvars(igrid)%wold(ixGlo1:ixGhi1,&
               ...
               vE_dot_grad_b(idir)) + vE(ixGlo1:ixGhi1,&
               idim) * tmp(ixGlo1:ixGhi1)
            call gradient(vE(ixGlo1:ixGhi1,idir),ixGlo1,ixGhi1,ixGlo1+1,&
               ixGhi1-1,idim,tmp)
            gridvars(igrid)%wold(ixGlo1:ixGhi1,&
  ...
  subroutine gca_integrate_particles(end_time)
    ...
    ! y(ndir+3) = particle(ipart)%self%u(3) ! Lorentz factor of particle
      ...
      if(.not. particle_in_igrid(ipart_working,igrid_working)) then
        ...
        if (ps(igrid_working)%x(ic1,1) .lt. y(1)) then
           ic11 = ic1
        else
           ic11 = ic1 -1
        end if
...
!       call odeint(y,nvar,tloc,tloc+dt_p,eps,h1,hmin,nok,nbad,derivs_gca,rkqs)
  ...
  subroutine gca_update_payload(igrid,w,wold,xgrid,xpart,upart,qpart,mpart,&
    ...
    double precision, intent(in)  :: w(ixGlo1:ixGhi1,1:nw),wold(ixGlo1:ixGhi1,&
       1:nw)
    double precision, intent(in)  :: xgrid(ixGlo1:ixGhi1,1:ndim),xpart(1:ndir),&
       upart(1:ndir),qpart,mpart,particle_time
