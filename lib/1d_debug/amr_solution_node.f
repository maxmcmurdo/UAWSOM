...
subroutine alloc_node(igrid)
  ...
  if(.not. allocated(ps(igrid)%w)) then
    ...
    call alloc_state_coarse(igrid, psc(igrid), ixCoGmin1,ixCoGmax1, ixCoGmin1,&
       ixCoGmax1)
    if(.not.convert) then
      ...
      case(RK3_BT,rk4,ssprk5,IMEX_CB3a)
        call alloc_state(igrid, ps2(igrid), ixGlo1,ixGhi1, ixGextmin1,&
           ixGextmax1, .false.)
        call alloc_state(igrid, ps3(igrid), ixGlo1,ixGhi1, ixGextmin1,&
           ixGextmax1, .false.)
      case(IMEX_ARS3,IMEX_232)
        ...
        call alloc_state(igrid, ps3(igrid), ixGlo1,ixGhi1, ixGextmin1,&
           ixGextmax1, .false.)
        call alloc_state(igrid, ps4(igrid), ixGlo1,ixGhi1, ixGextmin1,&
           ixGextmax1, .false.)
      end select
    end if
 ...
 do ix=ixGlo1,ixMhi1-nghostcells
    ps(igrid)%x(ix,1)=rnode(rpxmin1_,igrid)+(dble(ix-nghostcells)-half)*dx1
  end do
 ! update overlap cells of neighboring blocks in the same way to get the same values
 do ix=ixMhi1-nghostcells+1,ixGhi1
    ps(igrid)%x(ix,1)=rnode(rpxmax1_,igrid)+(dble(ix-ixMhi1)-half)*dx1
  end do

  dx1=2.0d0*rnode(rpdx1_,igrid)
 do ix=ixCoGmin1,ixCoGmax1
    psc(igrid)%x(ix,1)=rnode(rpxmin1_,igrid)+(dble(ix-nghostcells)-half)*dx1
  end do
 ...
 do ix=ixGextmin1,ixGextmax1
    xext(ix,1)=rnode(rpxmin1_,igrid)+(dble(ix-nghostcells)-half)*dx1
  end do
  ...
  select case (coordinate)
    ...
    case (Cartesian_expansion)
      ...
      delx_ext(ixGextmin1:ixGextmax1) = ps(igrid)%dx(ixGextmin1:ixGextmax1,1)
      if(associated(usr_set_surface)) call usr_set_surface(ixGextmin1,&
         ixGextmax1,xext(ixGextmin1:ixGextmax1,1),&
         delx_ext(ixGextmin1:ixGextmax1),exp_factor_ext(ixGextmin1:ixGextmax1),&
         del_exp_factor_ext(ixGextmin1:ixGextmax1),&
         exp_factor_primitive_ext(ixGextmin1:ixGextmax1))
      ps(igrid)%dvolume(ixGextmin1:ixGextmax1)= &
         exp_factor_primitive_ext(ixGextmin1:ixGextmax1)
      ...
      xc(ixCoGmin1:ixCoGmax1) = psc(igrid)%x(ixCoGmin1:ixCoGmax1,1)
      delxc(ixCoGmin1:ixCoGmax1) = psc(igrid)%dx(ixCoGmin1:ixCoGmax1,1)
      if(associated(usr_set_surface)) call usr_set_surface(ixCoGmin1,ixCoGmax1,&
         xc(ixCoGmin1:ixCoGmax1),delxc(ixCoGmin1:ixCoGmax1),&
         exp_factor_coarse(ixCoGmin1:ixCoGmax1),&
         del_exp_factor_coarse(ixCoGmin1:ixCoGmax1),&
         exp_factor_primitive_coarse(ixCoGmin1:ixCoGmax1))
      psc(igrid)%dvolume(ixCoGmin1:ixCoGmax1)= &
         exp_factor_primitive_coarse(ixCoGmin1:ixCoGmax1)
      psc(igrid)%ds(ixCoGmin1:ixCoGmax1,1)=psc(igrid)%dx(ixCoGmin1:ixCoGmax1,&
         1)
      ...
      if(z_>0.and.z_<=ndim) then
        ...
        if(phi_>z_.and.ndir>ndim) then
          ps(igrid)%dsC(ixGextmin1:ixGextmax1,phi_)=xext(ixGextmin1:ixGextmax1,&
             1)+half*ps(igrid)%dx(ixGextmin1:ixGextmax1,1)
        end if
      end if
...
subroutine alloc_state(igrid, s, ixGmin1,ixGmax1, ixGextmin1,ixGextmax1,&
  ...
  allocate(s%w(ixGmin1:ixGmax1,1:nw))
  s%igrid=igrid
  s%w=0.d0
  s%ixGmin1=ixGmin1;s%ixGmax1=ixGmax1;
   ixGsmin1 = ixGmin1-1; ixGsmax1 = ixGmax1
  if(stagger_grid) then
    allocate(s%ws(ixGsmin1:ixGsmax1,1:nws))
    s%ws=0.d0
    if(record_electric_field) then
      allocate(s%we(ixGsmin1:ixGsmax1,1:nws))
      s%we=0.d0
    end if
  ...
  if(alloc_once_for_ps) then
    ...
    if(nw_extra>0) allocate(s%wextra(ixGmin1:ixGmax1,1:nw_extra))
    ...
    if(B0field) then
      allocate(s%B0(ixGmin1:ixGmax1,1:ndir,0:ndim))
      allocate(s%J0(ixGmin1:ixGmax1,7-2*ndir:3))
    end if
    if(number_equi_vars > 0) then
      allocate(s%equi_vars(ixGmin1:ixGmax1,1:number_equi_vars,0:ndim))
    endif
...
subroutine alloc_state_coarse(igrid, s, ixGmin1,ixGmax1, ixGextmin1,&
   ixGextmax1)
  use mod_global_parameters 
