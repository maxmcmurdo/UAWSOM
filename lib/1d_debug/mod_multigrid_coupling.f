...
contains
  ...
  subroutine mg_setup_multigrid()
    ...
    if (any([ block_nx1 ] /= block_nx1)) error stop "Multigrid requires all block_nx to be equal"
  ...
  subroutine mg_copy_to_tree(iw_from, iw_to, restrict, restrict_gc, factor,&
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_from)) then
          mg%boxes(id)%cc(0:nc+1, iw_to) = fac * state_from(igrid)%w(ilo1:ihi1,&
              iw_from)
       else
          mg%boxes(id)%cc(0:nc+1, iw_to) = fac * ps(igrid)%w(ilo1:ihi1,&
              iw_from)
       end if
    end do
  ...
  subroutine mg_copy_from_tree(iw_from, iw_to, state_to)
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%w(ixMlo1:ixMhi1, iw_to) = mg%boxes(id)%cc(1:nc,&
              iw_from)
       else
          ps(igrid)%w(ixMlo1:ixMhi1, iw_to) = mg%boxes(id)%cc(1:nc, iw_from)
       end if
    end do
  ...
  subroutine mg_copy_from_tree_gc(iw_from, iw_to, state_to)
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%w(ilo1:ihi1, iw_to) = mg%boxes(id)%cc(0:nc+1,&
              iw_from)
       else
          ps(igrid)%w(ilo1:ihi1, iw_to) = mg%boxes(id)%cc(0:nc+1, iw_from)
       end if
    end do
  ...
  subroutine mg_tree_from_amrvac(mg)
    ...
    call mg_build_rectangle(mg, [ domain_nx1 ], block_nx1, dx(:,1),&
        [ xprobmin1 ], periodB, n_finer)
       ...
       do i = 1, size(mg%lvls(lvl)%ids)
          ...
          if (.not. pnode%leaf) then
             ...
             do i_c = 1, mg_num_children
                ...
                pnode_ch => pnode%child(c_ix(1))%node
