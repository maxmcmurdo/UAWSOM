...
contains
      ...
      if(ipe_particle == mype) then
        ...
        do idir=1,ndir
          call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
             w(ixGlo1:ixGhi1,iw_mom(idir)),ps(igrid)%x,x(:,n),v(idir,n))
        end do
  ...
  subroutine advect_fill_gridvars
    ...
    double precision, dimension(ixGlo1:ixGhi1,1:nw)   :: w

    do iigrid=1,igridstail; igrid=igrids(iigrid);

      dxlevel(1)=rnode(rpdx1_,igrid);

      gridvars(igrid)%w(ixGlo1:ixGhi1,1:ngridvars) = 0.0d0
      ...
      if(time_advance) then
        gridvars(igrid)%wold(ixGlo1:ixGhi1,1:ngridvars) = 0.0d0
  ...
  subroutine advect_update_payload(igrid,w,wold,xgrid,xpart,upart,qpart,mpart,&
    ...
    if (mynpayload > 0 ) then
      if (.not.time_advance) then
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,w(ixGlo1:ixGhi1,&
           iw_rho),xgrid,xpart,rho)
      else
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           wold(ixGlo1:ixGhi1,iw_rho),xgrid,xpart,rho1)
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,w(ixGlo1:ixGhi1,&
           iw_rho),xgrid,xpart,rho2)
  ...
  pure function advect_get_particle_dt(partp, end_time) result(dt_p)
    ...
    dt_cfl = min(rnode(rpdx1_,partp%igrid)/v(1),bigdouble)
  ...
  subroutine get_vec_advect(igrid,x,tloc,var,ibeg,iend)
    ...
    if(.not.time_advance) then
      do ivar=ibeg,iend
        iloc = ivar-ibeg+1
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%w(ixGlo1:ixGhi1,ivar),ps(igrid)%x(ixGlo1:ixGhi1,&
           1:ndim),x,var(iloc))
      end do
    else
      ...
      do ivar=ibeg,iend
        iloc = ivar-ibeg+1
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%wold(ixGlo1:ixGhi1,ivar),ps(igrid)%x(ixGlo1:ixGhi1,&
           1:ndim),x,e1(iloc))
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%w(ixGlo1:ixGhi1,ivar),ps(igrid)%x(ixGlo1:ixGhi1,&
           1:ndim),x,e2(iloc))
