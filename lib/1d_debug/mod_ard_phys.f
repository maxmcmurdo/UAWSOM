...
module mod_ard_phys
  ...
  double precision, public, protected :: A1(1) = 0.0d0
  !> Advection coefficients for second species (v) (if applicable)
  double precision, public, protected :: A2(1) = 0.0d0
  !> Advection coefficients for third species (w) (if applicable)
  double precision, public, protected :: A3(1) = 0.0d0
...
111    close(unitpar)
  ...
  subroutine ard_to_conserved(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    use mod_global_parameters
  ...
  subroutine ard_to_primitive(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    double precision, intent(inout) :: w(ixImin1:ixImax1, nw)
    double precision, intent(in)    :: x(ixImin1:ixImax1, 1:1)
  ...
  subroutine ard_get_cmax(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim, cmax)
    use mod_global_parameters
    ...
    cmax(ixOmin1:ixOmax1) = abs(A1(idim) * w(ixOmin1:ixOmax1,u_)**(adv_pow-1))
    if (number_of_species >= 2) then
       cmax(ixOmin1:ixOmax1) = max(cmax(ixOmin1:ixOmax1),&
           abs(A2(idim) * w(ixOmin1:ixOmax1,v_)**(adv_pow-1)))
    end if
    if (number_of_species >= 3) then
       cmax(ixOmin1:ixOmax1) = max(cmax(ixOmin1:ixOmax1),&
           abs(A3(idim) * w(ixOmin1:ixOmax1,w_)**(adv_pow-1)))
    end if

  end subroutine ard_get_cmax

  subroutine ard_get_cbounds(wLC, wRC, wLp, wRp, x, ixImin1,ixImax1, ixOmin1,&
    ...
    if (present(cmin)) then
       cmin(ixOmin1:ixOmax1,1) = min(A1(idim) * wmean(ixOmin1:ixOmax1,&
          u_)**(adv_pow-1), zero)
       cmax(ixOmin1:ixOmax1,1) = max(A1(idim) * wmean(ixOmin1:ixOmax1,&
          u_)**(adv_pow-1), zero)
       if (number_of_species >= 2) then
          cmin(ixOmin1:ixOmax1,1) = min(cmin(ixOmin1:ixOmax1,1),&
              A2(idim) * wmean(ixOmin1:ixOmax1,v_)**(adv_pow-1))
          cmax(ixOmin1:ixOmax1,1) = max(cmax(ixOmin1:ixOmax1,1),&
              A2(idim) * wmean(ixOmin1:ixOmax1,v_)**(adv_pow-1))
       end if
       if (number_of_species >= 3) then
          cmin(ixOmin1:ixOmax1,1) = min(cmin(ixOmin1:ixOmax1,1),&
              A3(idim) * wmean(ixOmin1:ixOmax1,w_)**(adv_pow-1))
          cmax(ixOmin1:ixOmax1,1) = max(cmax(ixOmin1:ixOmax1,1),&
              A3(idim) * wmean(ixOmin1:ixOmax1,w_)**(adv_pow-1))
       end if
    else
       cmax(ixOmin1:ixOmax1,1) = maxval(abs(A1(idim) * wmean(ixOmin1:ixOmax1,&
          u_)**(adv_pow-1)))
       if (number_of_species >=2) then
          cmax(ixOmin1:ixOmax1,1) = max(cmax(ixOmin1:ixOmax1,1),&
              maxval(abs(A2(idim) * wmean(ixOmin1:ixOmax1,v_)**(adv_pow-1))))
       end if
       if (number_of_species >=3) then
          cmax(ixOmin1:ixOmax1,1) = max(cmax(ixOmin1:ixOmax1,1),&
              maxval(abs(A3(idim) * wmean(ixOmin1:ixOmax1,w_)**(adv_pow-1))))
       end if
    end if

  end subroutine ard_get_cbounds

  subroutine ard_get_dt(w, ixImin1,ixImax1, ixOmin1,ixOmax1, dtnew, dx1, x)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    ...
    dtnew = min(dtnew, dtdiffpar * minval([ dx1 ])**2 / (2 * ndim * maxD))
  ...
  subroutine ard_get_flux(wC, w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim, f)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1, idim
    ...
    f(ixOmin1:ixOmax1, u_) = (A1(idim)/adv_pow) * w(ixOmin1:ixOmax1,&
       u_)**adv_pow
    if (number_of_species >=2) then
       f(ixOmin1:ixOmax1, v_) = (A2(idim)/adv_pow) * w(ixOmin1:ixOmax1,&
          v_)**adv_pow
    end if
    if (number_of_species >=3) then
       f(ixOmin1:ixOmax1, w_) = (A3(idim)/adv_pow) * w(ixOmin1:ixOmax1,&
          w_)**adv_pow
    end if

  end subroutine ard_get_flux

  subroutine ard_add_source_geom(qdt, ixImin1,ixImax1, ixOmin1,ixOmax1, wCT, w,&
      x)
  ...
  subroutine ard_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,&
    ...
    if (qsourcesplit .eqv. ard_source_split) then
       if (.not.use_imex_scheme) then
          call ard_laplacian(ixImin1,ixImax1, ixOmin1,ixOmax1,&
       ...
       case (eq_no_reac)
          w(ixOmin1:ixOmax1, u_) = w(ixOmin1:ixOmax1, u_) + qdt * D1 * lpl_u
       case default
          call mpistop("Unknown equation type")
       end select
  ...
  subroutine ard_laplacian(ixImin1,ixImax1,ixOmin1,ixOmax1,var,lpl)
    use mod_global_parameters
    integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1
  ...
  subroutine put_laplacians_onegrid(ixImin1,ixImax1,ixOmin1,ixOmax1,w)
    ...
    call ard_laplacian(ixImin1,ixImax1, ixOmin1,ixOmax1, w(ixImin1:ixImax1,&
        u_), lpl_u)
    if (number_of_species >= 2) then
       call ard_laplacian(ixImin1,ixImax1, ixOmin1,ixOmax1, w(ixImin1:ixImax1,&
           v_), lpl_v)
