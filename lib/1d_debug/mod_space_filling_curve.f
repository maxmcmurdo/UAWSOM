...
contains
  ...
  subroutine level1_Morton_order
    ...
    integer, allocatable :: gsq_sfc(:),seq_sfc(:),seq_ig1(:)
    integer :: ig1, ngsq1, isq, total_number
    ...
    do ig1=1,ngsq1
       gsq_sfc(ig1)=int(mortonEncode(ig1-1,ndim))+1
       seq_sfc(gsq_sfc(ig1))=gsq_sfc(ig1)
       seq_ig1(gsq_sfc(ig1))=ig1 
    end do
    ! mark blocks that are out of the domain and change Morton number
    do isq=1,total_number
       if (seq_ig1(isq)>ng1(1)) then
         seq_sfc(isq:total_number)=seq_sfc(isq:total_number)-1
         in_domain(isq)=.false.
       end if
    end do
    ! copy the modified Morton numbers to the blocks in the domain
    if(.not. allocated(iglevel1_sfc)) allocate(iglevel1_sfc(ng1(1)))
    if(.not. allocated(sfc_iglevel1)) allocate(sfc_iglevel1(ndim,nglev1))
    do isq=1,total_number
      if(in_domain(isq)) then
        iglevel1_sfc(seq_ig1(isq))=seq_sfc(isq)
        sfc_iglevel1(1,seq_sfc(isq))=seq_ig1(isq) 
      end if
    end do

    deallocate(gsq_sfc,seq_sfc,seq_ig1,in_domain)

  end subroutine level1_Morton_order

  integer(kind=8) function mortonEncode(ig1,ndim)
    use iso_fortran_env, only : int64
    implicit none
    integer(kind=4), intent(in) :: ig1,ndim
    integer(kind=4) :: i
    integer(kind=8) :: answer, lg1
