...
contains
  ...
  subroutine read_par_files()
    ...
    integer          :: nx_vec(1), block_nx_vec(1)
    integer          :: my_unit, iostate
    integer          :: ilev
    double precision :: dx_vec(1)
    ...
    double precision :: sizeuniformpart1
    ...
    namelist /methodlist/ time_stepper,time_integrator, source_split_usr,&
       ...
       small_density, small_values_method, small_values_daverage,&
        ...
        small_values_fix_iw, schmid_rad1

    namelist /boundlist/ nghostcells,ghost_copy,internalboundary,&
        typeboundary_min1,typeboundary_max1, save_physical_boundary

    namelist /meshlist/ refine_max_level,nbufferx1,refine_threshold,&
       ...
       max_blocks,block_nx1,domain_nx1,iprob,xprobmin1,xprobmax1,&
        w_refine_weight, prolongprimitive,coarsenprimitive, typeprolonglimit,&
...
'will try default value for qstretch_baselevel in dimension', 1
    ...
    dx_vec = [xprobmax1-xprobmin1] / nx_vec
  ...
  subroutine snapshot_write_header1(fh, offset_tree, offset_block,&
    ...
    call MPI_FILE_WRITE(fh, [ xprobmin1 ], ndim, MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, [ xprobmax1 ], ndim, MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, [ domain_nx1 ], ndim, MPI_INTEGER, st, er)
    call MPI_FILE_WRITE(fh, [ block_nx1 ], ndim, MPI_INTEGER, st, er)
  ...
  subroutine snapshot_read_header(fh, offset_tree, offset_block)
    ...
    if (maxval(abs(rbuf(1:ndim) - [ xprobmin1 ])) > 0) then
    ...
    ! xprobmax^D
    call MPI_FILE_READ(fh,rbuf(1:ndim),ndim,MPI_DOUBLE_PRECISION,st,er)
    if (maxval(abs(rbuf(1:ndim) - [ xprobmax1 ])) > 0) then
    ...
    if (any(ibuf(1:ndim) /= [ domain_nx1 ])) then
    ...
    ! block_nx^D
    call MPI_FILE_READ(fh,ibuf(1:ndim), ndim, MPI_INTEGER,st,er)
    if (any(ibuf(1:ndim) /= [ block_nx1 ])) then
    ...
    if (version > 4) then
      ...
      if (periodic(1) .and. .not.periodB(1) .or. .not.periodic(1) .and. &
         periodB(1)) call mpistop("change in periodicity in par file")
...
!        call mpistop("Cannot restart with a different physics type")
  ...
  pure integer function count_ix(ixOmin1,ixOmax1)
    integer, intent(in) :: ixOmin1,ixOmax1

    count_ix = product([ ixOmax1 ] - [ ixOmin1 ] + 1)
  end function count_ix

  !> Determine the shape of a block for output (whether to include ghost cells,
  !> and on which sides)
  subroutine block_shape_io(igrid, n_ghost, ixOmin1,ixOmax1, n_values)
    use mod_global_parameters
    ...
    n_values = count_ix(ixOmin1,ixOmax1) * nw
  ...
  subroutine write_snapshot
    ...
    integer                       :: ixOmin1,ixOmax1, n_ghost(2*ndim)
    integer                       :: ixOsmin1,ixOsmax1,n_values_stagger
    ...
    n_values = count_ix(ixGlo1,ixGhi1) * nw
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGshi1) * nws
    end if
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      call block_shape_io(igrid, n_ghost, ixOmin1,ixOmax1, n_values)
      if(stagger_grid) then
        w_buffer(1:n_values) = pack(ps(igrid)%w(ixOmin1:ixOmax1, 1:nw),&
            .true.)
        ixOsmin1 = ixOmin1 -1
        ixOsmax1 = ixOmax1 
        n_values_stagger= count_ix(ixOsmin1,ixOsmax1)*nws
        w_buffer(n_values+1:n_values+n_values_stagger) = &
           pack(ps(igrid)%ws(ixOsmin1:ixOsmax1, 1:nws), .true.)
  ...
  subroutine read_snapshot
    ...
    integer                       :: ix_buffer(2*ndim+1), n_values,&
        n_values_stagger
    integer                       :: ixOmin1,ixOmax1, ixOsmin1,ixOsmax1
    ...
    double precision :: ws(ixGslo1:ixGshi1,1:ndim)
    ...
    ! Allocate send/receive buffer
    n_values = count_ix(ixGlo1,ixGhi1) * nw_found
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGshi1) * nws
    end if
    allocate(w_buffer(n_values))
    allocate(w(ixGlo1:ixGhi1,1:nw_found))
    ...
    if (mype==0) then
      ...
      do ipe = 0, npe-1
        do Morton_no=Morton_start(ipe),Morton_stop(ipe)
          ...
          n_values = count_ix(ixOmin1,ixOmax1) * nw_found
          if(stagger_grid) then
            ixOsmin1 = ixOmin1 - 1
            ixOsmax1 = ixOmax1
            n_values_stagger = n_values
            n_values = n_values + count_ix(ixOsmin1,ixOsmax1) * nws
          end if
          ...
          if (mype == ipe) then ! Root task
            ...
            block=>ps(igrid)
            if(stagger_grid) then
              w(ixOmin1:ixOmax1, 1:nw_found) = &
                 reshape(w_buffer(1:n_values_stagger), shape(w(ixOmin1:ixOmax1,&
                  1:nw_found)))
              ps(igrid)%ws(ixOsmin1:ixOsmax1,&
                 1:nws)=reshape(w_buffer(n_values_stagger+1:n_values),&
                  shape(ws(ixOsmin1:ixOsmax1, 1:nws)))
            else
              w(ixOmin1:ixOmax1, 1:nw_found) = reshape(w_buffer(1:n_values),&
                  shape(w(ixOmin1:ixOmax1, 1:nw_found)))
            end if
            if (nw_found<nw) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixGlo1,ixGhi1,ixMlo1,ixMhi1,nw_found,w,&
                   ps(igrid)%x,ps(igrid)%w)
              else
                ps(igrid)%w(ixOmin1:ixOmax1,1:nw_found)=w(ixOmin1:ixOmax1,&
                   1:nw_found)
              end if
              ...
              if (associated(usr_transform_w)) then
                ...
                ps(igrid)%w(ixOmin1:ixOmax1,1:nw)=w(ixOmin1:ixOmax1,1:nw)
              end if
            else
              ps(igrid)%w(ixOmin1:ixOmax1,1:nw)=w(ixOmin1:ixOmax1,1:nw)
            end if
          else
            call MPI_SEND([ ixOmin1,ixOmax1, n_values ], 2*ndim+1, MPI_INTEGER,&
                ipe, itag, icomm, ierrmpi)
      ...
      do Morton_no=Morton_start(mype),Morton_stop(mype)
        ...
        if(stagger_grid) then
          n_values_stagger = count_ix(ixOmin1,ixOmax1) * nw_found
          ixOsmin1 = ixOmin1 - 1
          ixOsmax1 = ixOmax1
          w(ixOmin1:ixOmax1, 1:nw_found) = &
             reshape(w_buffer(1:n_values_stagger), shape(w(ixOmin1:ixOmax1,&
  ...
  subroutine read_snapshot_old()
    ...
    double precision              :: wio(ixGlo1:ixGhi1,1:nw)
    integer                       :: fh, igrid, Morton_no, iread
    integer                       :: levmaxini, ndimini, ndirini
    integer                       :: nwini, neqparini, nxini1
    ...
    if (mype==0) then
      ...
      if (ixGhi1/=nxini1+2*nghostcells) then
    ...
    if (npe>1) then
      if (mype==0) then
        sendini=(/nleafs,levmaxini,ndimini,ndirini,nwini,neqparini,it ,&
           nxini1 /)
      end if
      call MPI_BCAST(sendini,7+1,MPI_INTEGER,0,icomm,ierrmpi)
  ...
  subroutine get_volume_average(power, mode, volume)
    ...
    do iigrid = 1, igridstail
       ...
       do iw = 1, nw
          wsum(iw) = wsum(iw) + sum(ps(igrid)%dvolume(ixMlo1:ixMhi1)*ps(&
             igrid)%w(ixMlo1:ixMhi1,iw)**power)
       end do
    end do
  ...
  subroutine get_volume_average_func(func, f_avg, volume)
    ...
    do iigrid = 1, igridstail
       ...
       ! Store total volume in last element
       wsum(2) = wsum(2) + sum(ps(igrid)%dvolume(ixMlo1:ixMhi1))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do i1 = ixMlo1, ixMhi1
       wsum(1) = wsum(1) + ps(igrid)%dvolume(i1) * func(ps(igrid)%w(i1, :),&
           nw)
       end do
    end do
