...
contains
  ...
  subroutine rhd_ppm_flatcd(ixImin1,ixImax1,ixOmin1,ixOmax1,ixLmin1,ixLmax1,&
    ...
    if(rhd_energy) then
      drho(ixOmin1:ixOmax1) = rhd_gamma*abs(d2w(ixOmin1:ixOmax1,&
          rho_))/min(w(ixLmin1:ixLmax1, rho_), w(ixRmin1:ixRmax1, rho_))
      dp(ixOmin1:ixOmax1) = abs(d2w(ixOmin1:ixOmax1,&
  ...
  subroutine rhd_ppm_flatsh(ixImin1,ixImax1,ixOmin1,ixOmax1,ixLLmin1,ixLLmax1,&
    ...
    if(rhd_energy) then
      ...
      dp(ixOmin1:ixOmax1) =(rhd_gamma*w(ixOmin1:ixOmax1, e_)/w(ixOmin1:ixOmax1,&
          rho_))

      dp(ixOmin1:ixOmax1) = abs(w(ixRmin1:ixRmax1, e_)-w(ixLmin1:ixLmax1,&
          e_))/(w(ixOmin1:ixOmax1, rho_)*dp(ixOmin1:ixOmax1))
