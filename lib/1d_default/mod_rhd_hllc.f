...
contains
  ...
  subroutine rhd_diffuse_hllcd(ixImin1,ixImax1,ixOmin1,ixOmax1,idim,wLC,wRC,&
     fLC,fRC,patchf)
  ...
  subroutine rhd_get_lCD(wLC,wRC,fLC,fRC,cmin,cmax,idim,ixImin1,ixImax1,&
  ...
  subroutine rhd_get_wCD(wLC,wRC,whll,fRC,fLC,Fhll,patchf,lambdaCD,cmin,cmax,&
    ...
    do ix1=ixOmin1,ixOmax1
      if(abs(patchf(ix1))==1) then
        ...
        wCD(ix1,rho_) = wSub(ix1,rho_)*(cspeed(ix1)-vSub(ix1))*csmls
        do n=1,rhd_n_tracer
          iw = tracer(n)
          wCD(ix1,iw) = wSub(ix1,iw)*(cspeed(ix1)-vSub(ix1))*csmls
        end do
