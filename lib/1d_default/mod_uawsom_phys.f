...
!! end rad cool
  ...
  subroutine w_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,energy,&
    ...
    double precision :: Bsqr(ixImin1:ixImax1),vA(ixImin1:ixImax1),&
       ...
       Bfix(ixImin1:ixImax1),lnvA(ixImin1:ixImax1),lnrho(ixImin1:ixImax1),&
        gradlnvA(ixImin1:ixImax1) 
    ...
    radius(ixOmin1:ixOmax1) = 1.d8/unit_length * &
       ((Busr/unit_magneticfield)/B(ixOmin1:ixOmax1,1))**0.5d0 !Radius = R_0*(B0/B)0.5 TVD 2025 paper uses R_0 = 1Mm (1e8 cm)
    ...
    vA(ixImin1:ixImax1) = B(ixImin1:ixImax1,1)/dsqrt(w(ixImin1:ixImax1,rho_))

    lnvA(ixImin1:ixImax1) = log(vA(ixImin1:ixImax1))
    !lnrho(ixI^S) = log(w(ixI^S,rho_))

    call gradient(lnvA,ixImin1,ixImax1,ixOmin1,ixOmax1,ndim,gradlnvA)
    call gradient(vA,ixImin1,ixImax1,ixOmin1,ixOmax1,ndim,gradvA)
    ...
    Lperp(ixOmin1:ixOmax1) = (zeta(ixOmin1:ixOmax1) + 1.d0 - &
       ...
       1.d0)*3.1622776*(ff*dpi)**0.5d0*radius(ixOmin1:ixOmax1) !/min(pth(ixOmin1:ixOmax1),1.d0) 
    ...
    Lperp_AW(ixOmin1:ixOmax1) = (1/1.d8) * 1.5d5 * 1.0d2 * &
       (Tmp(ixOmin1:ixOmax1) / B(ixOmin1:ixOmax1,1))**0.5d0 
    ...
    do ix1 = ixOmin1, ixOmax1 
      w(ix1,wAminus_) = w(ix1,wAminus_) - qdt*(divv(ix1)*wCT(ix1,&
         wAminus_)/2.0d0 + wCT(ix1,wAminus_)*Gamma_minus(ix1) + ((w(ix1,&
         mom(1)) + vA(ix1))/vA(ix1))*refvA(ix1)*wCT(ix1,wAminus_))
      w(ix1,wAplus_)  = w(ix1,wAplus_)  - qdt*(divv(ix1)*wCT(ix1,&
         wAplus_)/2.0d0 + wCT(ix1,wAplus_)*Gamma_plus(ix1) - ((w(ix1,&
         mom(1)) + vA(ix1))/vA(ix1))*refvA(ix1)*wCT(ix1,wAminus_))
    end do
