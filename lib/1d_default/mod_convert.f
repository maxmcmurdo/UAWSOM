module mod_convert
  ...
  abstract interface
     ...
     function sub_convert_vars(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x,&
         nwc) result(wnew)
...
contains
  ...
  subroutine add_convert_method2(phys_convert_vars, nwc, aux_variable_names,&
    ...
    interface
     function phys_convert_vars(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x,&
         nwc) result(wnew)
       use mod_global_parameters
  ...
  subroutine convert_dat_generic(nwc, dataset_names, file_suffix,&
    ...
    interface
      ...
      function convert_vars(ixImin1,ixImax1, ixOmin1,ixOmax1,w,x,&
         nwc) result(wres)
        use mod_global_parameters
        integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1, nwc
        double precision, intent(in) ::  x(ixImin1:ixImax1,1:ndim)
        double precision, intent(in) :: w(ixImin1:ixImax1,1:nw)
        double precision  :: wres(ixOmin1:ixOmax1,1:nwc)
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      n_values = count_ix(ixOmin1,ixOmax1) * nwc
      ! end copied from block_shape_io

      ixImin1 = ixGlo1
      ixImax1 = ixGhi1

      w_buffer(1:n_values) = pack(convert_vars(ixImin1,ixImax1, ixOmin1,&
         ixOmax1,ps(igrid)%w(ixImin1:ixImax1, 1:nw),&
          ps(igrid)%x(ixImin1:ixImax1, 1:ndim),nwc), .true.)
