...
module m_octree_mg_1d
  ...
  logical, parameter, public :: mg_child_low(1, 2) = reshape([.true., .false.],&
      [1, 2])
...
contains
  ...
  subroutine mg_allocate_storage(mg)
    ...
    do lvl = mg%lowest_lvl, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_ids)
          ...
          allocate(mg%boxes(id)%cc(0:nc+1, mg_num_vars + mg%n_extra_vars))
  ...
  subroutine set_rhs(mg, f1, f2)
    ...
    do lvl = 1, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_leaves)
          ...
          mg%boxes(id)%cc(1:nc, mg_irhs) = f1 * mg%boxes(id)%cc(1:nc,&
              mg_iphi) + f2 * mg%boxes(id)%cc(1:nc, mg_irhs)
       end do
    end do
  ...
  subroutine box_gs_lpl(mg, id, nc, redblack_cntr)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      ...
      do i = i0, nc, di
         cc(i, n) = fac * ( idr2(1) * (cc(i+1, n) + cc(i-1, n)) - cc(i,&
             mg_irhs))
      end do
    end associate
...
!   end subroutine box_jacobi_lpl
  ...
  subroutine box_lpl(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      do i = 1, nc
            cc(i, i_out) = idr2(1) * (cc(i-1, n) + cc(i+1, n) - 2 * cc(i, n))
         end do
    end associate
  ...
  subroutine box_vhelmh(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, i_eps => mg_iveps)
      do i = 1, nc
         ...
         cc(i, i_out) = sum(2 * idr2 * a0*a(:)/(a0 + a(:)) * (u(:) - u0)) - &
  ...
  subroutine box_gs_vlpl(mg, id, nc, redblack_cntr)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, i_eps => mg_iveps)
      ...
      do i = i0, nc, di
         ...
         cc(i, n) = (sum(c(:) * u(:)) - cc(i, mg_irhs)) / sum(c(:))
      end do
    end associate
...
!   end subroutine vlpl_prolong
  ...
  subroutine bc_to_gc(mg, id, nc, iv, nb, bc_type)
    ...
    select case (nb)
    case (mg_neighb_lowx)
       mg%boxes(id)%cc(0, iv) = c0 * mg%boxes(id)%cc(0,&
           iv) + c1 * mg%boxes(id)%cc(1, iv) + c2 * mg%boxes(id)%cc(2, iv)
    case (mg_neighb_highx)
       mg%boxes(id)%cc(nc+1, iv) = c0 * mg%boxes(id)%cc(nc+1,&
           iv) + c1 * mg%boxes(id)%cc(nc, iv) + c2 * mg%boxes(id)%cc(nc-1, iv)
    end select
  ...
  subroutine prolong_onto(mg, id, nc, iv, iv_to, add, method)
    ...
    if (add) then
       mg%boxes(id)%cc(1:nc, iv_to) = mg%boxes(id)%cc(1:nc, iv_to) + tmp
    else
  ...
  subroutine box_helmh(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi)
      do i = 1, nc
         cc(i, i_out) = idr2(1) * (cc(i-1, n) + cc(i+1, n) - 2 * cc(i,&
             n)) - helmholtz_lambda * cc(i, n)
      end do
    end associate
    ...
    do lvl = mg%lowest_lvl, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_ids)
          ...
          if (include_ghostcells) then
             ...
             mg%boxes(id)%cc(1:nc, iv) = mg%boxes(id)%cc(1:nc, iv) - mean_iv
          end if
       end do
    end do
  ...
  real(dp) function get_sum(mg, iv)
    ...
    do lvl = 1, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_leaves)
          id = mg%lvls(lvl)%my_leaves(i)
          get_sum = get_sum + w * sum(mg%boxes(id)%cc(1:nc, iv))
       end do
    end do
  ...
  subroutine update_coarse(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl-1)%my_parents)
       ...
       mg%boxes(id)%cc(1:nc_c, mg_irhs) = mg%boxes(id)%cc(1:nc_c,&
           mg_irhs) + mg%boxes(id)%cc(1:nc_c, mg_ires)
  ...
  subroutine residual_box(mg, id, nc)
    ...
    mg%boxes(id)%cc(1:nc, mg_ires) = mg%boxes(id)%cc(1:nc,&
        mg_irhs) - mg%boxes(id)%cc(1:nc, mg_ires)
  ...
  subroutine restrict_set_buffer(mg, id, iv)
    ...
    if (p_rank /= mg%my_rank) then
       do i = 1, hnc
          tmp(i) = 0.5_dp * sum(mg%boxes(id)%cc(2*i-1:2*i, iv))
       end do
    ...
    do i_c = 1, mg_num_children
       ...
       if (c_rank == mg%my_rank) then
          ...
          mg%boxes(id)%cc(dix(1)+1:dix(1)+hnc,&
              iv) = reshape(mg%buf(c_rank)%recv(i+1:i+dsize), [hnc])
  ...
  subroutine box_ahelmh(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, i_eps1 => mg_iveps1,&
      ...
      do i = 1, nc
         ...
         cc(i, i_out) = sum(2 * idr2 * a0(:)*a(:)/(a0(:) + a(:)) * (u(:) - &
