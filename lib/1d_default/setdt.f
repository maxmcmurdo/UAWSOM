...
subroutine setdt()
  ...
  if (dtpar<=zero) then
    ...
    tco_mype = zero
    Tmax_mype = zero
 !$OMP PARALLEL DO PRIVATE(igrid,qdtnew,dtnew,dx1) REDUCTION(min:dtmin_mype) REDUCTION(max:cmax_mype,a2max_mype)
    do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
      ...
      call getdt_courant(ps(igrid)%w,ixGlo1,ixGhi1,ixMlo1,ixMhi1,qdtnew,dx1,&
         ps(igrid)%x,cmax_mype,a2max_mype)
      dtnew=min(dtnew,qdtnew)

      call phys_get_dt(ps(igrid)%w,ixGlo1,ixGhi1,ixMlo1,ixMhi1,qdtnew,dx1,&
         ps(igrid)%x)
      dtnew=min(dtnew,qdtnew)

      if (associated(usr_get_dt)) then
         call usr_get_dt(ps(igrid)%w,ixGlo1,ixGhi1,ixMlo1,ixMhi1,qdtnew,dx1,&
            ps(igrid)%x)
      end if
      dtnew          = min(dtnew,qdtnew)
  ...
  if(phys_trac) then
    ...
    case(1)
      ...
      call MPI_ALLREDUCE(tco_mype,tco_global,1,MPI_DOUBLE_PRECISION,MPI_MAX,&
         icomm,ierrmpi)
  ...
  contains
    ...
    subroutine getdt_courant(w,ixImin1,ixImax1,ixOmin1,ixOmax1,dtnew,dx1,x,&
      ...
      double precision, intent(inout) :: w(ixImin1:ixImax1,1:nw), dtnew,&
          cmax_mype, a2max_mype(ndim)

      integer :: idims
      double precision :: courantmax, dxinv(1:ndim), courantmaxtot,&
          courantmaxtots
      double precision :: cmax(ixImin1:ixImax1), cmaxtot(ixOmin1:ixOmax1)
      ...
      if(need_global_a2max) then
        call phys_get_a2max(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,a2max)
        do idims=1,ndim
          a2max_mype(idims) = max(a2max_mype(idims),a2max(idims))
        end do
      end if
      if(phys_trac) then
        call phys_get_tcutoff(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x,tco_local,&
           Tmax_local)
        ...
        if(slab_uniform) then
          ...
          do idims=1,ndim
            ...
            if(need_global_cmax) cmax_mype = max(cmax_mype,&
               maxval(cmax(ixOmin1:ixOmax1)))
            cmaxtot(ixOmin1:ixOmax1)=cmaxtot(ixOmin1:ixOmax1)+&
               cmax(ixOmin1:ixOmax1)/block%ds(ixOmin1:ixOmax1,idims)
          end do
        end if
      ...
      case (type_summax)
        if(slab_uniform) then
          ...
          do idims=1,ndim
            ...
            if(need_global_cmax) cmax_mype = max(cmax_mype,&
               maxval(cmax(ixOmin1:ixOmax1)))
            courantmax=max(courantmax,maxval(cmax(ixOmin1:ixOmax1)/block%ds(&
