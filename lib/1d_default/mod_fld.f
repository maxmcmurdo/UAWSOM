...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine get_fld_rad_force(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,&
    ...
    double precision :: kappaCT(ixOmin1:ixOmax1)
    ...
    double precision :: fld_R(ixOmin1:ixOmax1), lambda(ixOmin1:ixOmax1)

    !> Calculate and add sourceterms
    if(qsourcesplit .eqv. fld_Radforce_split) then
      active = .true.

      call fld_get_opacity(wCT, x, ixImin1,ixImax1, ixOmin1,ixOmax1, kappaCT)
      call fld_get_radflux(wCT, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
          rad_fluxCT)
      call fld_get_fluxlimiter(wCT, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
          lambda, fld_R)

      do idir = 1,ndim
        !> Radiation force = kappa*rho/c *Flux = lambda gradE
        radiation_forceCT(ixOmin1:ixOmax1,&
           idir) = kappaCT(ixOmin1:ixOmax1)*rad_fluxCT(ixOmin1:ixOmax1,&
      ...
      call fld_get_eddington(wCt, x, ixImin1,ixImax1, ixOmin1,ixOmax1, edd)
  ...
  subroutine fld_radforce_get_dt(w,ixImin1,ixImax1,ixOmin1,ixOmax1,dtnew,dx1,&
     x)
    use mod_global_parameters
    use mod_usr_methods
    ...
    do idim = 1, ndim
      radiation_force(ixOmin1:ixOmax1,idim) = w(ixOmin1:ixOmax1,&
         iw_rho)*kappa(ixOmin1:ixOmax1)*rad_flux(ixOmin1:ixOmax1,&
  ...
  subroutine get_fld_energy_interact(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,&
     x,energy,qsourcesplit,active)
    use mod_constants
    use mod_global_parameters
    use mod_usr_methods
  ...
  subroutine fld_get_opacity(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
      fld_kappa)
    use mod_global_parameters
    ...
    integer :: i,j,ix1, idir

    select case (fld_opacity_law)
      case('const')
        fld_kappa(ixOmin1:ixOmax1) = fld_kappa0/unit_opacity
      case('thomson')
        fld_kappa(ixOmin1:ixOmax1) = fld_kappa0/unit_opacity
      case('kramers')
        rho0 = half !> Take lower value of rho in domain
        fld_kappa(ixOmin1:ixOmax1) = fld_kappa0/unit_opacity*((w(&
           ixOmin1:ixOmax1,iw_rho)/rho0))
      case('bump')
        !> Opacity bump
        rho0 = 0.2d0 !0.5d-1
        n = 7.d0
        ...
        fld_kappa(ixOmin1:ixOmax1) = fld_kappa0/unit_opacity*(one + &
      ...
      case('non_iso')
        ...
        n = -7.d0/two
        fld_kappa(ixOmin1:ixOmax1) = fld_kappa0/unit_opacity*(w(&
           ixOmin1:ixOmax1,iw_rho)/rho0)*(Temp(ixOmin1:ixOmax1)/Temp0)**n
      case('fastwind')
        ...
        do ix1=ixOmin1,ixOmax1
          ...
          !> Hard limit on kappa
          fld_kappa(ix1) = min(fld_kappa(ix1),2.3d0*fld_kappa0/unit_opacity)
      ...
      case('special')
        ...
        call usr_special_opacity(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x,&
            fld_kappa)
  ...
  subroutine fld_get_fluxlimiter(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
      fld_lambda, fld_R)
    use mod_global_parameters
    use mod_geometry
    use mod_usr_methods
    ...
    double precision, intent(in) :: w(ixImin1:ixImax1, 1:nw)
    double precision, intent(in) :: x(ixImin1:ixImax1, 1:ndim)
    double precision, intent(out) :: fld_R(ixOmin1:ixOmax1),&
        fld_lambda(ixOmin1:ixOmax1)
    double precision :: kappa(ixOmin1:ixOmax1)
    ...
    case('Diffusion')
      fld_lambda(ixOmin1:ixOmax1) = 1.d0/3.d0
      fld_R(ixOmin1:ixOmax1) = zero
    ...
    case('FreeStream')
      ...
      fld_R(ixOmin1:ixOmax1) = dsqrt(normgrad2(ixOmin1:ixOmax1))/(kappa(&
         ixOmin1:ixOmax1)*w(ixOmin1:ixOmax1,iw_rho)*w(ixOmin1:ixOmax1,iw_r_e))

      !> Calculate the flux limiter, lambda
      fld_lambda(ixOmin1:ixOmax1) = one/fld_R(ixOmin1:ixOmax1)
    ...
    case('Pomraning')
      ...
      fld_lambda(ixOmin1:ixOmax1) = (2.d0+fld_R(ixOmin1:ixOmax1))/(6.d0+&
         3*fld_R(ixOmin1:ixOmax1)+fld_R(ixOmin1:ixOmax1)**2.d0)
    ...
    case('Pomraning2')
      ...
      fld_lambda(ixOmin1:ixOmax1) = one/fld_R(ixOmin1:ixOmax1)*(one/dtanh(&
         fld_R(ixOmin1:ixOmax1)) - one/fld_R(ixOmin1:ixOmax1))
      ...
      where(dtanh(fld_R(ixOmin1:ixOmax1)) .ne. dtanh(fld_R(ixOmin1:ixOmax1)))
        fld_lambda(ixOmin1:ixOmax1) = 1.d0/3.d0
      endwhere
    ...
    case('Minerbo')
      ...
      do ix1=ixOmin1,ixOmax1
          ...
          if (fld_R(ix1) .lt. 3.d0/2.d0) then
            fld_lambda(ix1) = 2.d0/(3.d0 + dsqrt(9.d0 + &
               12.d0*fld_R(ix1)**2.d0))
          else
            fld_lambda(ix1) = 1.d0/(1.d0 + fld_R(ix1) + dsqrt(1.d0 + &
               2.d0*fld_R(ix1)))
          endif
      enddo
    ...
    if (flux_lim_filter) then
      ...
      tmp_L(ixOmin1:ixOmax1) = fld_lambda(ixOmin1:ixOmax1)
      ...
      fld_lambda(ixOmin1:ixOmax1) = tmp_L(ixOmin1:ixOmax1)
    endif
  ...
  subroutine fld_get_radflux(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, rad_flux)
    use mod_global_parameters
    use mod_geometry
    ...
    do idir = 1,ndim
      ...
      rad_flux(ixOmin1:ixOmax1, idir) = -&
         (const_c/unit_velocity)*lambda(ixOmin1:ixOmax1)/(kappa(&
         ixOmin1:ixOmax1)*w(ixOmin1:ixOmax1,&
  ...
  subroutine fld_get_eddington(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
      eddington_tensor)
    use mod_global_parameters
    use mod_geometry
    ...
    double precision :: normgrad2(ixOmin1:ixOmax1), f(ixOmin1:ixOmax1)
    ...
    f(ixOmin1:ixOmax1) = lambda(ixOmin1:ixOmax1) + &
       lambda(ixOmin1:ixOmax1)**two * fld_R(ixOmin1:ixOmax1)**two
    f(ixOmin1:ixOmax1) = one/two*(one-f(ixOmin1:ixOmax1)) + &
       one/two*(3.d0*f(ixOmin1:ixOmax1) - one)

    
    eddington_tensor(ixOmin1:ixOmax1,1,1) = f(ixOmin1:ixOmax1)
  ...
  subroutine fld_get_radpress(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
      rad_pressure)
    use mod_global_parameters
    ...
    call fld_get_eddington(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1,&
        eddington_tensor)
...
!0887 dt_diff = 0.d0
  ...
  subroutine put_diffterm_onegrid(ixImin1,ixImax1,ixOmin1,ixOmax1,w)
    ...
    do idir = 1,ndim
      ...
      divF_h(ixOmin1:ixOmax1) = w(ixOmin1:ixOmax1,i_diff_mg)*w(hxOmin1:hxOmax1,&
         i_diff_mg)/(w(ixOmin1:ixOmax1,i_diff_mg) + w(hxOmin1:hxOmax1,&
         i_diff_mg))*(w(hxOmin1:hxOmax1,iw_r_e) - w(ixOmin1:ixOmax1,iw_r_e))
      divF_j(ixOmin1:ixOmax1) = w(ixOmin1:ixOmax1,i_diff_mg)*w(jxOmin1:jxOmax1,&
         i_diff_mg)/(w(ixOmin1:ixOmax1,i_diff_mg) + w(jxOmin1:jxOmax1,&
         i_diff_mg))*(w(jxOmin1:jxOmax1,iw_r_e) - w(ixOmin1:ixOmax1,iw_r_e))
  ...
  subroutine fld_get_diffcoef_central(w, wCT, x, ixImin1,ixImax1, ixOmin1,&
     ixOmax1)
    use mod_global_parameters
    use mod_geometry
    use mod_usr_methods
    ...
    double precision :: max_D(ixImin1:ixImax1), grad_r_e(ixImin1:ixImax1),&
        rad_e(ixImin1:ixImax1)
    integer :: idir,i,j, ix1


    call fld_get_opacity(wCT, x, ixImin1,ixImax1, ixOmin1,ixOmax1, kappa)
    call fld_get_fluxlimiter(wCT, x, ixImin1,ixImax1, ixOmin1,ixOmax1, lambda,&
        fld_R)

    !> calculate diffusion coefficient
    w(ixOmin1:ixOmax1,i_diff_mg) = (const_c/unit_velocity)*lambda(&
       ixOmin1:ixOmax1)/(kappa(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,iw_rho))

    where (w(ixOmin1:ixOmax1,i_diff_mg) .lt. 0.d0) w(ixOmin1:ixOmax1,&
       i_diff_mg) = smalldouble

    if (diff_coef_filter) then
      !call mpistop('Hold your bloody horses, not implemented yet ')
      call fld_smooth_diffcoef(w, ixImin1,ixImax1, ixOmin1,ixOmax1)
    endif
  ...
  subroutine fld_smooth_diffcoef(w, ixImin1,ixImax1, ixOmin1,ixOmax1)
    use mod_global_parameters
    ...
    tmp_D(ixOmin1:ixOmax1) = w(ixOmin1:ixOmax1,i_diff_mg)
    filtered_D(ixOmin1:ixOmax1) = zero

    do filter = 1,size_D_filter
      do ix1 = ixOmin1+size_D_filter,ixOmax1-size_D_filter
        do idim = 1,ndim
          filtered_D(ix1) = filtered_D(ix1) + tmp_D(ix1+filter*kr(idim,&
             1)) + tmp_D(ix1-filter*kr(idim,1))
        enddo
      enddo
    enddo
    ...
    w(ixOmin1:ixOmax1,i_diff_mg) = tmp_D(ixOmin1:ixOmax1)
  ...
  subroutine Energy_interaction(w, wCT, x, ixImin1,ixImax1, ixOmin1,ixOmax1)
    ...
    do ix1=ixOmin1,ixOmax1
      ...
      e_gas(ix1) = max(e_gas(ix1),small_pressure/(fld_gamma-1.d0))
    enddo
    ...
    if (fld_interaction_method .eq. 'Instant') then
      a1(ixOmin1:ixOmax1) = const_rad_a*(fld_mu*const_mp/const_kB*(fld_gamma-&
      ...
      a1(ixOmin1:ixOmax1) = 4*kappa(ixOmin1:ixOmax1)*sigma_b*(fld_gamma-&
