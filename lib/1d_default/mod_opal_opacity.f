...
subroutine set_opal_opacity(rho,temp,kappa)
  ...
  call get_kappa(Kappa_vals, Log_R_list, Log_T_list, R_input, T_input,&
      K_output)
...
subroutine interpolate_two_tables(Y1, Y2, Y_in, K1, K2, K_interp)
    ...
    do colum=2,20
    do row=7,76
        call interpolate1D(Y1,Y2,Y_in,K1(row,colum),K2(row,colum),K_interp(row,&
           colum))
    enddo
    enddo
...
subroutine get_kappa(Kappa_vals, Log_R_list, Log_T_list, R, T, K)
    ...
    call interpolate_KRT(low_r_index, up_r_index, low_t_index, up_t_index,&
        Log_R_list, Log_T_list, Kappa_vals, R, T, K)
...
subroutine interpolate_KRT(low_r, up_r, low_t, up_t, Log_R_list, Log_T_list,&
    Kappa_vals, R, T, k_interp)
