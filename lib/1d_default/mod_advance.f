...
contains
  ...
  subroutine advect1(method,dtfactor,idimmin,idimmax,qtC,psa,qt,psb)
    ...
    do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
       ...
       call process1_grid(method(block%level),igrid,qdt,ixGlo1,ixGhi1,idimmin,&
          idimmax,qtC,psa(igrid),qt,psb(igrid),pso(igrid))
    end do
    !$OMP END PARALLEL DO
    ...
    if (fix_conserve_global .and. fix_conserve_at_step) then
      ...
      if(stagger_grid) then
        ...
        !$OMP PARALLEL DO PRIVATE(igrid)
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          call phys_face_to_center(ixMlo1,ixMhi1,psb(igrid))
        end do
        !$OMP END PARALLEL DO
      end if
      if(phys_solve_eaux) then
        ...
        !$OMP PARALLEL DO PRIVATE(igrid)
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          call phys_energy_synchro(ixGlo1,ixGhi1,ixMlo1,ixMhi1,psb(igrid)%w,&
             psb(igrid)%x)
        end do
        !$OMP END PARALLEL DO
      end if
  ...
  subroutine process1_grid(method,igrid,qdt,ixImin1,ixImax1,idimmin,idimmax,&
     qtC,sCT,qt,s,sold)
    use mod_global_parameters
    use mod_fix_conserve
    ...
    call advect1_grid(method,qdt,ixImin1,ixImax1,idimmin,idimmax,qtC,sCT,qt,s,&
       sold,fC,fE,rnode(rpdx1_:rnodehi,igrid),ps(igrid)%x)
    ...
    if (fix_conserve_global .and. fix_conserve_at_step) then
      ...
      if(stagger_grid) call store_edge(igrid,ixImin1,ixImax1,fE,idimmin,&
         idimmax)
    end if
  ...
  subroutine advect1_grid(method,qdt,ixImin1,ixImax1,idimmin,idimmax,qtC,sCT,&
     qt,s,sold,fC,fE,dxs,x)
    ...
    integer, intent(in) :: method
    integer, intent(in) :: ixImin1,ixImax1, idimmin,idimmax
    ...
    case (fs_hll,fs_hllc,fs_hllcd,fs_hlld,fs_tvdlf,fs_tvdmu)
       call finite_volume(method,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,&
          idimmax,qtC,sCT,qt,s,sold,fC,fE,dxs,x)
    case (fs_cd,fs_cd4)
       call centdiff(method,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,&
          idimmax,qtC,sCT,qt,s,fC,fE,dxs,x)
    case (fs_hancock)
       call hancock(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,idimmax,qtC,&
          sCT,qt,s,dxs,x)
    case (fs_fd)
       call fd(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,idimmax,qtC,sCT,qt,&
          s,fC,fE,dxs,x)
    case (fs_tvd)
       call centdiff(fs_cd,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,idimmax,&
          qtC,sCT,qt,s,fC,fE,dxs,x)
       call tvdlimit(method,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,&
          idimmax,sCT,qt+qdt,s,fC,dxs,x)
    case (fs_source)
       call addsource2(qdt*dble(idimmax-idimmin+1)/dble(ndim),ixImin1,ixImax1,&
          ixOmin1,ixOmax1,1,nw,qtC,sCT%w,qt,s%w,x,.false.)
    case (fs_nul)
  ...
  subroutine process(iit,qt)
    ...
    if (associated(usr_process_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         dxlevel(1)=rnode(rpdx1_,igrid);
         block=>ps(igrid)
         call usr_process_grid(igrid,node(plevel_,igrid),ixGlo1,ixGhi1,ixMlo1,&
            ixMhi1, qt,ps(igrid)%w,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
  ...
  subroutine process_advanced(iit,qt)
    ...
    if (associated(usr_process_adv_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call usr_process_adv_grid(igrid,node(plevel_,igrid),ixGlo1,ixGhi1,&
            ixMlo1,ixMhi1, qt,ps(igrid)%w,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
      call getbc(qt,dt,ps,iwstart,nwgc,phys_req_diagonal)
    end if
