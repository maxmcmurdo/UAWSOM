...
111   close(unitpar)
  ...
  subroutine init_gridvars()
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      allocate(gridvars(igrid)%w(ixGlo1:ixGhi1,1:ngridvars))
      gridvars(igrid)%w = 0.0d0

      if (time_advance) then
        allocate(gridvars(igrid)%wold(ixGlo1:ixGhi1,1:ngridvars))
  ...
  subroutine fill_gridvars_default
    ...
    integer :: igrid, iigrid
    double precision :: E(ixGlo1:ixGhi1, ndir)
    double precision :: B(ixGlo1:ixGhi1, ndir)

    do iigrid=1,igridstail; igrid=igrids(iigrid);
      if (associated(usr_particle_fields)) then
        call usr_particle_fields(ps(igrid)%w, ps(igrid)%x, E, B)
        gridvars(igrid)%w(ixGlo1:ixGhi1,ep(:)) = E
        gridvars(igrid)%w(ixGlo1:ixGhi1,bp(:)) = B
      else
      ...
      if (time_advance) then
        if (associated(usr_particle_fields)) then
          ...
          gridvars(igrid)%wold(ixGlo1:ixGhi1,ep(:)) = E
          gridvars(igrid)%wold(ixGlo1:ixGhi1,bp(:)) = B
        else
  ...
  subroutine fields_from_mhd(igrid, w_mhd, w_part)
    ...
    double precision, intent(in)    :: w_mhd(ixGlo1:ixGhi1,nw)
    double precision, intent(inout) :: w_part(ixGlo1:ixGhi1,ngridvars)
    integer                         :: idirmin, idir
    double precision                :: current(ixGlo1:ixGhi1,7-2*ndir:3)
    double precision                :: w(ixGlo1:ixGhi1,1:nw)

    dxlevel(1)=rnode(rpdx1_,igrid);

    w(ixGlo1:ixGhi1,1:nw) = w_mhd(ixGlo1:ixGhi1,1:nw)
    w_part(ixGlo1:ixGhi1,bp(1):bp(3)) = 0.0d0
    w_part(ixGlo1:ixGhi1,ep(1):ep(3)) = 0.0d0

    call phys_to_primitive(ixGlo1,ixGhi1,ixGlo1,ixGhi1,w,ps(igrid)%x)

    ! fill with magnetic field:
    if (B0field) then
      do idir = 1, ndir
        w_part(ixGlo1:ixGhi1,bp(idir))=w(ixGlo1:ixGhi1,&
           iw_mag(idir))+ps(igrid)%B0(ixGlo1:ixGhi1,idir,0)
      end do
    else
      w_part(ixGlo1:ixGhi1,bp(:)) = w(ixGlo1:ixGhi1,iw_mag(:))
    end if
!    w_part(ixG^T,bp(:)) = w(ixG^T,iw_mag(:))

    ! fill with current
    current = zero
    call particle_get_current(w,ixGlo1,ixGhi1,ixGlo1+1,ixGhi1-1,idirmin,&
       current)
    w_part(ixGlo1:ixGhi1,jp(:)) = current(ixGlo1:ixGhi1,:)

    ! fill with electric field
    w_part(ixGlo1:ixGhi1,ep(1)) = w_part(ixGlo1:ixGhi1,&
       bp(2)) * w(ixGlo1:ixGhi1,iw_mom(3)) - w_part(ixGlo1:ixGhi1,&
       bp(3)) * w(ixGlo1:ixGhi1,iw_mom(2)) + particles_eta * &
       current(ixGlo1:ixGhi1,1)
    w_part(ixGlo1:ixGhi1,ep(2)) = w_part(ixGlo1:ixGhi1,&
       bp(3)) * w(ixGlo1:ixGhi1,iw_mom(1)) - w_part(ixGlo1:ixGhi1,&
       bp(1)) * w(ixGlo1:ixGhi1,iw_mom(3)) + particles_eta * &
       current(ixGlo1:ixGhi1,2)
    w_part(ixGlo1:ixGhi1,ep(3)) = w_part(ixGlo1:ixGhi1,&
       bp(1)) * w(ixGlo1:ixGhi1,iw_mom(2)) - w_part(ixGlo1:ixGhi1,&
       bp(2)) * w(ixGlo1:ixGhi1,iw_mom(1)) + particles_eta * &
       current(ixGlo1:ixGhi1,3)

    ! Hall term
    if (particles_etah > zero) then
      w_part(ixGlo1:ixGhi1,ep(1)) = w_part(ixGlo1:ixGhi1,&
         ep(1)) + particles_etah/w(ixGlo1:ixGhi1,&
         iw_rho) * (current(ixGlo1:ixGhi1,2) * w_part(ixGlo1:ixGhi1,&
         bp(3)) - current(ixGlo1:ixGhi1,3) * w_part(ixGlo1:ixGhi1,bp(2)))
      w_part(ixGlo1:ixGhi1,ep(2)) = w_part(ixGlo1:ixGhi1,&
         ep(2)) + particles_etah/w(ixGlo1:ixGhi1,&
         iw_rho) * (-current(ixGlo1:ixGhi1,1) * w_part(ixGlo1:ixGhi1,&
         bp(3)) + current(ixGlo1:ixGhi1,3) * w_part(ixGlo1:ixGhi1,bp(1)))
      w_part(ixGlo1:ixGhi1,ep(3)) = w_part(ixGlo1:ixGhi1,&
         ep(3)) + particles_etah/w(ixGlo1:ixGhi1,&
         iw_rho) * (current(ixGlo1:ixGhi1,1) * w_part(ixGlo1:ixGhi1,&
         bp(2)) - current(ixGlo1:ixGhi1,2) * w_part(ixGlo1:ixGhi1,bp(1)))
    end if
  ...
  subroutine particle_get_current(w,ixImin1,ixImax1,ixOmin1,ixOmax1,idirmin,&
     current)
    use mod_global_parameters
    use mod_geometry
    ...
    if (B0field) then
      do idir = 1, ndir
        bvec(ixImin1:ixImax1,idir)=w(ixImin1:ixImax1,&
           iw_mag(idir))+block%B0(ixImin1:ixImax1,idir,0)
      end do
    else
      do idir = 1, ndir
        bvec(ixImin1:ixImax1,idir)=w(ixImin1:ixImax1,iw_mag(idir))
      end do
    end if
...
!        call write_particle_output()
  ...
  subroutine get_vec(ix,igrid,x,tloc,vec)
    ...
    if (associated(usr_particle_analytic)) then
      ...
      do idir=1,ndir
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%w(ixGlo1:ixGhi1,ix(idir)),&
            ps(igrid)%x(ixGlo1:ixGhi1,1:ndim),x,vec(idir))
      end do
    else
      do idir=1,ndir
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%wold(ixGlo1:ixGhi1,ix(idir)),&
            ps(igrid)%x(ixGlo1:ixGhi1,1:ndim),x,vec1(idir))
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%w(ixGlo1:ixGhi1,ix(idir)),&
            ps(igrid)%x(ixGlo1:ixGhi1,1:ndim),x,vec2(idir))
      end do
  ...
  subroutine interpolate_var(igrid,ixImin1,ixImax1,ixOmin1,ixOmax1,gf,x,xloc,&
     gfloc)
    use mod_global_parameters
...
!    if (particle(index)%self%follow) print*, 'follow index:', index
  ...
  logical function particle_in_domain(x)
    ...
    particle_in_domain = all(x >= [ xprobmin1 ]) .and. all(x < [ xprobmax1 ]) !Jannis: as before < instead of <= here, necessary?
  ...
  logical function particle_in_igrid(ipart, igrid)
    ...
    if (.not. allocated(ps(igrid)%w)) then
      ...
      grid_rmin         = [ rnode(rpxmin1_,igrid) ]
      grid_rmax         = [ rnode(rpxmax1_,igrid) ]
  ...
  subroutine set_neighbor_ipe()
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      case (neighbor_coarse) ! fine-coarse
        call ipe_fc(i1,igrid,ipe_is_neighbor)
      case (neighbor_sibling) ! same level
        call ipe_srl(i1,igrid,ipe_is_neighbor)
      case (neighbor_fine) ! coarse-fine
        call ipe_cf(i1,igrid,ipe_is_neighbor)
      end select
  ...
  subroutine ipe_fc(i1,igrid,ipe_is_neighbor)
    use mod_global_parameters
    integer, intent(in) :: i1, igrid
    logical, intent(inout) :: ipe_is_neighbor(0:npe-1)

    ipe_is_neighbor(neighbor(2,i1,igrid)) = .true.

  end subroutine ipe_fc

  subroutine ipe_srl(i1,igrid,ipe_is_neighbor)
    use mod_global_parameters
    integer, intent(in) :: i1, igrid
    logical, intent(inout) :: ipe_is_neighbor(0:npe-1)
  ...
  subroutine ipe_cf(i1,igrid,ipe_is_neighbor)
    use mod_global_parameters
    integer, intent(in)    :: i1, igrid
    logical, intent(inout) :: ipe_is_neighbor(0:npe-1)
    integer                :: ic1, inc1
    ...
    ipe_is_neighbor( neighbor_child(2,inc1,igrid) ) = .true.
