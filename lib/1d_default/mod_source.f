...
contains
  ...
  subroutine add_split_source(prior)
    ...
    case (sourcesplit_sfs)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         call addsource2(0.5d0*dt,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nw,qt,&
            ps(igrid)%w,qt,ps(igrid)%w,ps(igrid)%x,.true.,src_active)
      end do
      !$OMP END PARALLEL DO
    case (sourcesplit_sf)
      !$OMP PARALLEL DO PRIVATE(igrid)
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         block=>ps(igrid)
         dxlevel(1)=rnode(rpdx1_,igrid);
         call addsource2(dt  ,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nw,qt,ps(igrid)%w,&
    ...
    case (sourcesplit_ssf)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         block=>ps(igrid)
         dxlevel(1)=rnode(rpdx1_,igrid);
         call addsource2(0.5d0*dt,ixGlo1,ixGhi1,ixGlo1,ixGhi1,1,nw,qt,&
            ps(igrid)%w,qt,ps(igrid)%w,ps(igrid)%x,.true.,src_active)
         call addsource2(dt  ,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nw,qt,ps(igrid)%w,&
            qt,ps(igrid)%w,ps(igrid)%x,.true.,src_active)
      end do
    ...
    case (sourcesplit_ssfss)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         block=>ps(igrid)
         dxlevel(1)=rnode(rpdx1_,igrid);
         call addsource2(0.25d0*dt,ixGlo1,ixGhi1,ixGlo1,ixGhi1,1,nw,qt,&
            ps(igrid)%w,qt,ps(igrid)%w,ps(igrid)%x,.true.,src_active)
         call addsource2(0.5d0*dt,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nw,qt,&
            ps(igrid)%w,qt,ps(igrid)%w,ps(igrid)%x,.true.,src_active)
      end do
  ...
  subroutine addsource2(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,iwmin,iwmax,qtC,&
     wCT,qt,w,x,qsourcesplit,src_active,wCTprim)
    use mod_global_parameters
    ...
    call phys_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,&
       qsourcesplit,tmp_active,wCTprim)
