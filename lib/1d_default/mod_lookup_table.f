...
contains
  ...
  pure function LT_get_mcol_at_loc(my_lt, loc) result(col_values)
    ...
    col_values = loc%low_frac * my_lt%cols_rows(:,&
        loc%low_ix) + (1-loc%low_frac) * my_lt%cols_rows(:, loc%low_ix+1)
  ...
  elemental function LT_get_col_at_loc(my_lt, col_ix, loc) result(col_value)
    ...
    col_value = loc%low_frac * my_lt%rows_cols(loc%low_ix,&
        col_ix) + (1-loc%low_frac) * my_lt%rows_cols(loc%low_ix+1, col_ix)
  ...
  pure subroutine LT2_set_col(my_lt, col_ix, x1, x2, y)
    ...
    do ix = 1, my_lt%n_points(1)
       my_lt%rows_cols(ix, :, col_ix) = LT_get_spaced_data(x2, tmp(ix, :), c2)
    end do
    deallocate(c1,c2)
  ...
  elemental function LT2_get_col_at_loc(my_lt, col_ix, loc) result(col_value)
    ...
    col_value = sum(w * my_lt%rows_cols(ix(1):ix(1)+1, ix(2):ix(2)+1, col_ix))
  ...
  elemental function LT3_get_col_at_loc(my_lt, col_ix, loc) result(col_value)
    ...
    col_value = sum(w * my_lt%rows_cols(ix(1):ix(1)+1, ix(2):ix(2)+1,&
        ix(3):ix(3)+1, col_ix))
