...
!-----------------------------------------------------------------------------
   ...
   do i1=-1,1
      if (i1==0) then 
         if (isafety(igrid,mype) > nsafety) neighbor_active(i1,&
            igrid) = .false.
      end if
...
integer function igrid_active(igrid)
  ...
  !-----------------------------------------------------------------------------
  ixOmin1=ixGlo1+nghostcells;ixOmax1=ixGhi1-nghostcells;

  igrid_active = -1

  if (associated(usr_flag_grid)) then
     call usr_flag_grid(global_time,ixGlo1,ixGhi1,ixOmin1,ixOmax1,ps(igrid)%w,&
